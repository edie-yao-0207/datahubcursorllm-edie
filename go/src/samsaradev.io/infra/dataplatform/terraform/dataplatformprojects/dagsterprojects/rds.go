...
const (
	defaultParameterGroup = "default.aurora-postgresql14"
	engine                = "aurora-postgresql"
	engineVersion         = "14.17"
	instanceClass         = infraconsts.DEPRECATED_T4G_Medium
)
...
func rdsResources(config dataplatformconfig.DatabricksConfig, v vpcIds) (map[string][]tf.Resource, error) {
	...
	clusterIdentifier := fmt.Sprintf("%s-prod-pg", DagsterResourceBaseName)
	dbSubnetGroup := fmt.Sprintf("%s-%s-dbsubnet", DagsterResourceBaseName, config.Region)
	...
	cluster := rds.Cluster{
		...
		VpcSecurityGroupIds: []string{
			"${aws_security_group.dagster-vpn-ingress.id}",  // (tf managed in load balancer)
			eksClusterSecurityGroupId,                       // (created by EKS; not tf managed)
			"${aws_security_group.dagster-vpc-internal.id}", // (tf managed in vpc)
		},
		...
		Tags:                  getDagsterTags("rds-cluster"),
	}
	...
	writerInstance := rds.ClusterInstance{
		...
		Tags:                    getDagsterTags("rds-cluster-instance"),
		Engine:                  pointer.StringPtr(engine),
		EngineVersion:           pointer.StringPtr(engineVersion),
