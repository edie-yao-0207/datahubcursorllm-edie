...
WHERE
key  IN ('asset-utilization-v2-enabled')
AND date = (
SELECT MAX(date)
FROM datamodel_launchdarkly_bronze.segment_flag
WHERE date <= GREATEST('2024-01-15', '2025-10-01')
)
),
variations
AS (
...
WHERE
ffv.feature_flag_key  IN ('asset-utilization-v2-enabled')
AND ffv.date = (
SELECT MAX(date)
...
WHERE
feature_flag_key  IN ('asset-utilization-v2-enabled')
AND date = (
SELECT MAX(date)
FROM datamodel_launchdarkly_bronze.feature_flag_environment
WHERE date <= GREATEST('2024-01-15', '2025-10-01')
...
SELECT MAX(date)
FROM datamodel_launchdarkly_bronze.rule_clause rc
WHERE rc.date <= GREATEST('2024-01-15', '2025-10-01')
)
),
relevant_rules
AS (
...
WHERE
o.date = '2024-01-15'
AND DATE(fo._run_dt) BETWEEN DATE_SUB('2024-01-15', 6)
AND '2024-01-15'
AND (
(fo.net_quantity > 0)
)
AND (
fo.product_sku IN ('LIC-VG-ENT', 'LIC-VG-ENT-PLTFM-ESS', 'LIC-VG-ENT-PLTFM-PREM', 'LIC-VG-ENTERPRISE', 'LIC-VG-ESS-PLTFM-ESS', 'LIC-VG-ESS-PLTFM-PREM', 'LIC-VG-ESS-PLUS-PLTFM-ESS', 'LIC-VG-ESS-PLUS-PLTFM-PREM', 'LIC-VG-EXPRESS', 'LIC-VG-LD', 'LIC-VG-NOELD', 'LIC-VG-PREM-PLTFM-ESS', 'LIC-VG-PREM-PLTFM-PREM', 'LIC-VG-PREMIER-PLTFM-ESS', 'LIC-VG-PREMIER-PLTFM-PREM', 'LIC-VG-PREMIER-PS', 'LIC-VG-PS')
)
UNION ALL
SELECT DISTINCT o.org_id, fo.product_sku AS sku, xref.is_legacy_license
FROM edw.silver.fct_license_orders_daily_snapshot fo
...
ON o.id = fpss.org_id
WHERE
fp.ld_feature_key  IN ('asset-utilization-v2-enabled')
...
ON fv.feature_flag_key  IN ('asset-utilization-v2-enabled')
LEFT OUTER
JOIN pivoted_conditions_exploded ldo -- Joining to see what specifically is enabled
ON
(
