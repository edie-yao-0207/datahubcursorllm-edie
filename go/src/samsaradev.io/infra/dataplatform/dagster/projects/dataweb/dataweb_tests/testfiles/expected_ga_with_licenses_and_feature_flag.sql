...
WHERE
key  IN ('unified-coaching-enabled')
AND date = (
SELECT MAX(date)
FROM datamodel_launchdarkly_bronze.segment_flag
...
WHERE
ffv.feature_flag_key  IN ('unified-coaching-enabled')
AND ffv.date = (
SELECT MAX(date)
...
WHERE
feature_flag_key  IN ('unified-coaching-enabled')
AND date = (
SELECT MAX(date)
FROM datamodel_launchdarkly_bronze.feature_flag_environment
WHERE date <= GREATEST('2025-09-15', '2025-10-01')
...
AND (
fo.product_sku IN ('LIC-VG-ENT', 'LIC-VG-EXPRESS', 'LIC-VG-LD', 'LIC-VG-NOELD', 'LIC-VG-PS', 'LIC-VG-ESS-PLUS-PLTFM-ESS', 'LIC-VG-ESS-PLUS-PLTFM-PREM', 'LIC-VG-ENT-PLTFM-PREM', 'LIC-VG-ENT-PLTFM-ESS', 'LIC-VG-COMPLIANCE')
)
UNION ALL
SELECT DISTINCT o.org_id, fo.product_sku AS sku, xref.is_legacy_license
FROM edw.silver.fct_license_orders_daily_snapshot fo
...
relevant_licenses_legacy
AS (
SELECT DISTINCT org_id
FROM relevant_licenses
WHERE is_legacy_license
),
relevant_licenses_core
AS (
SELECT DISTINCT org_id
FROM relevant_licenses
WHERE is_legacy_license = FALSE
AND sku NOT IN ('LIC-VG-COMPLIANCE')
),
relevant_licenses_add_on
AS (
SELECT DISTINCT org_id
FROM relevant_licenses
WHERE sku IN ('LIC-VG-COMPLIANCE')
),
...
ON fv.feature_flag_key  IN ('unified-coaching-enabled')
LEFT OUTER
JOIN pivoted_conditions_exploded ldo -- Joining to see what specifically is enabled
ON
(
