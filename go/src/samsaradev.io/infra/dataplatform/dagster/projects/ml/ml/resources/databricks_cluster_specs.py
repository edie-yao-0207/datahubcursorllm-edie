...
DEFAULT_CLUSTER_LIBRARIES = [
    ...
    {"pypi": {"package": "pyproj==3.7.1"}},
    # ML packages for anomaly detection
    {"pypi": {"package": "numpy==2.2.6"}},
    {"pypi": {"package": "pandas==2.3.2"}},
    {"pypi": {"package": "scikit-learn==1.7.0"}},
    {"pypi": {"package": "scipy==1.14.1"}},
    {"pypi": {"package": "h3==4.3.1"}},
...
def build_step_config(
    ...
    logger.info(f"[build_step_config] region={region}, env={env}, mode={mode}")
    logger.info(
        f"[build_step_config] Will use existing cluster: {(env == 'dev' or mode in ['DRY_RUN', 'LOCAL_DATAHUB_RUN']) and mode != 'LOCAL_PROD_CLUSTER_RUN'}"
    )
    logger.info(f"[build_step_config] account_id={account_id}, prefix={prefix}")
    ...
    logger.info(f"[build_step_config] databricks_cluster_id={databricks_cluster_id}")
    ...
    if databricks_cluster_id:
        ...
        logger.info(
            f"[build_step_config] Using EXISTING cluster: {databricks_cluster_id}"
        )
        logger.info(f"[build_step_config] Token available: {bool(databricks_token)}")
    else:
        ...
        logger.info(f"[build_step_config] Creating NEW job cluster (production mode)")
...
class SamsaraDatabricksStepLauncher(DatabricksPySparkStepLauncher):
    ...
    def _handle_exception(self, exception: Exception) -> None:
        ...
        # Re-raise unknown exceptions instead of swallowing them
        raise exception
    ...
    def _get_databricks_task(self, run_id: str, step_key: str) -> Mapping[str, Any]:
        ...
        This will create a 'spark_python_task' dict where `python_file` is a path on DBFS/S3
    ...
    def launch_step(self, step_context: StepExecutionContext) -> Iterator[DagsterEvent]:
        ...
        logger = logging.getLogger(__name__)
        logger.info("[SamsaraDatabricksStepLauncher] launch_step called")
        logger.info(
            f"[SamsaraDatabricksStepLauncher] Has existing cluster: {bool(self.run_config['cluster'].get('existing'))}"
        )

        if self.run_config["cluster"].get("existing"):
            logger.info(
                f"[SamsaraDatabricksStepLauncher] Using existing cluster: {self.run_config['cluster']['existing']}"
            )
        else:
            logger.info("[SamsaraDatabricksStepLauncher] Will create new job cluster")

        self._with_hooks(step_context)

        if self.run_config["cluster"].get("existing"):
            logger.info(
                "[SamsaraDatabricksStepLauncher] Launching on EXISTING cluster (dev mode)"
            )
            yield from super().launch_step(step_context)

        else:
            logger.info(
                "[SamsaraDatabricksStepLauncher] Launching on NEW job cluster (prod mode)"
            )
            ...
            try:
                ...
                step_context.log.error(
                    f"[SamsaraDatabricksStepLauncher] Error during launch_step: {e}"
                )
                step_context.log.error(
                    f"[SamsaraDatabricksStepLauncher] Traceback: {traceback.format_exc()}"
                )
...
class ConfigurableDatabricksStepLauncher(ConfigurableResource):
    ...
    def create_resource(self, context) -> SamsaraDatabricksStepLauncher:
        ...
        logger = logging.getLogger(__name__)
        logger.info("[ConfigurableDatabricksStepLauncher] create_resource called")
        ...
        logger.info(
            f"[ConfigurableDatabricksStepLauncher] Building step config for region={region}"
        )
        ...
        if config["run_config"]["cluster"].get("existing"):
            logger.info(
                f"[ConfigurableDatabricksStepLauncher] Using EXISTING cluster: {config['run_config']['cluster']['existing']}"
            )
            return SamsaraDatabricksStepLauncher(config)
        else:
            logger.info("[ConfigurableDatabricksStepLauncher] Using NEW job cluster")
        ...
        # Set data security mode for Unity Catalog compatibility
        if context.resource_config.get("data_security_mode"):
            config["run_config"]["cluster"]["new"][
                "data_security_mode"
            ] = context.resource_config["data_security_mode"]
        if context.resource_config.get("single_user_name"):
            config["run_config"]["cluster"]["new"][
                "single_user_name"
            ] = context.resource_config["single_user_name"]
