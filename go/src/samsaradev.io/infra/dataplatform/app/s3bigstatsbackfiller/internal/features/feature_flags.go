 ...
 * GO_DIR="infra/dataplatform/app/s3bigstatsbackfiller" taskrunner gen/feature_flags/backend
 *
...
func (user *BackendUser) AsBackendLdUser() *lduser.User {
	...
	user.Do(func() {
		...
		custom["s3bigstat"] = ldvalue.String(user.S3bigstat)
...
const FFKeyS3bigstatsBackfillEnabled = "s3bigstats-backfill-enabled"

func S3bigstatsBackfillEnabledOrErrorWithBridge(user interfaces.BackendUser, bridge *launchdarkly.SDKBridge) (bool, error) {
	return utils.BoolVariationOrErrorWithBridge(
		bridge,
		FFKeyS3bigstatsBackfillEnabled,
		user,
...
// https://app.launchdarkly.com/samsara/production/features/s3bigstats-backfill-enabled/targeting
func S3bigstatsBackfillEnabledWithBridge(user interfaces.BackendUser, bridge *launchdarkly.SDKBridge) bool {
	value, _ := S3bigstatsBackfillEnabledOrErrorWithBridge(user, bridge)
	return value
}
