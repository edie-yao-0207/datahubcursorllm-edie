...
awsregion="us-west-2"
uri="${registry}/datahub-frontend-prod"
tag="${uri}"
...
helm upgrade --install prerequisites . --namespace=datahub-prod  --values values.yaml
popd > /dev/null
...
cp -f "values.yaml" "values_prod_with_tag.yaml"
sed -i "s/\$COMMIT_SHA_SHORT_frontend/${COMMIT_SHA_SHORT}/g" "values_prod_with_tag.yaml"

helm upgrade --install datahub . --namespace=datahub-prod  --values values_prod_with_tag.yaml --timeout 45m --debug
popd > /dev/null
