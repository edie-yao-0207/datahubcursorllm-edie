select
  ...
  round(
    PERCENTILE_CONT(0.5) WITHIN GROUP (
      ORDER BY
        (
          to_unix_timestamp(s.created_at) - (s.event_ms + ko.value.received_delta_seconds * 1000) / 1000
        )
    ) / 60,
    2
  ) as p50_latency_mins,
...
from
  ...
  inner join kinesisstats.osdaccelerometer ko on ko.value.proto_value.accelerometer_event.event_id = s.detail_proto.event_id
...
where
  ...
  and a.event_type in (
    ...
    'haPhonePolicy',
    'haRolloverProtectionBrakeControlAct',
    'haRolloverProtectionEngineControlAc',
    'haSeatbeltPolicy',
    'haSharpTurn',
    'haSmokingPolicy',
    'haTailgating',
