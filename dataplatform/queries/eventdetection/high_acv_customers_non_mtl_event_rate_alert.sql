-- Purpose: SQL query to alert when the rate of persisted non mtl events for high ACV customers drops to 0.
WITH high_acv_safety_events AS (
  ...
  AND value.proto_value.accelerometer_event.ml_run_tag IS null
  AND a.event_type in (
      'haAccel',
      'haBraking',
      'haCameraMisaligned',
