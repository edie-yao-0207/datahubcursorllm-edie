select cast(date as date) as date, sum(same_org) as same_org, count(*) - sum(same_org) as diff_orgs
from
(select han.date,
        case when han.peripheral_org_id = han.scanner_org_id then 1 else 0 end as same_org
