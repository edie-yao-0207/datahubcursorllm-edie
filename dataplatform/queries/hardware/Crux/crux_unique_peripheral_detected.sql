select coalesce(x.date,y.date) as date, x.total_assets, y.total_orgs
from (select cast(date as date) as date, count(distinct(peripheral_asset_id)) as total_assets
...
group by date) as x
join (select cast(date as date) as date, count(distinct(han.peripheral_org_id)) as total_orgs
from hardware_analytics.at11_data as han
join clouddb.organizations as o1
on han.peripheral_org_id = o1.id
join clouddb.organizations as o2
