select cast(last_detected_date as date) as last_detected_date, count(distinct(peripheral_asset_id)) as cnt
...
where last_detected_date < current_date()-1
group by last_detected_date
order by last_detected_date