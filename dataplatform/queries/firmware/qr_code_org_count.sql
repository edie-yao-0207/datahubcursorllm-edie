create or replace temp view cohort as (
    ...
    and (cm_product_name in ('CM32', 'CM34')) -- QR code only runs on dual facing cameras
...
select
  a.date,
  count(distinct a.org_id) as count
from dataprep_firmware.device_daily_qr_code_enabled a
