...
create or replace temp view per_trip_hour as (
	select
		date,
		org_id,
		cm_device_id,
		(1000 * 60 * 60) * overheated_count / (end_ms - start_ms) as overheated_state_transitions_per_trip_hour
	from dataprep_safety.cm_device_health_intervals
	where on_trip = true
);

select
	a.date,
	percentile_approx(overheated_state_transitions_per_trip_hour, 0.5) as median,
  percentile_approx(overheated_state_transitions_per_trip_hour, 0.9) as p90,
	percentile_approx(overheated_state_transitions_per_trip_hour, 0.99) as p99,
	percentile_approx(overheated_state_transitions_per_trip_hour, 0.999) as p999
from per_trip_hour as a
