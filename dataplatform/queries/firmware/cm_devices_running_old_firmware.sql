...
create or replace temp view devices_on_old_firmware as (
  select
    date,
    last_reported_cm_build as latest_build_on_day,
    count(*) as num_devices
  from active_devices
  where datediff(month, regexp_extract(last_reported_cm_build, '^\\d{4}-\\d{2}-\\d{2}', 0), date) >= 6
  group by
    date,
    last_reported_cm_build
);
