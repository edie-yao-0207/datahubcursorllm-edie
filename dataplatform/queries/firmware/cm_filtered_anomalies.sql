...
create or replace temp view per_device as (
  select
    a.date,
    a.org_id,
    a.device_id,
    first(a.total_trip_duration_ms) / (60 * 60 * 1000) as trip_duration_hours
  from dataprep_firmware.device_daily_filtered_anomalies_by_service as a
...
create or replace temp view denom as (
  select
    date,
    sum(trip_duration_hours) as total_trip_duration_hours,
...
select
  ...
  b.anomaly_service,
  a.device_count,
  sum(coalesce(b.num_anomalies, 0)) as num_anomalies,
  sum(coalesce(b.num_anomalies, 0)) / a.device_count as filtered_anomalies_per_device,
  sum(coalesce(b.num_anomalies, 0)) / a.total_trip_duration_hours as filtered_anomalies_per_trip_hour
from denom as a
left join dataprep_firmware.device_daily_filtered_anomalies_by_service as b
  on a.date = b.date
join daily_cohort as c
  on b.date = c.date
  and b.org_id = c.org_id
  and b.device_id = c.cm_device_id
group by
  a.date,
  b.anomaly_service,
  a.device_count,
