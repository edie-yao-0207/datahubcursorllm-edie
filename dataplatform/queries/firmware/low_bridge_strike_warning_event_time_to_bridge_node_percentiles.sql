...
create or replace temp view time_to_bridge_node as (
  select
    b.date,
    (a.distance_to_bridge_node_meters * 0.001) / a.gps_speed_kmph * 3600 as time_to_bridge_node_secs
  from
    dataprep_firmware.low_bridge_strike_warning_events as a
...
select
  date,
  mean(time_to_bridge_node_secs) as avg,
  percentile_approx(time_to_bridge_node_secs, 0.01) as p01,
  percentile_approx(time_to_bridge_node_secs, 0.1) as p10,
  percentile_approx(time_to_bridge_node_secs, 0.25) as p25,
  percentile_approx(time_to_bridge_node_secs, 0.5) as median,
  percentile_approx(time_to_bridge_node_secs, 0.75) as p75,
  percentile_approx(time_to_bridge_node_secs, 0.90) as p90,
  percentile_approx(time_to_bridge_node_secs, 0.99) as p99
from
  time_to_bridge_node
group by
  date
