...
create or replace temp view device_daily as (
	select
		...
		a.device_id,
		a.day_phase,
		sum(a.duration_ms) / count(*) as avg_duration_ms
	from dataprep_firmware.qr_code_scan_motions as a
...
select
	a.date,
	a.day_phase,
	percentile_approx(a.avg_duration_ms, 0.5) as median,
    percentile_approx(a.avg_duration_ms, 0.90) as p90,
	percentile_approx(a.avg_duration_ms, 0.99) as p99,
	percentile_approx(a.avg_duration_ms, 0.999) as p999
