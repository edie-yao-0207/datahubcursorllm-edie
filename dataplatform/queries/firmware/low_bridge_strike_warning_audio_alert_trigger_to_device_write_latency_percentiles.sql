...
create or replace temp view audio_alert_latency as (
  select
    *,
    device_write_start_ms - trigger_time_ms as trigger_to_device_write_latency_ms
  from
    dataprep_firmware.low_bridge_strike_warning_audio_alerts
  where
    device_write_start_ms > 0 -- not set in shadow mode
);

select
  b.date,
  mean(trigger_to_device_write_latency_ms) as avg,
  percentile_approx(trigger_to_device_write_latency_ms, 0.5) as median,
  percentile_approx(trigger_to_device_write_latency_ms, 0.75) as p75,
  percentile_approx(trigger_to_device_write_latency_ms, 0.90) as p90,
  percentile_approx(trigger_to_device_write_latency_ms, 0.99) as p99
from
