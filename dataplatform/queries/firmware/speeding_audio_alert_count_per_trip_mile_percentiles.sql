...
-- Calculate alert count per trip mile for each device
create or replace temp view audio_alert_count_per_trip_mile as (
  select
    b.date,
    b.org_id,
    b.device_id,
    coalesce(sum(a.count), 0) / b.trip_miles as count
  from
...
from
  audio_alert_count_per_trip_mile
group by
  date
