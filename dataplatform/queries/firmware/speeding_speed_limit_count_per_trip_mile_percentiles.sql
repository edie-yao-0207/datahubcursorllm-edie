...
-- Filtered devices with miles
create or replace temp view filtered_devices_with_miles as (
  select
    a.date,
    a.org_id,
...
-- Calculate speed limit count per trip mile
create or replace temp view speed_limit_count_per_trip_mile as (
  select
    b.date,
    b.org_id,
    b.device_id,
    coalesce(sum(a.count), 0) / b.trip_miles as count_per_trip_mile
  from
    dataprep_firmware.speeding_speed_limit_count as a
    right join filtered_devices_with_miles as b
      on a.date = b.date
      and a.org_id = b.org_id
...
select
  date,
  percentile_approx(count_per_trip_mile, 0.01) as p1,
  percentile_approx(count_per_trip_mile, 0.10) as p10,
  percentile_approx(count_per_trip_mile, 0.5) as median,
  percentile_approx(count_per_trip_mile, 0.90) as p90,
  percentile_approx(count_per_trip_mile, 0.99) as p99
from
  speed_limit_count_per_trip_mile
group by
  date
