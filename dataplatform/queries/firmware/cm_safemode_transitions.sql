...
create or replace temp view per_trip_hour as (
	select
		...
		org_id,
		cm_device_id,
		(1000 * 60 * 60) * safe_mode_count / (end_ms - start_ms) as safe_mode_state_transitions_per_trip_hour
...
select
	a.date,
	percentile_approx(safe_mode_state_transitions_per_trip_hour, 0.5) as median,
  percentile_approx(safe_mode_state_transitions_per_trip_hour, 0.9) as p90,
	percentile_approx(safe_mode_state_transitions_per_trip_hour, 0.99) as p99,
	percentile_approx(safe_mode_state_transitions_per_trip_hour, 0.999) as p999
