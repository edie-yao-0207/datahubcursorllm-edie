...
create or replace temp view per_device as (
  ...
  from dataprep_firmware.device_daily_anomalies_by_service as a
...
select
  ...
  sum(coalesce(b.num_anomalies, 0)) / a.device_count as anomalies_per_device,
  sum(coalesce(b.num_anomalies, 0)) / a.total_trip_duration_hours as anomalies_per_trip_hour
from denom as a
left join dataprep_firmware.device_daily_anomalies_by_service as b
  on a.date = b.date
