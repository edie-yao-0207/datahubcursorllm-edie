...
-- Filtered devices with miles and audio alerts enabled
create or replace temp view filtered_devices_with_miles as (
  ...
  where b.audio_alerts_enabled = true
);

-- Calculate total trip miles
create or replace temp view total_trip_miles as (
  select
    date,
    sum(trip_miles) as total_trip_miles
  from
    filtered_devices_with_miles
  group by
    date
);

-- Calculate daily alert count
create or replace temp view daily_count as (
  select
    a.date,
    sum(a.count) as count
  from
    dataprep_firmware.speeding_audio_alert_count as a
    join filtered_devices_with_miles as b
      on a.date = b.date
      and a.org_id = b.org_id
...
-- Calculate count per trip mile
select
  a.date,
  b.count / a.total_trip_miles as count_per_trip_mile
from
  total_trip_miles as a
  join daily_count as b
    on a.date = b.date
order by
  a.date desc
