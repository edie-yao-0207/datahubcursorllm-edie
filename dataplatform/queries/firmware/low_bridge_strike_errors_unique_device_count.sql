...
create or replace temp view filtered_devices as (
  ...
  from
    daily_cohort as a
    join dataprep_firmware.low_bridge_strike_daily_devices as b
      on a.date = b.date
...
select
  a.date,
  a.metric_name,
  coalesce(count(distinct a.device_id), 0) as count
from
  dataprep_firmware.low_bridge_strike_error_count as a
