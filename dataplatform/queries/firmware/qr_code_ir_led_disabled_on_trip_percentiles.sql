-- Create a cohort of devices based on the filterable attributes.
...
-- Join the cohort with the daily metadata, filtering by date and firmware.
...
-- Create a temporary view to calculate count per trip for each device
create or replace temp view device_daily as (
  select
    a.date,
    a.org_id,
    a.device_id,
    sum(b.count) / sum(a.count) as count_per_trip
  from dataprep_firmware.qr_code_trip_count as a
  left join dataprep_firmware.qr_code_ir_led_disabled_on_trip as b
    on a.org_id = b.org_id
    and a.device_id = b.device_id
    and a.date = b.date
  join daily_cohort as c
    on a.date = c.date
    and a.org_id = c.org_id
    and a.device_id = c.cm_device_id
  group by
    a.date,
    a.org_id,
    a.device_id
...
-- Calculate percentiles of IR LED disabled count per trip
select
  a.date,
  percentile_approx(count_per_trip, 0.5) as median,
  percentile_approx(count_per_trip, 0.90) as p90,
  percentile_approx(count_per_trip, 0.99) as p99,
  percentile_approx(count_per_trip, 0.999) as p999
from device_daily as a
group by a.date
