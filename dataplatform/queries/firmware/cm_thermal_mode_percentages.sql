...
-- Compute thermal mode duration for each device
create or replace temporary view cm_thermal_mode as (
  SELECT
    a.date,
    a.device_id,
    a.cm_device_id,
    b.cm_product_name,
    a.org_id,
    total_overheated_duration_ms,
    total_safe_mode_duration_ms,
...
create or replace temp view thermal_agg as (
  select
    date,
    sum(total_overheated_duration_ms) / sum(total_trip_duration_ms) as overheated_ratio,
    sum(total_safe_mode_duration_ms) / sum(total_trip_duration_ms) as safemode_ratio,
    1 - (
      sum(
        total_safe_mode_duration_ms + total_overheated_duration_ms
      ) / sum(total_trip_duration_ms)
    ) as thermal_uptime,
    100 * (
      sum(total_overheated_duration_ms) / sum(total_trip_duration_ms)
    ) as overheated_pct,
    100 * (
      sum(total_safe_mode_duration_ms) / sum(total_trip_duration_ms)
    ) as safemode_pct,
    100 * (
      1 - (
        sum(
