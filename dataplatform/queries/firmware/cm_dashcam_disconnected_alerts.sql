...
select
  date(from_unixtime(wi.occurred_at_ms / 1000)) as incident_date,
  count(*) as count
from workflowsdb_shards.workflow_incidents wi
join workflowsdb_shards.triggers t
  on t.workflow_uuid = wi.workflow_id
 and t.type = 1012
where wi.org_id in (select distinct org_id from daily_cohort)
  and date(from_unixtime(wi.occurred_at_ms / 1000))
