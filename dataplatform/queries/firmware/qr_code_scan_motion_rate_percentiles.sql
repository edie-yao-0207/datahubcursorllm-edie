...
create or replace temp view scan_motions_per_device as (
	select
		date,
		day_phase,
		org_id,
		device_id,
		count(*) as count
	from dataprep_firmware.qr_code_scan_motions
	group by
		date,
		day_phase,
		org_id,
...
create or replace temp view device_daily as (
	select
		...
		a.org_id,
		a.device_id,
		b.count / a.count as count_per_trip
	from dataprep_firmware.qr_code_trip_count as a
	left join scan_motions_per_device as b
		on a.org_id = b.org_id
...
select
	a.date,
	a.day_phase,
	percentile_approx(a.count_per_trip, 0.5) as median,
    percentile_approx(a.count_per_trip, 0.90) as p90,
	percentile_approx(a.count_per_trip, 0.99) as p99,
	percentile_approx(a.count_per_trip, 0.999) as p999
