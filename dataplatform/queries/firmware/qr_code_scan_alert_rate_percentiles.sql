...
create or replace temp view device_daily as (
	select
		...
		a.org_id,
		a.device_id,
		sum(coalesce(b.count, 0)) / sum(a.count) as count_per_trip
	from dataprep_firmware.qr_code_trip_count as a
	left join dataprep_firmware.qr_code_scan_alert_count as b
		on a.org_id = b.org_id
		and a.device_id = b.device_id
		and a.date = b.date
		and a.day_phase = b.day_phase
...
select
	a.date,
	day_phase,
	percentile_approx(count_per_trip, 0.001) as p01,
	percentile_approx(count_per_trip, 0.01) as p1,
	percentile_approx(count_per_trip, 0.1) as p10,
