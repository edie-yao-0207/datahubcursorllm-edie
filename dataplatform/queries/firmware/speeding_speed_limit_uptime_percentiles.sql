...
create or replace temp view speed_limit_uptime as (
  select
    a.date,
    a.org_id,
    a.device_id,
    sum(a.has_speed_limit_duration_ms) / sum(a.total_trip_duration_ms) as uptime
  from
    dataprep_firmware.speeding_map_matching_speedlimit_uptime as a
...
select
  date,
  percentile_approx(uptime, 0.01) as p1,
  percentile_approx(uptime, 0.05) as p5,
  percentile_approx(uptime, 0.1) as p10,
  percentile_approx(uptime, 0.5) as median,
  percentile_approx(uptime, 0.90) as p90,
  percentile_approx(uptime, 0.99) as p99
from
  speed_limit_uptime
group by
  date
