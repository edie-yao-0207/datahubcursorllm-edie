...
with daily_org_device_count as (
	select
		a.date,
		a.org_id,
		count(*) as count
	from
		dataprep_firmware.tile_based_rolling_stop_daily_active_devices as a
...
daily_org_device_count_with_rankings as (
	select
		date,
		org_id,
		count,
		row_number() over(partition by date order by count desc) as day_rank
	from
