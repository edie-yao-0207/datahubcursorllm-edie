...
select
  a.date,
  sum(a.total_connection_attempts) as total_connection_attempts,
  sum(a.successful_connection_attempts) as successfully_connected,
  sum(a.total_connection_attempts) - sum(a.successful_connection_attempts) as failed_to_connect,
  sum(a.successful_connection_attempts) / sum(a.total_connection_attempts) as success_ratio
from dataprep_firmware.cm_device_daily_livestream_attempts as a
