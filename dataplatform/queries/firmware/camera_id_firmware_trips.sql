...
create or replace temp view daily_cohort as (
  ...
  join datamodel_core_silver.stg_device_activity_daily as cm_activity
    on cm_activity.date = a.date
    and cm_activity.device_id = a.cm_device_id
    and cm_activity.is_active
  join datamodel_core_silver.stg_device_activity_daily as vg_activity
    on vg_activity.date = a.date
    and vg_activity.device_id = a.vg_device_id
    and vg_activity.is_active
...
from (
  ...
  where true
    ...
    and trips.date = from_unixtime(trips.trip_end_ms / 1000, "yyyy-MM-dd")
    and trips.trip_end_ms - trips.trip_start_ms > 4 * 60 * 1000
  group by
    trips.date,
    trips.vg_device_id,
    trips.trip_start_ms
