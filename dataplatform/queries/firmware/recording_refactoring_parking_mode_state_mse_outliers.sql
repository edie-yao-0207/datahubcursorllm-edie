...
with weighted_mse as (
  select
    a.date,
    a.device_id,
    sum( (is_actual_parking_mode - is_target_parking_mode) * (is_actual_parking_mode - is_target_parking_mode) * (end_ms - start_ms))
...
wse_stats as (
  select
    date,
    avg(weighted_mse) as mean_wse,
    stddev(weighted_mse) as stddev_wse
  from
    weighted_mse
  group by
    date
...
from
  weighted_mse w
  join wse_stats ws on w.date = ws.date
where
  w.weighted_mse > ws.mean_wse + 2 * ws.stddev_wse
  and ( 'all' in ({{ day_type }}) or day_type in ({{ day_type }}) )
order by
  date desc,
  w.weighted_mse desc
