...
select
  a.date,
  percentile_approx(max_time_from_first_push_to_reboot_ms, 0.5) / (60 * 60  * 1000) as median_max_time_from_first_push_to_reboot_hours,
  percentile_approx(max_delta_download_duration_ms, 0.5) / (60 * 60  * 1000) as median_max_delta_download_duration_hours,
  percentile_approx(max_delta_apply_duration_ms, 0.5) / (60 * 60  * 1000) as median_max_delta_apply_duration_hours,
  percentile_approx(max_full_script_download_duration_ms, 0.5) / (60 * 60  * 1000) as median_max_full_script_download_duration_hours,
  percentile_approx(max_upgrade_script_duration_ms, 0.5) / (60 * 60  * 1000) as median_max_upgrade_script_duration_hours,
  percentile_approx(max_wait_duration_ms, 0.5) / (60 * 60  * 1000) as median_max_wait_duration_hours
from dataprep_firmware.cm_device_daily_upgrade_stats as a
