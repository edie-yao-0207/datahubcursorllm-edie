...
with metrics as (
  select
    a.date,
    a.device_id,
    sum(case when is_actual_parking_mode = 1 and is_target_parking_mode = 1 then 1 else 0 end ) /
      sum(case when is_target_parking_mode = 1 then 1 else 0 end ) as precision,
    sum(case when is_actual_parking_mode = 1 and is_target_parking_mode = 1 then 1 else 0 end )  /
      sum(case when is_actual_parking_mode = 1 then 1 else 0 end ) as recall,
    case
      when dayofweek(a.date) in (1, 7) then 'Weekend'
      else 'Weekday'
    end as day_type
  from
    dataprep_firmware.cm_actual_and_target_parking_mode_intervals_daily a
    join daily_cohort b on a.date = b.date and a.device_id = b.cm_device_id
  group by
    a.date,
    a.device_id
)
select
  date,
  percentile_approx(precision, 0.01) as precision_p01,
  percentile_approx(precision, 0.05) as precision_p05,
  percentile_approx(precision, 0.10) as precision_p10,
  percentile_approx(precision, 0.20) as precision_p20,
  percentile_approx(recall, 0.01) as recall_p01,
  percentile_approx(recall, 0.05) as recall_p05,
  percentile_approx(recall, 0.10) as recall_p10,
  percentile_approx(recall, 0.20) as recall_p20
from
  metrics
where
   ( 'all' in ({{ day_type }}) or day_type in ({{ day_type }}) )
group by
  date
