...
create or replace temp view edl_time_occurrences as (
  select
    c.cm_device_id as cm_device_id,
    sum(scm_end_ms - scm_start_ms) as edl_duration_ms,
    sum(total_trip_scm_duration_ms) as edl_trip_duration_ms,
    avg(total_trip_scm_duration_ms) as avg_edl_trip_duration_ms,
    count(*) as num_edl,
    a.scm_start_date
  from data_analytics.scm_327_trips a
    join daily_cohort c on c.vg_device_id = a.vg_device_id and c.date = date(a.scm_start_date)
  where
    a.scm_start_date between ('{{ date_range.start }}')
    and ('{{ date_range.end }}')
  group by
    cm_device_id,
    scm_start_date
...
create or replace temporary view cm_recording_uptime as (
  SELECT
    ...
    a.cm_device_id,
    a.org_id,
    h.last_reported_cm_build,
    h.cm_last_heartbeat_date,
    h.total_trip_recording_duration_ms,
    h.total_trip_grace_recording_duration_ms,
    h.total_trip_connected_duration_ms,
    h.total_trip_duration_ms,
    (
      h.total_trip_grace_recording_duration_ms / h.total_trip_duration_ms
    ) as recording_uptime_pct,
    (
      h.total_trip_grace_recording_duration_ms / h.total_trip_connected_duration_ms
    ) as recording_uptime_connected_pct,
    coalesce(c.edl_trip_duration_ms, 0)/h.total_trip_duration_ms as edl_downtime_trip_pct,
    coalesce(c.edl_duration_ms, 0) as edl_duration_ms,
    coalesce(c.edl_trip_duration_ms, 0) as edl_trip_duration_ms,
    coalesce(c.avg_edl_trip_duration_ms, 0) as avg_edl_trip_duration_ms
  FROM
    daily_cohort as a
  join
    dataprep_safety.cm_device_health_daily as h
  on
    a.cm_device_id = h.cm_device_id and a.date = h.date
  left join
    edl_time_occurrences as c
  on
    a.cm_device_id = c.cm_device_id and a.date = c.scm_start_date
  WHERE
    h.total_trip_duration_ms > 600000
    and h.cm_last_heartbeat_date > date_sub(a.date, 1) -- remove CMs without heartbeat
);
...
SELECT
  ...
  SUM(edl_trip_duration_ms) as total_edl_trip_time,
  ...
  SUM(edl_trip_duration_ms) / SUM(total_trip_duration_ms) as total_edl_downtime,
  AVG(avg_edl_trip_duration_ms)/1000 as average_edl_trip_duration_sec,
  ...
  percentile_approx(edl_downtime_trip_pct, 0.50) as edl_downtime_median,
  percentile_approx(edl_downtime_trip_pct, 0.99) as edl_downtime_p99,
  percentile_approx(edl_downtime_trip_pct, 0.999) as edl_downtime_p999
FROM
  cm_recording_uptime
GROUP BY
  1
