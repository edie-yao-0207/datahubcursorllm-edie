...
create or replace temp view filtered_devices as (
  ...
  from
    daily_cohort as a
    join dataprep_firmware.low_bridge_strike_daily_active_devices as b
      on a.date = b.date
      and a.cm_device_id = b.device_id
      and a.org_id = b.org_id
);

create or replace temp view audio_alert_latency as (
  select
    *,
    playback_start_ms - trigger_time_ms as trigger_to_playback_start_latency_ms
  from
    dataprep_firmware.low_bridge_strike_warning_audio_alerts
  where
    playback_start_ms > 0 -- ensure valid playback timestamp
);

select
  b.date,
  mean(trigger_to_playback_start_latency_ms) as avg,
  percentile_approx(trigger_to_playback_start_latency_ms, 0.5) as median,
  percentile_approx(trigger_to_playback_start_latency_ms, 0.75) as p75,
  percentile_approx(trigger_to_playback_start_latency_ms, 0.90) as p90,
  percentile_approx(trigger_to_playback_start_latency_ms, 0.99) as p99
from
  audio_alert_latency as a
