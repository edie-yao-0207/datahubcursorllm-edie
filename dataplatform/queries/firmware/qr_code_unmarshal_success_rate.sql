...
create or replace temp view device_daily as (
	select
		...
		sum(
			case
				when value.proto_value.q_r_code_scan_debug.scan_status >= 4 then 1
				else 0
			end
		) as numerator,
		count(*) as denominator
	from dataprep_firmware.qr_code_debug
	where value.proto_value.q_r_code_scan_debug.scan_status >= 3
	group by
		date,
		day_phase,
		org_id,
...
select
	a.date,
	a.day_phase,
	percentile_approx(a.numerator / a.denominator, 0.5) as median,
    percentile_approx(a.numerator / a.denominator, 0.1) as p10,
	percentile_approx(a.numerator / a.denominator, 0.01) as p1,
	percentile_approx(a.numerator / a.denominator, 0.001) as p01
