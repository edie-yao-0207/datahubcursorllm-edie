...
select
  a.date,
   case -- https://github.com/samsara-dev/firmware/blob/master/samsara/go/src/third_party_modified/samsaradev.io/hubproto/maptileproto/speedlimits.proto#L6
      when a.speed_limit_source = 0 then "NONE"
      when a.speed_limit_source = 1 then "SAMSARA"
      when a.speed_limit_source = 2 then "CUSTOMER"
      when a.speed_limit_source = 4 then "IOWA_DOT"
      else a.speed_limit_source
    end as speed_limit_source_string,
    case -- https://github.com/samsara-dev/firmware/blob/master/samsara/go/src/third_party_modified/samsaradev.io/hubproto/maptileproto/map_tile.proto#L34
      when a.speed_limit_layer_source = 0 then "NONE"
      when a.speed_limit_layer_source = 1 then "OSRM_BASE"
      when a.speed_limit_layer_source = 2 then "SPEEDLIMIT"
      when a.speed_limit_layer_source = 3 then "SPEEDLIMIT_CUSTOMER_OVERRIDE"
      when a.speed_limit_layer_source = 6 then "SPEEDLIMIT_EXPERIMENTAL"
      else a.speed_limit_layer_source
    end as speed_limit_layer_source_string,
...
select
  date,
  concat_ws("-", speed_limit_source_string, speed_limit_layer_source_string) as speed_limit_source_combined,
  count
from filtered_query
