...
with metrics as (
  select
    a.date,
    a.device_id,
    sum(case when is_actual_recording = 1 and is_target_recording = 1 then 1 else 0 end ) /
      sum(case when is_target_recording = 1 then 1 else 0 end ) as precision,
    sum(case when is_actual_recording = 1 and is_target_recording = 1 then 1 else 0 end )  /
      sum(case when is_actual_recording = 1 then 1 else 0 end ) as recall,
    case when dayofweek(a.date) in (1, 7) then 'Weekend' else 'Weekday' end as day_type
  from dataprep_firmware.cm_actual_and_target_recording_intervals_daily a
