create or replace temp view cohort as (
	...
	where date = least(('{{ date_range.end }}'), string(date_sub(current_date(), 1)))
...
select
  ...
  reason,
  case reason
    when 1 then "External request"
    when 2 then "Parking mode"
    when 3 then "Camera button"
    when 4 then "Panic button"
    when 5 then "Always on"
    when 6 then "Trip"
    when 7 then "Accel"
    when 8 then "Movement"
    when 9 then "Engine state"
    when 10 then "Equipment activity"
    when 11 then "Engine state (unknown)"
    when 12 then "Engine state (unsupported)"
    else cast(reason as string)
  end as reason_name,
  count(reason)
from dataprep_firmware.cm_dashcam_state_intervals as a
join daily_cohort as b
  on a.date = b.date
  and a.org_id = b.org_id
  and a.device_id = b.cm_device_id
lateral view explode(recording_reason) exploded_reasons as reason
where a.date between ('{{ date_range.start }}') and ('{{ date_range.end }}')
group by a.date, reason, reason_name
