...
create or replace temp view daily_org_device_count as (
	select
		a.date,
		a.org_id,
		count(*) as active_device_count
	from
		dataprep_firmware.low_bridge_strike_daily_active_devices as a
...
create or replace temp view daily_org_device_count_with_rankings as (
	select
		date,
		org_id,
		active_device_count,
		row_number() over(partition by date order by active_device_count desc) as day_rank
	from
		daily_org_device_count
);
...
select
	...
	active_device_count
from
	daily_org_device_count_with_rankings
where
	day_rank <= 10
order by
	date desc,
