...
select
  ...
  sum(a.trip_count) as total_trip_count,
  sum(a.trip_miles) as total_trip_miles,
  sum(a.trip_count) / count(*) as trip_count_per_device,
  sum(a.trip_miles) / count(*) as trip_miles_per_device
from dataprep_firmware.tile_based_rolling_stop_daily_active_devices as a
