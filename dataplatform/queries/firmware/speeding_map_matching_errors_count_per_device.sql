...
create or replace temp view filtered_devices as (
  ...
  from
    daily_cohort as a
    join dataprep_firmware.speeding_daily_devices as b
      on a.date = b.date
...
create or replace temp view daily_count as (
  ...
  from
    dataprep_firmware.speeding_map_matching_errors_count as a
...
select
  a.date,
  b.count / a.device_count as count
from
  device_count as a
  join daily_count as b on a.date = b.date
order by
  a.date desc
