...
select
  a.date,
  a.harsh_event_type,
  sum(a.total_event_occurence) as num_events,
  60 * (sum(a.total_event_occurence) / sum(t.total_duration_mins)) as num_events_per_trip_hour
...
join daily_cohort as c
  ...
  and a.linked_cm_id = c.cm_device_id
where a.date between ('{{ date_range.start }}')
  and ('{{ date_range.end }}')
  and t.date between ('{{ date_range.start }}')
  and ('{{ date_range.end }}')
group by
  a.date,
  a.harsh_event_type
