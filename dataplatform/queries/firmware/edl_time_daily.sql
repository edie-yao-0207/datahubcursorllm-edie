...
-- Create view for EDL time occurrences
create or replace temp view edl_time_occurrences as (
  select
    c.cm_device_id as cm_device_id,
    avg(scm_end_ms - scm_start_ms) as edl_duration_ms,
    avg(total_trip_scm_duration_ms) as edl_trip_duration_ms,
    count(*) as num_edl,
    a.scm_start_date
...
-- Compute total EDL time
select
  scm_start_date as date,
  sum(edl_duration_ms) as total_edl_time_ms,
  sum(edl_trip_duration_ms) as total_edl_trip_time_ms,
  sum(num_edl) as total_edl_occurences
from
  edl_time_occurrences
group by
  1
order by
  1 desc