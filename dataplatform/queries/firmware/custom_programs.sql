...
select
  c.product_program_id,
  d.build,
  e.name,
  e.description,
  count(*) as device_count
from (
  select * from dataprep_firmware.firmware_upgrade_status_by_reason
  where reason = "custom firmware program or override"
) as a
join cohort as b
  on a.org_id = b.org_id
	and a.device_id = b.cm_device_id
join firmwaredb.rollout_stage_firmwares c
  on a.product_id = c.product_id
  and b.cm_product_program_id = c.product_program_id
  and b.cm_rollout_stage_id = c.rollout_stage
join firmwaredb.product_firmwares as d
  on a.product_id = d.product_id
  and c.product_firmware_id = d.id
join firmwaredb.product_programs as e
  on a.product_id = e.product_id
  and b.cm_product_program_id = e.id
group by
  c.product_program_id,
  d.build,
  e.name,
  e.description
order by c.product_program_id desc
