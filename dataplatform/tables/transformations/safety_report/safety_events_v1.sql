WITH safety_events_v1 AS (
  SELECT
    ...
    tev2.trip_start_ms as tev2_trip_start_ms,
  ...
  FROM
    ...
    -- We can now join with triage_v2 as all the sev1 data is backfilled into triage_v2
    -- from 2016 but does not include internal orgs. Do a left join to err on the side of keeping safety events, pending product alignment.
    LEFT JOIN safetyeventtriagedb_shards.triage_events_v2 tev2
    ON tev2.org_id = events.org_id
    AND tev2.device_id = events.device_id
    AND tev2.start_ms = events.event_ms
    -- Only include version 1 which are SEV1 events
    AND version = 1
    ...
    -- We need this here as safety_events.trip_start_ms is not always accurate and is sometimes 0
    AND trips.start_ms = IF(tev2.trip_start_ms IS NULL OR tev2.trip_start_ms = 0, events.trip_start_ms, tev2.trip_start_ms)
...
SELECT
  ...
  driver_id,
  event_ms,
  (CASE WHEN tev2_trip_start_ms != 0 THEN tev2_trip_start_ms ELSE trip_start_ms END) as trip_start_ms,
  trip_end_ms,
  trip_version,
  additional_labels,
  in_dismissed_states,
