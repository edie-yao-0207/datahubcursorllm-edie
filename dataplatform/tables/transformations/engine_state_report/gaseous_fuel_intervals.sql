WITH gaseous_fuel_stats AS (
  SELECT *
  FROM kinesisstats.osdfuelconsumedgaseous
...
FROM (
  SELECT
    ...
    value.int_value AS gaseous_fuel_consumed_grams
  FROM gaseous_fuel_stats
)
WHERE
  ...
  AND date >= ${start_date}
  AND date < ${end_date}
  AND gaseous_fuel_consumed_grams >= 0
