...
device_bootcount_vg AS (
  ...
  FROM
    ...
    JOIN osdhubserverdeviceheartbeat ON osdhubserverdeviceheartbeat.object_id = cm_vg_intervals.device_id
    AND osdhubserverdeviceheartbeat.org_id = cm_vg_intervals.org_id
    AND osdhubserverdeviceheartbeat.time >= cm_vg_intervals.start_ms - 5 * 60 * 1000
    AND osdhubserverdeviceheartbeat.time <= cm_vg_intervals.end_ms
    AND osdhubserverdeviceheartbeat.date = cm_vg_intervals.date
  GROUP BY
    cm_vg_intervals.org_id,
    cm_vg_intervals.device_id,
    cm_vg_intervals.cm_device_id,
...
device_build_and_bootcount_vg AS (
  ...
  FROM
    device_bootcount_vg
    JOIN osdhubserverdeviceheartbeat ON osdhubserverdeviceheartbeat.object_id = device_bootcount_vg.device_id
