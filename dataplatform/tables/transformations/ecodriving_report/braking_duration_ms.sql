...
-- Get instances of friction braking during the date range
osd_aggregate_friction_braking_ms AS (
    SELECT
        date,
        org_id,
        object_id,
        ...
        value.int_value AS duration_ms
    FROM
        kinesisstats.osDAggregateDurationFrictionBrakingMs
    WHERE
        date >= ${start_date}
        AND date < ${end_date}
),
...
combined_braking AS (
    ...
    SELECT
        ...
        duration_ms
    FROM
        osd_aggregate_friction_braking_ms
),
