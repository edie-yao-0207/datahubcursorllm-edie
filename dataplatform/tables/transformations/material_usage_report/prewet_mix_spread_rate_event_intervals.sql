/*
 * This node computes prewet total quantity spread from the SpreadRatePrewetLitersPerTonne
 * stat by multiplying each stat with the closest dry spread rate stat on or before it to
 * get a spread rate in (volume/distance) units.
 */

-- Include invalid spread rate states, which are assumed to have a
-- value of 0 so we can correctly account for time periods where
-- spread rate was unknwon as 0
WITH prewet_mix_spread_rates AS (
  SELECT
    org_id,
  ...
  FROM
    kinesisstats.osdsaltspreaderspreadrateprewetliterspertonne
  WHERE
...
spreader_event_intervals AS (
  SELECT
    *
  FROM material_usage_report.spreader_event_intervals
...
spread_rates_within_events AS (
  SELECT
    ...
    sei.spreader_end_ms,
    sei.mode,
    prewet_mix_spread_rates.time AS spread_rate_time,
    spread_rate
  FROM
    spreader_event_intervals AS sei
    LEFT JOIN prewet_mix_spread_rates ON prewet_mix_spread_rates.time BETWEEN sei.spreader_start_ms
    AND sei.spreader_end_ms
    AND prewet_mix_spread_rates.org_id = sei.org_id
    AND prewet_mix_spread_rates.device_id = sei.device_id
),
...
spread_rates_before_events AS (
  ...
  FROM
    ...
    LEFT JOIN prewet_mix_spread_rates ON prewet_mix_spread_rates.time < intervals.spreader_start_ms
    AND prewet_mix_spread_rates.time >= (
      intervals.spreader_start_ms - 1000 * 60
    )
    AND prewet_mix_spread_rates.org_id = intervals.org_id
    AND prewet_mix_spread_rates.device_id = intervals.device_id
),
...
spreader_event_prewet_mix_spread_rates AS (
  SELECT
    *
  FROM
    spread_rates_within_events
...
dry_spread_rates AS (
  SELECT
    ...
    value.int_value AS spread_rate
  FROM
    kinesisstats.osdsaltspreadernormalizedspreadratedrymilligramspermeter
  WHERE
    date >= DATE_SUB(${start_date}, 1)
    AND date < ${end_date}
),
spreader_event_prewet_mix_with_dry_spread_rates AS (
  select
   prewet.org_id,
   prewet.device_id,
   prewet.date,
mode,
   ...
   spreader_end_ms,
   spread_rate_time,
   prewet.spread_rate as prewet_liters_per_tonne,
   max((dry.spread_rate, dry.time)).spread_rate as dry_mg_per_meter,
   mode
 from spreader_event_prewet_mix_spread_rates prewet join dry_spread_rates dry
 on prewet.org_id = dry.org_id and prewet.device_id = dry.device_id and dry.time <= prewet.spread_rate_time
 group by prewet.org_id, prewet.device_id, prewet.date, spreader_start_ms, spreader_end_ms, spread_rate_time, prewet.spread_rate, mode
),
/*
  ...
  FROM
    (
      SELECT
        ...
        prewet_liters_per_tonne,
        dry_mg_per_meter,
        -- L/tonne * (ml/L) * (tonne / mg) * mg/m = ml/m
        prewet_liters_per_tonne / 1000000 * dry_mg_per_meter as spread_rate,
      ...
      FROM
        spreader_event_prewet_mix_with_dry_spread_rates
    )
    -- Exclude intervals where either computed prewet spread rate is
...
SELECT
  ...
  spread_rate * distance_covered_meters as prewet_material_spread_ml,
  time_spread_ms
FROM
