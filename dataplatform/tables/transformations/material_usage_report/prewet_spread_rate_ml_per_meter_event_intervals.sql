...
WITH spread_rates AS (
  ...
  FROM
    kinesisstats.osdsaltspreaderspreadrateprewetmilliliterspermeter
  WHERE
...
spread_rate_intervals AS (
  ...
  FROM
    ...
    -- Exclude intervals where spread rate is 0 since
    -- as those are intervals where no material is used
    WHERE spread_rate > 0
),
...
SELECT
  ...
  END AS prewet_material_spread_ml,
  time_spread_ms
FROM
