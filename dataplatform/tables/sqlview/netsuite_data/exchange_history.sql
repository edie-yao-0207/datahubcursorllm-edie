...
WITH item_receipt_serials AS (
 ...
 item_receipt_inventory_numbers.serial AS serial_number,
 item_receipts_lines.item_id,
 items.item_name AS sku


FROM edw.netsuite_sterling.transactions AS item_receipts
INNER JOIN edw.netsuite_sterling.transaction_line AS item_receipts_lines
  ON item_receipts.transaction_id = item_receipts_lines.transaction_id
INNER JOIN edw.netsuite_sterling.transaction_inventory_numbers AS item_receipt_inventory_numbers
  ON item_receipt_inventory_numbers.transaction_id = item_receipts.transaction_id
    AND item_receipt_inventory_numbers.transaction_line_id = item_receipts_lines.transaction_line_id
INNER JOIN edw.netsuite_sterling.item_product_group AS items
...
WHERE
  ...
  item_receipt_inventory_numbers.serial IS NOT NULL
), exchange_synced AS (
  ...
  FROM edw.netsuite_sterling.transactions AS transactions
  LEFT JOIN edw.netsuite_sterling.transactions AS item_receipts
    ON item_receipts.created_from_id = transactions.transaction_id
      AND item_receipts.transaction_type = 'Item Receipt'
  LEFT JOIN edw.netsuite_sterling.transaction_inventory_numbers AS item_receipt_inventory_numbers
    ON item_receipt_inventory_numbers.transaction_id = item_receipts.transaction_id
...
SELECT
  ...
  items.item_name AS sku_product_returned,
  items.displayname AS name_product_returned,
  items.item_name AS sku_product_received,
  ...
  transactions.is_deleted AS _fivetran_deleted

FROM edw.netsuite_sterling.transactions AS transactions
INNER JOIN edw.netsuite_sterling.transaction_line AS transaction_lines
...
INNER JOIN edw.netsuite_sterling.transaction_address AS address
...
WHERE
  ...
  AND (items.item_name LIKE 'HW-%' OR items.item_name LIKE 'CBL-%' OR items.item_name LIKE 'ACC-%')
