...
WITH order_synced AS (
  ...
  LEFT JOIN edw.netsuite_sterling.transaction_inventory_numbers AS transaction_inventory_numbers
...
SELECT
  ...
  sales_orders.is_deleted AS _fivetran_deleted

FROM edw.netsuite_sterling.transactions AS sales_orders
...
LEFT JOIN edw.netsuite_sterling.transactions AS item_fulfillments
  ON sales_orders.order_number = item_fulfillments.order_number
    AND item_fulfillments.transaction_type = 'Item Fulfillment'
LEFT JOIN edw.netsuite_sterling.transaction_address AS item_fulfillment_address
...
LEFT JOIN edw.netsuite_sterling.transaction_tracking_numbers AS item_fulfillment_tracking_numbers
  ON item_fulfillment_tracking_numbers.transaction_id = item_fulfillments.transaction_id
LEFT JOIN edw.netsuite_sterling.transaction_line AS item_fulfillment_lines
...
LEFT JOIN edw.netsuite_sterling.item_product_group AS item_fulfillment_line_items
...
LEFT JOIN edw.netsuite_sterling.transaction_inventory_numbers AS transaction_inventory_numbers
  ON transaction_inventory_numbers.transaction_id = item_fulfillments.transaction_id
    AND transaction_inventory_numbers.transaction_line_id = item_fulfillment_lines.transaction_line_id
WHERE
   (sales_orders.transaction_type = 'Sales Order' OR sales_orders.transaction_type = 'Transfer Order')
   AND (item_fulfillment_line_items.item_name LIKE 'LIC-%'
    OR item_fulfillment_line_items.item_name LIKE 'HW-%'
    OR item_fulfillment_line_items.item_name LIKE 'CBL-%'
    OR item_fulfillment_line_items.item_name LIKE 'ACC-%')
