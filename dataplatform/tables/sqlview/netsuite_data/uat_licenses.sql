...
SELECT
  ...
  END AS end_date
FROM
  biztech_edw_netsuite_sb1.transactions AS t
  INNER JOIN biztech_edw_netsuite_sb1.transaction_lines AS tl ON t.transaction_id = tl.transaction_id
  FULL OUTER JOIN biztech_edw_netsuite_sb1.customers AS customers on customers.CUSTOMER_ID = t.END_CUSTOMER_ID
  FULL OUTER JOIN biztech_edw_netsuite_sb1.customers AS entity on entity.CUSTOMER_ID = t.ENTITY_ID
  FULL OUTER JOIN biztech_edw_netsuite_sb1.items AS items ON items.ITEM_ID = tl.ITEM_ID
  FULL OUTER JOIN biztech_edw_netsuite_sb1.transactions AS created_from_transaction ON created_from_transaction.transaction_id = t.created_from_id
  LEFT JOIN biztech_edw_netsuite_sb1.transactions AS created_from_transaction_2 ON created_from_transaction_2.order_number = t.order_number and created_from_transaction_2.transaction_type = 'Sales Order' and created_from_transaction_2.order_type = 'Revenue Opportunity'
