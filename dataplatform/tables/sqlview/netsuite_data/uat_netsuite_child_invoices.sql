...
FROM
  biztech_edw_netsuite_sb1.transactions AS transactions
  INNER JOIN biztech_edw_netsuite_sb1.customers AS customers -- Many transactions to One customer
    ON transactions.entity_id = customers.customer_id
  LEFT OUTER JOIN biztech_edw_netsuite_sb1.currencies AS currencies -- Many transactions to One currency
    ON transactions.currency_id = currencies.currency_id
  LEFT OUTER JOIN netsuite_data.uat_transaction_addresses AS transaction_addresses -- One transaction to One transaction_address
    ON transaction_addresses.transaction_id = transactions.transaction_id
  LEFT OUTER JOIN netsuite_data.uat_consolidated_invoices_map AS consolidated_invoices_map
    ON consolidated_invoices_map.transaction_id = transactions.transaction_id
  LEFT OUTER JOIN netsuite_data.uat_netsuite_child_invoice_amounts AS invoice_amounts
