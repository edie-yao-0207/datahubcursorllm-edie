...
queued_bg_grouped_by_uuid_successful AS (
  SELECT
    uuid,
    date,
    count(*) AS attempts
...
total_uploads AS (
  SELECT
    date,
    count(attempts) AS total_uploads
  FROM
    queued_bg_grouped_by_uuid
  GROUP BY
    date
),
...
total_successful_uploads AS (
  SELECT
    date,
    count(attempts) AS total_successful_uploads
  FROM
    queued_bg_grouped_by_uuid_successful
  GROUP BY
    date
),
...
duration_p99_ms AS (
  SELECT
    date,
    percentile_approx(durationMs, 0.95) AS duration_p99_ms
  FROM
    queued_logs_table
  WHERE
    success == true
