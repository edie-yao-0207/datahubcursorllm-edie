...
driver_hos_settings_eu AS (
    SELECT
        ...
        setting,
        value
    FROM delta.`s3://samsara-eu-rds-delta-lake/table-parquet/prod-compliancedb/compliancedb/driver_hos_settings_v0`
    ...
    SELECT
        ...
        setting,
        value
    FROM delta.`s3://samsara-eu-rds-delta-lake/table-parquet/prod-compliance-shard-1db/compliancedb/driver_hos_settings_v0`
),
oil_well_exemptions AS (
    SELECT DISTINCT
        hos.org_id
    FROM compliancedb_shards.driver_hos_settings hos
    WHERE
        hos.setting = 'oilwell_exemption_toggle_enabled'
        AND hos.value = 'true'
        AND COALESCE(DATE(hos.deleted_at), DATE_ADD('{PARTITION_START}', 1)) >= '{PARTITION_START}' --valid record

    UNION ALL

    SELECT DISTINCT
        hos.org_id
    FROM driver_hos_settings_eu hos
    WHERE
        hos.setting = 'oilwell_exemption_toggle_enabled'
        AND hos.value = 'true'
        AND COALESCE(DATE(hos.deleted_at), DATE_ADD('{PARTITION_START}', 1)) >= '{PARTITION_START}' --valid record
...
relevant_orgs AS (
    -- All Canada customers with at least one driver with oil well exemption active are enabled
    SELECT
        o.org_id,
        org_name,
        CASE
...
events AS (
    -- Usage is defined as all Oil Well claim events from drivers
    SELECT
        ce.org_id,
        DATE(ce.server_created_at) AS event_date,
        ce.driver_id AS user_id
    FROM compliancedb_shards.driver_hos_exemption_claim_events ce
    WHERE
       exemption_type_id = 8 -- Oil Well Exemption
       AND DATE(ce.server_created_at) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
       AND COALESCE(DATE(ce.server_deleted_at), DATE_ADD('{PARTITION_START}', 1)) >= '{PARTITION_START}' --valid record
       AND COALESCE(DATE(ce.client_deleted_at), DATE_ADD('{PARTITION_START}', 1)) >= '{PARTITION_START}' --valid record
       AND ce.driver_id != 0
       AND ce.driver_id IS NOT NULL
    ...
    SELECT
        ...
        DATE(ce.server_created_at) AS event_date,
        ce.driver_id AS user_id
    FROM delta.`s3://samsara-eu-rds-delta-lake/table-parquet/prod-compliancedb/compliancedb/driver_hos_exemption_claim_events_v0` ce
    WHERE
       exemption_type_id = 8 -- Oil Well Exemption
       AND DATE(ce.server_created_at) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
       AND COALESCE(DATE(ce.server_deleted_at), DATE_ADD('{PARTITION_START}', 1)) >= '{PARTITION_START}' --valid record
    ...
    SELECT
        ...
        DATE(ce.server_created_at) AS event_date,
        ce.driver_id AS user_id
    FROM delta.`s3://samsara-eu-rds-delta-lake/table-parquet/prod-compliance-shard-1db/compliancedb/driver_hos_exemption_claim_events_v0` ce
    WHERE
       exemption_type_id = 8 -- Oil Well Exemption
       AND DATE(ce.server_created_at) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
       AND COALESCE(DATE(ce.server_deleted_at), DATE_ADD('{PARTITION_START}', 1)) >= '{PARTITION_START}' --valid record
...
enabled_drivers AS (
    ...
    JOIN compliancedb_shards.driver_hos_settings hos
        ON dd.org_id = hos.org_id
        AND dd.driver_id = hos.driver_id
    WHERE
        dd.date = '{PARTITION_START}'
        AND (DATE(dd.deleted_at) IN ('0101-01-01', '0101-01-02') OR DATE(dd.deleted_at) >= DATE_SUB('{PARTITION_START}', 27))
        AND hos.setting = 'oilwell_exemption_toggle_enabled'
        AND hos.value = 'true'
        AND COALESCE(DATE(hos.deleted_at), DATE_ADD('{PARTITION_START}', 1)) >= '{PARTITION_START}' --valid record
    GROUP BY dd.org_id
    ...
    JOIN relevant_orgs rl
        ON dd.org_id = rl.org_id
    JOIN driver_hos_settings_eu hos
        ON dd.org_id = hos.org_id
        AND dd.driver_id = hos.driver_id
    WHERE
        dd.date = '{PARTITION_START}'
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (Exemptions claimed)
-- usage_monthly: Usage in the last 28 days (Exemptions claimed)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (Exemptions claimed)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (Exemptions claimed)
-- daily_active_user: Number of unique users using the feature on the day in question (drivers claiming exemptions)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (drivers claiming exemptions)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (drivers claiming exemptions)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Oil Well Exemption feature""",
        row_meaning="""Each row represents an org's usage of Oil Well Exemption over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_oil_well_exemption_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_oil_well_exemption_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_oil_well_exemption_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_oil_well_exemption_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_oil_well_exemption_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_oil_well_exemption_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_oil_well_exemption_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_oil_well_exemption_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_oil_well_exemption_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:compliancedb_shards.driver_hos_exemption_claim_events",
        "us-west-2|eu-west-1:finopsdb.customer_info",
        "us-west-2|eu-west-1:compliancedb_shards.driver_hos_settings",
    ],
)
def agg_executive_scorecard_oil_well_exemption_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_oil_well_exemption_global")
