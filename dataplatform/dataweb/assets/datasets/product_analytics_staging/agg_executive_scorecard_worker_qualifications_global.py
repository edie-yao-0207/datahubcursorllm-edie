...
enabled_drivers AS (
    ...
    -- This does not apply to LIC-QUALS as that is handled via license assignment
    SELECT
      dd.org_id,
...
{ENABLED_USERS_SQL}
enabled_users_aggregated AS (
    SELECT
        org_id,
        COALESCE(COUNT(DISTINCT CASE WHEN expire_at IS NULL OR DATE(expire_at) >= '{PARTITION_START}' THEN user_id END), 0) AS daily_enabled_users,
        COALESCE(COUNT(DISTINCT CASE WHEN expire_at IS NULL OR DATE(expire_at) >= DATE_SUB('{PARTITION_START}', 6) THEN user_id END), 0) AS weekly_enabled_users,
        COALESCE(COUNT(DISTINCT CASE WHEN expire_at IS NULL OR DATE(expire_at) >= DATE_SUB('{PARTITION_START}', 27) THEN user_id END), 0) AS monthly_enabled_users
    FROM enabled_users
    GROUP BY org_id
),
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Usage in the last 7 days (qualification uploads)
-- usage_monthly: Usage in the last 28 days (qualification uploads)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (qualification uploads)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (qualification uploads)
-- daily_active_user: Number of unique users using the feature on the day in question (users who submitted a qualification)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (users who submitted a qualification)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (users who submitted a qualification)
...
SELECT
    ...
    COALESCE(eu.daily_enabled_users, 0) + COALESCE(ed.daily_enabled_drivers, 0) AS daily_enabled_users,
    COALESCE(e.weekly_active_users, 0) AS weekly_active_users,
    COALESCE(eu.weekly_enabled_users, 0) + COALESCE(ed.weekly_enabled_drivers, 0) AS weekly_enabled_users,
    COALESCE(e.monthly_active_users, 0) AS monthly_active_users,
    COALESCE(eu.monthly_enabled_users, 0) + COALESCE(ed.monthly_enabled_drivers, 0) AS monthly_enabled_users,
...
LEFT OUTER JOIN enabled_users_aggregated eu
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Worker Qualifications feature""",
        row_meaning="""Each row represents an org's usage of Worker Qualifications over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_worker_qualifications_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_worker_qualifications_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_worker_qualifications_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_qualifications_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_qualifications_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_qualifications_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_qualifications_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_worker_qualifications_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_qualifications_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:formsdb_shards.form_templates",
        "us-west-2|eu-west-1:formsdb_shards.form_template_revisions",
        "us-west-2|eu-west-1:formsdb_shards.form_template_revision_contents",
...
def agg_executive_scorecard_worker_qualifications_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_worker_qualifications_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("worker_qualifications")
