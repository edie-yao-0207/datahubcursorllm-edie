...
relevant_orgs AS (
    -- Open Beta, filtering will be done by FF/licenses/self-serve later
    SELECT
        o.org_id,
        org_name,
        CASE
...
relevant_licenses AS (
    -- Relevant licenses:
    -- Legacy Safety Enterprise
    -- FY25 Safety Enterprise
    -- FY26 Safety Enterprise
    -- FY25 Telematics Enterprise
    -- Legacy Telematics Enterprise
    -- FY26 Telematics Enterprise
    ...
    WHERE
        ...
        AND (
            fo.product_sku LIKE 'LIC-CM-D-ENT%' -- Safety Enterprise FY25/FY26
            OR fo.product_sku LIKE 'LIC-CM-S-ENT%' -- Safety Enterprise FY25/FY26
            OR fo.product_sku LIKE 'LIC-CM1-ENT%' -- Legacy Enterprise
            OR fo.product_sku LIKE 'LIC-CM2-ENT%' -- Legacy Enterprise
            OR fo.product_sku LIKE 'LIC-VG-ENT%' -- Telematics Enterprise Legacy/FY25/FY26
            OR (fo.product_sku LIKE 'LIC-CM%' AND xref.is_core_license AND xref.is_legacy_license)
            OR (fo.product_sku LIKE 'LIC-VG%' AND xref.is_core_license AND xref.is_legacy_license)
        )
...
final_relevant_orgs AS (
    ...
    -- 1. Orgs who have a relevant license
    -- 2. Orgs who have been added to the Beta and have a relevant license
    SELECT
        ro.org_id,
        ro.org_name,
    ...
    WHERE
        ro.release_type in ('Early Adopter', 'Phase 1')
...
{ENABLED_USERS_SQL}
events AS (
    -- Usage is defined as either one route page visit to fleet_supervisor_insights or hitting one of the relevant Mixpanel metrics
    -- Mixpanel metrics:
    ...
    -- For all of these, only filter to the fleet_supervisor_insights route
    -- Join to enabled users to solve case of changing emails
    SELECT
        DATE(mix.mp_date) AS event_date,
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_view_safety_score_navigation_click mix
    ...
    WHERE
        mix.mp_date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
        ...
        AND routename IN ('fleet_supervisor_insights')
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_view_ecodriving_score_navigation_click mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_insights_drivers_to_recognize_click_driver_name mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_insights_drivers_requiring_attention_click_send_to_coaching mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_insights_drivers_requiring_attention_click_send_message mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_insights_drivers_requiring_attention_click_driver_name mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_insights_coaching_timeliness_click_send_notification mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_insights_coaching_effectiveness_click_view_drivers mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.supervisor_insights_training_timeliness_click_send_message mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.positive_recognition_kudos_send_kudos mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
    ...
    SELECT
        ...
        CAST(mix.orgid AS BIGINT) AS org_id,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.cloud_route mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND CAST(mix.orgid AS BIGINT) = eu.org_id
    WHERE
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (page visits/Mixpanel actions)
-- usage_monthly: Usage in the last 28 days (page visits/Mixpanel actions)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (page visits/Mixpanel actions)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (page visits/Mixpanel actions)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Performance Overview feature""",
        row_meaning="""Each row represents an org's usage of Performance Overview over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_performance_overview_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_performance_overview_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_performance_overview_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_performance_overview_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_performance_overview_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_performance_overview_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_performance_overview_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_performance_overview_global_enablement",
            lookback_days=1,
            tolerance=0.1,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_performance_overview_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2:mixpanel_samsara.supervisor_view_safety_score_navigation_click",
        "us-west-2:mixpanel_samsara.supervisor_insights_drivers_to_recognize_click_driver_name",
        "us-west-2:mixpanel_samsara.supervisor_insights_drivers_requiring_attention_click_send_to_coaching",
        "us-west-2:mixpanel_samsara.supervisor_insights_drivers_requiring_attention_click_send_message",
        "us-west-2:mixpanel_samsara.supervisor_insights_drivers_requiring_attention_click_driver_name",
        "us-west-2:mixpanel_samsara.supervisor_insights_coaching_timeliness_click_send_notification",
        "us-west-2:mixpanel_samsara.supervisor_insights_coaching_effectiveness_click_view_drivers",
        "us-west-2:mixpanel_samsara.supervisor_insights_training_timeliness_click_send_message",
        "us-west-2:mixpanel_samsara.positive_recognition_kudos_send_kudos",
        "us-west-2:mixpanel_samsara.cloud_route",
    ],
)
def agg_executive_scorecard_performance_overview_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_performance_overview_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("performance_overview")
    enabled_users_sql = get_enabled_users_sql(json_config, PARTITION_START)
    query = QUERY.format(
        PARTITION_START=PARTITION_START,
        flag_name='show-coaching-intelligence-supervisor-focus-tab',
