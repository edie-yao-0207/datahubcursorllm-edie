...
relevant_licenses AS (
    -- All Safety customers are relevant for Manager-Led Coaching
...
events AS (
    -- Usage is defined as all completed manager-led coaching sessions
    SELECT
        CAST(org_id AS BIGINT) AS org_id,
        DATE(completed_date) AS event_date,
        driver_id AS user_id
    FROM
        coachingdb_shards.coaching_sessions
    WHERE
        DATE(completed_date) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
        AND session_type IN (1, 2) -- Manager-Led Coaching
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Manager-Led Coaching feature""",
        row_meaning="""Each row represents an org's usage of Manager-Led Coaching over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_manager_coaching_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_manager_coaching_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_manager_coaching_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_manager_coaching_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_manager_coaching_global_usage",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_manager_coaching_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_manager_coaching_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_manager_coaching_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_manager_coaching_global_usage_change",
            sql_query="""
                WITH today_usage AS (
...
def agg_executive_scorecard_manager_coaching_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_manager_coaching_global")
