...
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Usage in the last 7 days (modal opens)
-- usage_monthly: Usage in the last 28 days (modal opens)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (modal opens)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (modal opens)
-- daily_active_user: Number of unique users using the feature on the day in question (in this context, users opening the modal)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (in this context, users opening the modal)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (in this context, users opening the modal)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the AI Fault Codes feature""",
        row_meaning="""Each row represents an org's usage of AI Fault Codes over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_ai_fault_codes_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_ai_fault_codes_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_ai_fault_codes_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ai_fault_codes_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ai_fault_codes_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ai_fault_codes_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ai_fault_codes_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_ai_fault_codes_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ai_fault_codes_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2:mixpanel_samsara.fleet_vehicle_show_faults_open_fault_description_modal",
        "us-west-2:mixpanel_samsara.fleet_maintenance_status_open_fault_description_modal",
...
def agg_executive_scorecard_ai_fault_codes_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_ai_fault_codes_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("ai_fault_codes")
