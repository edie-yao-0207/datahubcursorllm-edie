...
QUERY = """
-- Note that we specifically hardcode some orgs to include that have just a single license, which should be filtered out here
-- We will need to track that over time and adjust as necessary
WITH {RELEVANT_ORGS}
enabled_users AS (
    SELECT
...
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Usage in the last 7 days (devices that have moved or been assigned to an incident)
-- usage_monthly: Usage in the last 28 days (devices that have moved or been assigned to an incident)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (devices that have moved or been assigned to an incident)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (devices that have moved or been assigned to an incident)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Worker Safety (Wearable) feature""",
        row_meaning="""Each row represents an org's usage of Worker Safety (Wearable) over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_worker_safety_wearable_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_worker_safety_wearable_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_worker_safety_wearable_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_global_usage",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_global_enabled_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_worker_safety_wearable_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:dynamodb.worker_safety_incidents",
        "us-west-2|eu-west-1:dynamodb.worker_safety_wearable_assignments",
        "us-west-2|eu-west-1:productsdb.gateway_device_history",
        "us-west-2|eu-west-1:productsdb.gateways",
        "us-west-2|eu-west-1:kinesisstats_history.location",
...
def agg_executive_scorecard_worker_safety_wearable_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_worker_safety_wearable_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("worker_safety_wearable")
