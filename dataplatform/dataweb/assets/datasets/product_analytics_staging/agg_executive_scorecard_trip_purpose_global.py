...
enabled_drivers_us AS (
    ...
    JOIN datamodel_core_bronze.raw_clouddb_organizations rco
        ON dd.org_id = rco.id
    WHERE
        dd.date = '{PARTITION_START}'
        AND rco.date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
        AND (DATE(dd.deleted_at) IN ('0101-01-01', '0101-01-02') OR DATE(dd.deleted_at) >= DATE_SUB('{PARTITION_START}', 27))
        AND rco.settings_proto.organization_driver_app_settings.customizable_driver_features_settings.trip_log_enabled -- trip log is enabled
    GROUP BY dd.org_id
),
enabled_drivers_eu AS (
    ...
    JOIN relevant_licenses rl
        ON dd.org_id = rl.org_id
    JOIN delta.`s3://samsara-eu-datamodel-warehouse/datamodel_core_bronze.db/raw_clouddb_organizations` rco
        ON dd.org_id = rco.id
    WHERE
        dd.date = '{PARTITION_START}'
        AND rco.date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
...
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Whether the org has set the trip purpose in the last 7 days or not
-- usage_monthly: Whether the org has set the trip purpose in the last 28 days or not
-- usage_prior_month: Whether the org has set the trip purpose in the 28 days prior to the last 28 days or not
-- usage_weekly_prior_month: Whether the org has set the trip purpose in the 7 days prior to the last 28 days or not
...
-- org_active_day: Whether the org had a report active on the day in question
-- org_active_week: Whether the org had a report active on the day in question
-- org_active_week_prior_month: Whether the org had a report active at least 28 days ago
-- org_active_month: Whether the org had a report active on the day in question
-- org_active_prior_month: Whether the org had a report active at least 28 days ago
SELECT
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Trip Purpose feature""",
        row_meaning="""Each row represents an org's usage of Trip Purpose over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_trip_purpose_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_trip_purpose_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_trip_purpose_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_trip_purpose_global_usage",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_trip_purpose_global_enabled_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_trip_purpose_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_trip_purpose_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_trip_purpose_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_trip_purpose_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:tripsdb_shards.trip_purpose_assignments",
        "us-west-2|eu-west-1:finopsdb.customer_info",
        "us-west-2|eu-west-1:datamodel_core_bronze.raw_clouddb_organizations",
    ],
)
def agg_executive_scorecard_trip_purpose_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_trip_purpose_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("trip_purpose")
