...
enabled_users AS (
    SELECT
        CAST(OrgId AS BIGINT) AS org_id,
        DATE(FROM_UNIXTIME(StartMs / 1000)) AS start_date,
        CASE
            WHEN DATE(FROM_UNIXTIME(EndMs / 1000)) = '1970-01-01' THEN NULL
            ELSE DATE(FROM_UNIXTIME(EndMs / 1000))
        END AS end_date,
        PolymorphicAssigneeId AS user_id
    FROM dynamodb.worker_safety_wearable_assignments
    WHERE
        DATE(FROM_UNIXTIME(EndMs / 1000)) = '1970-01-01'
        OR DATE(FROM_UNIXTIME(EndMs / 1000)) >= DATE_SUB('{PARTITION_START}', 27)
    ...
    SELECT
        ...
        END AS end_date,
        PolymorphicAssigneeId AS user_id
    FROM delta.`s3://samsara-eu-dynamodb-delta-lake/table/worker-safety-wearable-assignments`
    WHERE
        DATE(FROM_UNIXTIME(EndMs / 1000)) = '1970-01-01'
        OR DATE(FROM_UNIXTIME(EndMs / 1000)) >= DATE_SUB('{PARTITION_START}', 27)
),
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Usage in the last 7 days (wearable button presses/fall detection)
-- usage_monthly: Usage in the last 28 days (wearable button presses/fall detection)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (wearable button presses/fall detection)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (wearable button presses/fall detection)
-- daily_active_user: Number of unique users using the feature on the day in question (users wearing the devices)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (users wearing the devices)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (users wearing the devices)
...
SELECT
    ...
    COUNT(DISTINCT CASE
        WHEN eu.start_date <= '{PARTITION_START}'
        AND (eu.end_date IS NULL OR eu.end_date >= '{PARTITION_START}')
        THEN eu.user_id
    END) AS daily_enabled_users,
    COALESCE(e.weekly_active_users, 0) AS weekly_active_users,
    COUNT(DISTINCT CASE
        WHEN eu.start_date <= '{PARTITION_START}'
        AND (eu.end_date IS NULL OR eu.end_date >= DATE_SUB('{PARTITION_START}', 6))
        THEN eu.user_id
    END) AS weekly_enabled_users,
    COALESCE(e.monthly_active_users, 0) AS monthly_active_users,
    COUNT(DISTINCT CASE
        WHEN eu.start_date <= '{PARTITION_START}'
        AND (eu.end_date IS NULL OR eu.end_date >= DATE_SUB('{PARTITION_START}', 27))
        THEN eu.user_id
    END) AS monthly_enabled_users,
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Worker Safety (Wearable) Incidents feature""",
        row_meaning="""Each row represents an org's usage of Worker Safety (Wearable) Incidents over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_worker_safety_wearable_incidents_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_worker_safety_wearable_incidents_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_worker_safety_wearable_incident_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_incidents_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_incidents_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_incidents_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_incidents_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_worker_safety_wearable_incidents_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_wearable_incidents_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:dynamodb.worker_safety_sos_signals",
        ...
        "us-west-2|eu-west-1:dynamodb.worker_safety_wearable_assignments",
    ],
)
def agg_executive_scorecard_worker_safety_wearable_incidents_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_worker_safety_wearable_incidents_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("worker_safety_wearable_incidents")
    ...
    query = QUERY.format(
        ...
        INTERNAL_USERS_CTE=INTERNAL_USERS_CTE.format(PARTITION_START=PARTITION_START),
        USAGE_SQL=usage_sql,
    )
