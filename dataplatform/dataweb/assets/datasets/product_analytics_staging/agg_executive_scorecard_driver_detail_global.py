...
-- daily_active_user: Number of unique users using the feature on the day in question (users who visited the page)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question (users who can visit the page)
-- weekly_active_users: Number of unique users using the feature in the last 7 days (users who visited the page)
...
-- monthly_active_users: Number of unique users using the feature in the last 28 days (users who visited the page)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Driver Detail feature""",
        row_meaning="""Each row represents an org's usage of Driver Detail over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_driver_detail_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_driver_detail_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_driver_detail_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_detail_global_usage",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_detail_global_enabled_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_detail_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_detail_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_driver_detail_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_detail_global_usage_change",
            sql_query="""
                WITH today_usage AS (
...
def agg_executive_scorecard_driver_detail_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_driver_detail_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("driver_detail")
