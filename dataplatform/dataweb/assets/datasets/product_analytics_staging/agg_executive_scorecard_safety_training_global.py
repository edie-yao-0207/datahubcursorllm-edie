...
relevant_orgs AS (
    ...
    -- Only eligible in the following locales:
    ...
    WHERE
        ...
        AND o.locale IN ('us', 'ca', 'mx', 'gb', 'ie')
...
relevant_licenses AS (
    -- Safety Enterprise licenses are applicable licenses
    ...
    WHERE
        ...
        AND (
            fo.product_sku = 'LIC-CM-D-ENTERPRISE'
            OR fo.product_sku = 'LIC-CM-S-ENTERPRISE'
            OR fo.product_sku = 'LIC-CM1-ENT'
            OR fo.product_sku = 'LIC-CM2-ENT'
            OR fo.product_sku LIKE 'LIC-CM-D-ENT-%'
            OR fo.product_sku LIKE 'LIC-CM-S-ENT-%'
            OR fo.product_sku IN ('LIC-TRAINING', 'LIC-TRAINING-PROMO', 'LIC-FL-APPS')
            OR (fo.product_sku LIKE 'LIC-CM%' AND xref.is_legacy_license AND xref.is_core_license)
        )
...
events_us AS (
    -- Usage is defined as orgs with at least one safety training assigned from a non-internal user
...
events_eu AS (
    ...
    JOIN courses_eu c
        ON f.form_template_uuid = c.global_course_uuid
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Safety Training feature""",
        row_meaning="""Each row represents an org's usage of Safety Training over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_safety_training_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_safety_training_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_safety_training_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_safety_training_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_safety_training_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_safety_training_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_safety_training_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_safety_training_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_safety_training_global_usage_change",
            sql_query="""
                WITH today_usage AS (
...
def agg_executive_scorecard_safety_training_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_safety_training_global")
