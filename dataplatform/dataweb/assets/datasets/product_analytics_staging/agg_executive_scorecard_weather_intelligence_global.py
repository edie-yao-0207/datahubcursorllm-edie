...
relevant_orgs AS (
    ...
    WHERE
        ...
        AND o.locale IN ('us', 'gb', 'ie', 'ca', 'mx')
...
relevant_licenses AS (
    -- Safety Enterprise is the relevant license
    ...
    WHERE
        ...
        AND (
            ...
            OR fo.product_sku LIKE 'LIC-CM1-ENT%'
            OR fo.product_sku LIKE 'LIC-CM2-ENT%'
        )
...
{ENABLED_USERS_SQL}
events AS (
    -- Usage is defined as any hits to the following events: smart_maps_weather_forecast_overlay_details_clicked
    -- We filter out any internal usage and we also filter to when it was toggled on
    -- Join to enabled users to solve case of changing emails
    SELECT
        CAST(orgid AS BIGINT) AS org_id,
        TO_DATE(mp_date) AS event_date,
        COALESCE(eu.user_id, cr.mp_user_id, cr.mp_device_id) AS user_id
    FROM mixpanel_samsara.smart_maps_weather_forecast_overlay_details_clicked cr
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(cr.mp_user_id, cr.mp_device_id) = eu.email
        AND CAST(cr.orgid AS BIGINT) = eu.org_id
    WHERE
        cr.orgid IS NOT NULL
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (toggle events)
-- usage_monthly: Usage in the last 28 days (toggle events)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (toggle events)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (toggle events)
-- daily_active_user: Number of unique users using the feature on the day in question (clicking the toggle)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (clicking the toggle)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (clicking the toggle)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Weather Intelligence feature""",
        row_meaning="""Each row represents an org's usage of Weather Intelligence over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_weather_intelligence_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_weather_intelligence_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_weather_intelligence_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_weather_intelligence_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_weather_intelligence_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_weather_intelligence_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_weather_intelligence_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_weather_intelligence_global_enablement", lookback_days=1, tolerance=0.1),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_weather_intelligence_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2:mixpanel_samsara.smart_maps_weather_forecast_overlay_details_clicked",
    ],
)
def agg_executive_scorecard_weather_intelligence_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_weather_intelligence_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("weather_intelligence")
