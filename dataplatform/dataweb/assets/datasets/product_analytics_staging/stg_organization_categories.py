...
QUERY = """--sql
    ...
    FROM read_files(
        's3://samsara-databricks-workspace/dataengineering/csvs/FMCSA_CENSUS1_2024Oct.txt',
        format => 'csv',
        header => true,
        mode => 'PERMISSIVE')),
    company_census AS (
    ...
    FROM read_files(
        's3://samsara-databricks-workspace/dataengineering/csvs/Company_Census_File_20241203.csv',
        format => 'csv',
        header => true,
        mode => 'PERMISSIVE')),
  drivers AS (
...
@table(
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_stg_organization_categories"),
        PrimaryKeyDQCheck(
            name="dq_pk_stg_organization_categories",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_stg_organization_categories",
            non_null_columns=[
                "date",
                "fleet_composition",
        ...
        TrendDQCheck(
            name="dq_trend_stg_organization_categories",
            tolerance=0.5,
            lookback_days=1,
        ...
        ValueRangeDQCheck(
            name="dq_value_range_stg_organization_categories",
            column_range_map={"raw_fuel_category_diesel": (0, 1)},
            block_before_write=False,
        ),
    ],
    upstreams=[
        "us-west-2:product_analytics_staging.stg_population_density",
        "us-west-2:product_analytics_staging.map_population_density",
        "us-west-2:datamodel_core.dim_organizations",
        "us-west-2:datamodel_telematics.fct_trips",
        "us-west-2:datamodel_core.dim_devices",
        "us-west-2:datamodel_core.lifetime_device_activity",
        "us-west-2:datamodel_core.dim_organizations_tenure",
    ],
)
def stg_organization_categories(context: AssetExecutionContext) -> str:
    context.log.info("Updating stg_organization_categories")
