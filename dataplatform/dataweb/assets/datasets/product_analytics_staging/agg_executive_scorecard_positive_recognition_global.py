...
relevant_orgs AS (
    -- Relevant orgs are those not excluded from FF
    -- FF: https://app.launchdarkly.com/projects/samsara/flags/safety-driver-streaks-v2/targeting?env=production&selected-env=production
    SELECT
        o.org_id,
        org_name,
        CASE
...
relevant_licenses AS (
    -- Relevant SKUs signed off on by EM/PM
    -- 1. Legacy Safety
    -- 2. FY25 Safety Enterprise
    -- 3. FY26 Safety Enterprise
    -- 4. FY25 Telematics Enterprise
    -- 5. Legacy Telematics
    -- 6. FY26 Telematics Enterprise
    -- 7. FY26 Compliance Add-on
    ...
    WHERE
        ...
        AND (
            (fo.product_sku LIKE 'LIC-CM%' AND xref.is_legacy_license AND xref.is_core_license) -- Legacy Safety
            OR fo.product_sku IN ('LIC-CM-D-ENT-PLTFM-ESS', 'LIC-CM-D-ENT-PLTFM-PREM', 'LIC-CM-S-ENT-PLTFM-ESS', 'LIC-CM-S-ENT-PLTFM-PREM')
            OR fo.product_sku IN ('LIC-CM-D-ENTERPRISE', 'LIC-CM-S-ENTERPRISE')
            OR fo.product_sku IN ('LIC-VG-ENT-PLTFM-ESS', 'LIC-VG-ENT-PLTFM-PREM')
            OR (fo.product_sku LIKE 'LIC-VG%' AND xref.is_legacy_license AND xref.is_core_license)
            OR fo.product_sku IN ('LIC-VG-ENTERPRISE')
            OR fo.product_sku IN ('LIC-VG-COMPLIANCE')
        )
...
traces_us AS (
    -- Usage is defined as having any kudos
    SELECT
    ...
    LEFT OUTER JOIN users_us u
        ON prs.shared_by_id = u.user_id
    WHERE
        org_id IS NOT NULL
        AND u.user_id IS NULL -- Filter out internal users
        AND TO_DATE(created_at) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
),
traces_eu AS (
    SELECT
        CAST(org_id AS BIGINT) AS org_id,
...
traces AS (
    SELECT * FROM traces_us

    UNION ALL

    SELECT * FROM traces_eu
),
{ENABLED_USERS_SQL}
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (kudos sent)
-- usage_monthly: Usage in the last 28 days (kudos sent)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (kudos sent)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (kudos sent)
-- daily_active_user: Number of unique users using the feature on the day in question (in this context, sending a kudos)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (in this context, sending a kudos)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (in this context, sending a kudos)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Positive Recognition feature""",
        row_meaning="""Each row represents an org's usage of Positive Recognition over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_positive_recognition_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_positive_recognition_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_positive_recognition_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_positive_recognition_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_positive_recognition_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_positive_recognition_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_positive_recognition_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_positive_recognition_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_positive_recognition_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:coachingdb_shards.positive_recognition_shares",
...
def agg_executive_scorecard_positive_recognition_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_positive_recognition_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("positive_recognition")
    enabled_users_sql = get_enabled_users_sql(json_config, PARTITION_START)
    query = QUERY.format(
        PARTITION_START=PARTITION_START,
        flag_name='safety-driver-streaks-v2',
