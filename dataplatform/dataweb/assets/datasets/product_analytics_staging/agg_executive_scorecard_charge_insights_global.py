...
QUERY = """
WITH {RELEVANT_ORGS}
electric_vehicles AS (
    -- An org needs at least ten connected EVs to access Charge Insights
    SELECT DISTINCT
        dd.org_id,
        COUNT(DISTINCT dd.device_id) AS total_evs
    FROM datamodel_core.dim_devices dd
    JOIN product_analytics_staging.dim_device_vehicle_properties dft
        ON dd.device_id = dft.device_id
        AND dd.org_id = dft.org_id
    WHERE
        dd.device_type = 'VG - Vehicle Gateway'
        AND dd.date = '{PARTITION_START}'
        AND dft.fuel_group = 4 -- EV
        AND dft.date = (SELECT MAX(date) FROM product_analytics_staging.dim_device_vehicle_properties)
    GROUP BY 1
    HAVING COUNT(DISTINCT dd.device_id) >= 10
    ...
    JOIN data_tools_delta_share.product_analytics_staging.dim_device_vehicle_properties dft
        ON dd.device_id = dft.device_id
        AND dd.org_id = dft.org_id
    WHERE
        dd.device_type = 'VG - Vehicle Gateway'
        AND dd.date = '{PARTITION_START}'
        AND dft.fuel_group = 4 -- EV
        AND dft.date = (SELECT MAX(date) FROM data_tools_delta_share.product_analytics_staging.dim_device_vehicle_properties)
    GROUP BY 1
    HAVING COUNT(DISTINCT dd.device_id) >= 10
),
{ENABLED_USERS_SQL}
...
JOIN electric_vehicles ev
    ON o.org_id = ev.org_id
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Charge Insights feature""",
        row_meaning="""Each row represents an org's usage of Charge Insights over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_charge_insights_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_charge_insights_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_charge_insights_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_charge_insights_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_charge_insights_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_charge_insights_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_charge_insights_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_charge_insights_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_charge_insights_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:product_analytics_staging.dim_device_vehicle_properties",
...
def agg_executive_scorecard_charge_insights_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_charge_insights_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("charge_insights")
