...
all_roadside_settings_org_us AS (
    SELECT
        EntityId,
        Namespace,
        FROM_JSON(Settings, 'struct<roadsideParkingSettings:struct<enabled:boolean>>').roadsideParkingSettings.enabled as enabled,
...
latest_device_settings_us AS (
    ...
    SELECT
        REGEXP_EXTRACT(EntityId, 'DEVICE#(.+)', 1) AS device_id,
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (enabled for roadside parking)
-- usage_monthly: Usage in the last 28 days (enabled for roadside parking)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (enabled for roadside parking)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (enabled for roadside parking)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Roadside Parking feature""",
        row_meaning="""Each row represents an org's usage of Roadside Parking over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_roadside_parking_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_roadside_parking_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_roadside_parking_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_roadside_parking_global_enablement", lookback_days=1, tolerance=0.02),
    ],
...
def agg_executive_scorecard_roadside_parking_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_roadside_parking_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("roadside_parking")
