...
relevant_licenses AS (
    -- Any VG license on the core bundles has access
...
{ENABLED_USERS_SQL}
traces AS (
    -- Usage is defined as any visit to the Fuel Purchase Details page
    SELECT
    ...
    WHERE
        ...
        AND mix.routename = 'fuel_purchase_details'
),
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (fuel page visits)
-- usage_monthly: Usage in the last 28 days (fuel page visits)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (fuel page visits)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (fuel page visits)
-- daily_active_user: Number of unique users using the feature on the day in question (users visiting the page)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question (users visiting the page)
-- weekly_active_users: Number of unique users using the feature in the last 7 days (users visiting the page)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days (users visiting the page)
-- monthly_active_users: Number of unique users using the feature in the last 28 days (users visiting the page)
-- monthly_enabled_users: Number of unique users who had access to the feature in the last 28 days (users visiting the page)
...
JOIN relevant_events rl
    ON o.org_id = rl.org_id
JOIN relevant_licenses rl2
    ON o.org_id = rl2.org_id
LEFT OUTER JOIN traces_aggregated e
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Fuel Card Transactions feature""",
        row_meaning="""Each row represents an org's usage of Fuel Card Transactions over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_fuel_card_transactions_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_fuel_card_transactions_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_fuel_card_transactions_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_fuel_card_transactions_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_fuel_card_transactions_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_fuel_card_transactions_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_fuel_card_transactions_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_fuel_card_transactions_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_fuel_card_transactions_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:product_analytics.dim_app_installs",
        "us-west-2|eu-west-1:fuelcardsdb_shards.fleetcor_integration_configs",
        "us-west-2|eu-west-1:fuelcardintegrationsdb.wex_integration_configs",
...
def agg_executive_scorecard_fuel_card_transactions_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_fuel_card_transactions_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("fuel_card_transactions")
