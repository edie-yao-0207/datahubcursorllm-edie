...
QUERY = """
WITH {RELEVANT_ORGS}
tachograph_devices AS (
    SELECT DISTINCT dd.org_id
    FROM datamodel_core.dim_devices dd
    JOIN datamodel_core.dim_product_variants dpv
        ON dd.product_id = dpv.product_id
        AND dd.variant_id = dpv.variant_id
    WHERE
        dd.last_cable_id IN (7)
        AND dpv.hw_variant_name IN ('HW-VG34-EU')
    ...
    JOIN datamodel_core.dim_product_variants dpv
        ...
        AND dd.variant_id = dpv.variant_id
    WHERE
        dd.last_cable_id IN (14, 25, 32, 33, 34)
        AND dpv.hw_variant_name IN ('HW-VG54-EU', 'HW-VG55-EU')
        AND date = '{PARTITION_START}'
...
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Usage in the last 7 days (infringements coached/closed)
-- usage_monthly: Usage in the last 28 days (infringements coached/closed)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (infringements coached/closed)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (infringements coached/closed)
-- daily_active_user: Number of unique users using the feature on the day in question (drivers who were coached on infringements)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (drivers who were coached on infringements)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (drivers who were coached on infringements)
...
JOIN tachograph_devices td
    ON o.org_id = td.org_id
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Smart Compliance feature""",
        row_meaning="""Each row represents an org's usage of Smart Compliance over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_smart_compliance_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_smart_compliance_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_smart_compliance_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_smart_compliance_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_smart_compliance_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_smart_compliance_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_smart_compliance_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_smart_compliance_global_enablement", lookback_days=1, tolerance=0.1),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_smart_compliance_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2:datamodel_core.dim_product_variants",
        ...
        "us-west-2|eu-west-1:coachingdb_shards.coachable_item",
    ],
)
def agg_executive_scorecard_smart_compliance_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_smart_compliance_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("smart_compliance")
