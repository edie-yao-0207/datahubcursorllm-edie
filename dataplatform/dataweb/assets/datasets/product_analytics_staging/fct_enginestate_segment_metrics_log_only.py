...
@table(
    ...
    description=build_table_description(
        table_desc="""
        Log-only engine state segment metrics table that analyzes signals during segments of time
        in which log-only engine state is reported in order to determine the accuracy of our
        engine state algorithm.
        """.strip(),
        row_meaning="""
        Each row represents a time segment with:
        - Reported log-only engine state
    ...
    dq_checks=build_general_dq_checks(
        asset_name="fct_enginestate_segment_metrics_log_only",
        primary_keys=ENGINE_STATE_SEGMENT_METRICS_PRIMARY_KEYS,
        non_null_keys=ENGINE_STATE_SEGMENT_METRICS_NON_NULL_COLUMNS,
    ),
)
def fct_enginestate_segment_metrics_log_only(
    context: AssetExecutionContext
) -> DataFrame:
    """
    Log-only engine state segment metrics table.

    Uses log-only engine state (osdenginestatelogonly) and power state (osdpowerstate)
    ...
    return get_engine_state_segment_stats(
        context=context,
        spark=spark,
        engine_state_table="osdenginestatelogonly",
