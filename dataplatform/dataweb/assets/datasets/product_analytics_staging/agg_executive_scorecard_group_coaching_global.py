...
events AS (
    -- Usage is defined as all group coaching report runs
    SELECT
        CAST(gc.OrgId AS BIGINT) AS org_id,
        DATE(gc.CreatedAt) AS event_date,
        gc.CreatedBy AS user_id
    FROM
        dynamodb.group_coaching gc
    LEFT OUTER JOIN users_us u
        ON gc.CreatedBy = u.user_id
    WHERE
        DATE(gc.CreatedAt) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
        AND gc.ItemType = 'GROUP_COACHING_REPORT'
        AND u.user_id IS NULL -- Not internal
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (group coaching sessions)
-- usage_monthly: Usage in the last 28 days (group coaching sessions)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (group coaching sessions)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (group coaching sessions)
-- daily_active_user: Number of unique users using the feature on the day in question (in this context, running a group coaching session)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (in this context, running a coaching session)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (in this context, running a coaching session)
...
LEFT OUTER JOIN events_aggregated e
    ON e.org_id = ro.org_id
LEFT OUTER JOIN enabled_users eu
    ON eu.org_id = ro.org_id
GROUP BY
    ro.org_id,
    ro.org_name,
    ro.org_category,
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Group Coaching feature""",
        row_meaning="""Each row represents an org's usage of Group Coaching over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_group_coaching_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_group_coaching_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_group_coaching_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_group_coaching_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_group_coaching_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_group_coaching_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_group_coaching_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_group_coaching_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
           name="dq_sql_agg_executive_scorecard_group_coaching_global_usage_change",
           sql_query="""
               WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:dynamodb.group_coaching",
...
def agg_executive_scorecard_group_coaching_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_group_coaching_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("group_coaching")
