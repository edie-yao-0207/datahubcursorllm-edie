...
relevant_orgs AS (
    -- All Canada customers that have Canada HoS enabled are the applicable orgs
    SELECT
        o.org_id,
        org_name,
        CASE
...
events AS (
    -- Usage is defined as all HoS logs submitted by drivers
    SELECT
        hl.org_id,
        date AS event_date,
        hl.driver_id AS user_id
    FROM datamodel_telematics_silver.stg_hos_logs hl
    WHERE
    ...
    SELECT
        ...
        date AS event_date,
        hl.driver_id AS user_id
    FROM delta.`s3://samsara-eu-datamodel-warehouse/datamodel_telematics_silver.db/stg_hos_logs` hl
    WHERE
       date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
       AND hl.driver_id != 0
       AND hl.driver_id IS NOT NULL
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Canada HoS feature""",
        row_meaning="""Each row represents an org's usage of Canada HoS over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_canada_hos_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_canada_hos_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_canada_hos_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_canada_hos_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_canada_hos_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_canada_hos_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
                WHERE usage_weekly > 5000000
            """,
            expected_value=0,
            operator=Operator.eq,
            block_before_write=False,
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_canada_hos_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
                WHERE weekly_active_users > 25000
            """,
            expected_value=0,
            operator=Operator.eq,
            block_before_write=False,
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_canada_hos_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_canada_hos_global_usage_change",
           sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:datamodel_telematics_silver.stg_hos_logs",
        "us-west-2|eu-west-1:clouddb.groups",
        "us-west-2|eu-west-1:finopsdb.customer_info",
    ],
)
def agg_executive_scorecard_canada_hos_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_canada_hos_global")
