...
relevant_licenses_legacy AS (
    ...
    -- 1. Legacy Telematics
    -- 2. FY25: Telematics Enterprise, Telematics Essentials Plus
    -- FY25 licenses must also have the Compliance Add-On
...
relevant_licenses_core AS (
    ...
    WHERE
        ...
        AND (
            fo.product_sku IN (
                'LIC-VG-ENT-PLTFM-ESS', 'LIC-VG-ENT-PLTFM-PREM', 'LIC-VG-ESS-PLUS-PLTFM-ESS', 'LIC-VG-ESS-PLUS-PLTFM-PREM', 'LIC-VG-COMPLIANCE'
            ) -- Telematics FY25/Compliance
        )
...
final_relevant_orgs AS (
    ...
    -- 1. EA/Phase 1/FF orgs with proper license for legacy
    -- 2. EA/Phase 1/FF orgs with proper core license
    ...
    JOIN relevant_licenses_legacy rl
    ...
    SELECT
        ...
        ro.region
    FROM relevant_orgs ro
    JOIN relevant_licenses_core rl
        ON ro.org_id = rl.org_id
),
...
events AS (
    -- Usage is defined as all completed coaching sessions with HoS as a behavior
    SELECT
    ...
    WHERE
        ...
        AND coachable_item_type = 42 -- HoS
        AND csb.driver_id != 0
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the HoS Coaching feature""",
        row_meaning="""Each row represents an org's usage of HoS Coaching over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_hos_coaching_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_hos_coaching_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_hos_coaching_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_hos_coaching_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_hos_coaching_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_hos_coaching_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_hos_coaching_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_hos_coaching_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
           name="dq_sql_agg_executive_scorecard_hos_coaching_global_usage_change",
           sql_query="""
               WITH today_usage AS (
...
def agg_executive_scorecard_hos_coaching_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_hos_coaching_global")
