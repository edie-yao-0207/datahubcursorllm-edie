...
off_duty_deferrals AS (
    SELECT DISTINCT
        dd.org_id
    FROM datamodel_platform.dim_drivers dd
    WHERE
        dd.date = '{PARTITION_START}'
        AND (DATE(dd.deleted_at) IN ('0101-01-01', '0101-01-02') OR DATE(dd.deleted_at) >= DATE_SUB('{PARTITION_START}', 55))
        AND dd.eld_defer_off_duty_exemption_enabled = 1
...
relevant_orgs AS (
    -- Customers with at least one driver having off duty exemptions enabled are eligible
    SELECT
        o.org_id,
        org_name,
        CASE
    ...
    JOIN off_duty_deferrals odd
        ON o.org_id = odd.org_id
...
events AS (
    -- Usage is defined as all claims for off-duty deferral
    SELECT
        hos.org_id,
        DATE(hos.start_at) AS event_date,
        hos.driver_id AS user_id
    FROM compliancedb_shards.driver_hos_charts hos
    WHERE
       hos.defer_off_duty_enabled = 1
       AND DATE(hos.start_at) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
       AND hos.driver_id != 0
       AND hos.driver_id IS NOT NULL
    ...
    SELECT
        ...
        DATE(hos.start_at) AS event_date,
        hos.driver_id AS user_id
    FROM delta.`s3://samsara-eu-rds-delta-lake/table-parquet/prod-compliancedb/compliancedb/driver_hos_charts_v1` hos
    WHERE
       hos.defer_off_duty_enabled = 1
       AND DATE(hos.start_at) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
       AND hos.driver_id != 0
    ...
    SELECT
        ...
        DATE(hos.start_at) AS event_date,
        hos.driver_id AS user_id
    FROM delta.`s3://samsara-eu-rds-delta-lake/table-parquet/prod-compliance-shard-1db/compliancedb/driver_hos_charts_v1` hos
    WHERE
       hos.defer_off_duty_enabled = 1
       AND DATE(hos.start_at) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
       AND hos.driver_id != 0
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (claims submitted)
-- usage_monthly: Usage in the last 28 days (claims submitted)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (claims submitted)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (claims submitted)
-- daily_active_user: Number of unique users using the feature on the day in question (drivers submitting claims)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Off Duty Deferral feature""",
        row_meaning="""Each row represents an org's usage of Off Duty Deferral over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_off_duty_deferral_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_off_duty_deferral_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_off_duty_deferral_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_off_duty_deferral_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_off_duty_deferral_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_off_duty_deferral_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_off_duty_deferral_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_off_duty_deferral_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_off_duty_deferral_global_usage_change",
           sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:compliancedb_shards.driver_hos_charts",
        "us-west-2|eu-west-1:finopsdb.customer_info",
    ],
)
def agg_executive_scorecard_off_duty_deferral_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_off_duty_deferral_global")
