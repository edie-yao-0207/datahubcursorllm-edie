...
relevant_orgs AS (
    -- License filtering is below, but this is open to EA/Phase 1 orgs or orgs on the FF
    -- FF: https://app.launchdarkly.com/projects/samsara/flags/worker-safety-enabled/targeting?env=production&selected-env=production
    SELECT
        o.org_id,
        org_name,
        CASE
            ...
            AND (
                ...
                OR (
                    ldo.negate = FALSE AND (
                        ...
                        WHEN ldo.op = 'lessThan' THEN CAST(o.org_id AS STRING) < ldo.org_ids[0]
                        WHEN ldo.op = 'greaterThan' THEN CAST(o.org_id AS STRING) > ldo.org_ids[0] END
                    )
                )
                OR (
    ...
    WHERE
        ...
        AND ldo.variation IS NULL -- Excluded
...
final_relevant_orgs AS (
    ...
    -- 1. Orgs who have opted in via the Feature Flag
    -- 2. Orgs who have been added to the Beta
    SELECT
        ro.org_id,
        ro.org_name,
        ...
        ro.org_category,
        ro.region
    FROM opt_in_orgs_us o
    JOIN relevant_orgs ro
        ON o.org_id = ro.org_id
...
{ENABLED_USERS_SQL}
events_us AS (
    -- Usage is defined as any Worker Safety Mobile SOS signal or check-in
    SELECT
    ...
    WHERE
        s.orgid IS NOT NULL
        AND s.date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
    ...
    WHERE
        t.orgid IS NOT NULL
        AND t.date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
        AND i.user_id IS NULL -- Filter out internal users
),
events_eu AS (
    -- Usage is defined as any Worker Safety Mobile SOS signal or check-in
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (worker safety incidents or check-ins)
-- usage_monthly: Usage in the last 28 days (worker safety incidents or check-ins)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (worker safety incidents or check-ins)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (worker safety incidents or check-ins)
-- daily_active_user: Number of unique users using the feature on the day in question (in this context, triggering an incident or check-in)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (in this context, triggering an incident or check-in)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (in this context, triggering an incident or check-in)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Worker Safety (Mobile) feature""",
        row_meaning="""Each row represents an org's usage of Worker Safety (Mobile) over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_worker_safety_mobile_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_worker_safety_mobile_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_worker_safety_mobile_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_mobile_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_mobile_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_mobile_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_mobile_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_worker_safety_mobile_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_worker_safety_mobile_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:dynamodb.worker_safety_timers",
...
def agg_executive_scorecard_worker_safety_mobile_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_worker_safety_mobile_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("worker_safety_mobile")
    enabled_users_sql = get_enabled_users_sql(json_config, PARTITION_START)
    query = QUERY.format(
        PARTITION_START=PARTITION_START,
        flag_name='worker-safety-enabled',
