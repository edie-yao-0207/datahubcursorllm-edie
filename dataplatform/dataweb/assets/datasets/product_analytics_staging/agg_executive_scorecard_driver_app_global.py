...
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Usage in the last 7 days (Driver App events)
-- usage_monthly: Usage in the last 28 days (Driver App events)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (Driver App events)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (Driver App events)
...
-- org_active_day: Whether the org used the feature on the day in question (drivers)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Driver App feature""",
        row_meaning="""Each row represents an org's usage of Driver App over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_driver_app_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_driver_app_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_driver_app_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_app_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_app_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_app_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
                WHERE usage_weekly > 1000000000
            """,
            expected_value=0,
            operator=Operator.eq,
            block_before_write=False,
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_app_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_driver_app_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_driver_app_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:datamodel_platform_silver.stg_driver_app_events",
...
def agg_executive_scorecard_driver_app_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_driver_app_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("driver_app")
