...
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Usage in the last 7 days (API calls)
-- usage_monthly: Usage in the last 28 days (API calls)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (API calls)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (API calls)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the API""",
        row_meaning="""Each row represents an org's usage of the API over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_api_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_api_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_api_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_api_global_usage",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_api_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
                WHERE usage_weekly > 100000000
            """,
            expected_value=0,
            operator=Operator.eq,
            block_before_write=False,
        ),
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_fct_api_usage_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_api_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:product_analytics.fct_api_usage",
...
def agg_executive_scorecard_api_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_api_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("api")
