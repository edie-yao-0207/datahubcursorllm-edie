...
relevant_licenses AS (
    -- Any Telematics license is relevant
    ...
    WHERE
        ...
        AND (
            ...
            OR fo.product_sku IN ('LIC-CM1-ENT', 'LIC-CM2-ENT')
        )
...
{ENABLED_USERS_SQL}
events AS (
    -- All of the following Mixpanel events are relevant for LSD:
    -- 1. fleet_address_autocomplete_lsd_coordinate_search
    -- 2. fleet_address_autocomplete_location_selected
    -- 3. fleet_lsdoverlay_toggle
    ...
    FROM mixpanel_samsara.fleet_address_autocomplete_lsd_coordinate_search mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND mix.orgid = eu.org_id
    WHERE
    ...
    SELECT
        ...
        DATE_TRUNC('day', mix.mp_date) AS event_date,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.fleet_address_autocomplete_location_selected mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND mix.orgid = eu.org_id
    WHERE
    ...
    SELECT
        ...
        DATE_TRUNC('day', mix.mp_date) AS event_date,
        COALESCE(eu.user_id, mix.mp_user_id, mix.mp_device_id) AS user_id
    FROM mixpanel_samsara.fleet_lsdoverlay_toggle mix
    LEFT OUTER JOIN enabled_users eu
        ON COALESCE(mix.mp_user_id, mix.mp_device_id) = eu.email
        AND mix.orgid = eu.org_id
    WHERE
        ...
        AND COALESCE(mix.mp_user_id, mix.mp_device_id) NOT LIKE '%@samsara%'
        AND COALESCE(mix.mp_user_id, mix.mp_device_id) NOT LIKE '%@samsara-service-account.com'
        AND COALESCE(mix.toggledon, FALSE) = TRUE -- Only considered toggled on
),
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the LSD Mapping feature""",
        row_meaning="""Each row represents an org's usage of LSD Mapping over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_lsd_mapping_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_lsd_mapping_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_lsd_mapping_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_lsd_mapping_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_lsd_mapping_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_lsd_mapping_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
                WHERE usage_weekly > 20000
            """,
            expected_value=0,
            operator=Operator.eq,
            block_before_write=False,
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_lsd_mapping_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_lsd_mapping_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
           name="dq_sql_agg_executive_scorecard_lsd_mapping_global_usage_change",
           sql_query="""
               WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2:mixpanel_samsara.fleet_address_autocomplete_lsd_coordinate_search",
        "us-west-2:mixpanel_samsara.fleet_address_autocomplete_location_selected",
        "us-west-2:mixpanel_samsara.fleet_lsdoverlay_toggle",
...
def agg_executive_scorecard_lsd_mapping_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_lsd_mapping_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("lsd_mapping")
