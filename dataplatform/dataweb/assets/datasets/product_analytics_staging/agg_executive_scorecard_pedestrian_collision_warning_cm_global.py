...
latest_org_settings_us AS (
    ...
    SELECT
        ...
        road_facing_dashcam_enabled AS org_road_facing_dashcam_enabled,
...
latest_device_settings_us AS (
    ...
    SELECT
        ...
        road_facing_dashcam_enabled AS device_road_facing_dashcam_enabled,
...
device_resolved_settings_us AS (
    ...
    SELECT
        ...
        COALESCE(ds.device_road_facing_dashcam_enabled, los.org_road_facing_dashcam_enabled) AS road_facing_dashcam_enabled,
...
device_enabled_history_us AS (
    ...
    WHERE
        enabled = true
        AND road_facing_dashcam_enabled = true
),
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Pedestrian Collision Warning (CM) feature""",
        row_meaning="""Each row represents an org's usage of Pedestrian Collision Warning (CM) over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_pedestrian_collision_warning_cm_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_pedestrian_collision_warning_cm_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_pedestrian_collision_warning_cm_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_pedestrian_collision_warning_cm_global_enablement", lookback_days=1, tolerance=0.02),
    ],
...
def agg_executive_scorecard_pedestrian_collision_warning_cm_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_pedestrian_collision_warning_cm_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("pedestrian_collision_warning_cm")
