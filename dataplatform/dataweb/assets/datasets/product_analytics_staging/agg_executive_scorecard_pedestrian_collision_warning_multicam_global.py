...
multicam_licenses AS (
    ...
    WHERE
        ...
        AND (
            fo.product_sku IN ('LIC-MC-AIM4', 'LIC-MC-AIM8')
        )
...
camera_role_settings_org_us AS (
    SELECT
        EntityId,
        Namespace,
        FROM_JSON(
        Settings,
        'struct<cameraRolesConfigurationSettings:struct<analogInput1:struct<cameraRoleSetting:struct<role:int>>, analogInput2:struct<cameraRoleSetting:struct<role:int>>, analogInput3:struct<cameraRoleSetting:struct<role:int>>, analogInput4:struct<cameraRoleSetting:struct<role:int>>>>'
        ).cameraRolesConfigurationSettings.analogInput1.cameraRoleSetting.role AS analog_input_1_role,
        FROM_JSON(
        Settings,
        'struct<cameraRolesConfigurationSettings:struct<analogInput1:struct<cameraRoleSetting:struct<role:int>>, analogInput2:struct<cameraRoleSetting:struct<role:int>>, analogInput3:struct<cameraRoleSetting:struct<role:int>>, analogInput4:struct<cameraRoleSetting:struct<role:int>>>>'
        ).cameraRolesConfigurationSettings.analogInput2.cameraRoleSetting.role AS analog_input_2_role,
        FROM_JSON(
        Settings,
        'struct<cameraRolesConfigurationSettings:struct<analogInput1:struct<cameraRoleSetting:struct<role:int>>, analogInput2:struct<cameraRoleSetting:struct<role:int>>, analogInput3:struct<cameraRoleSetting:struct<role:int>>, analogInput4:struct<cameraRoleSetting:struct<role:int>>>>'
        ).cameraRolesConfigurationSettings.analogInput3.cameraRoleSetting.role AS analog_input_3_role,
        FROM_JSON(
        Settings,
        'struct<cameraRolesConfigurationSettings:struct<analogInput1:struct<cameraRoleSetting:struct<role:int>>, analogInput2:struct<cameraRoleSetting:struct<role:int>>, analogInput3:struct<cameraRoleSetting:struct<role:int>>, analogInput4:struct<cameraRoleSetting:struct<role:int>>>>'
        ).cameraRolesConfigurationSettings.analogInput4.cameraRoleSetting.role AS analog_input_4_role,
        _approximate_creation_date_time
    FROM dynamodb.settings
    WHERE
        Namespace = 'MediaPlatformCameraRoles'
        AND EntityId LIKE 'ORG#%'
        AND DATE(FROM_UNIXTIME(_approximate_creation_date_time / 1000)) <= '{PARTITION_START}'
),
latest_camera_role_settings_org_us AS (
    -- Get the latest org-level camera role settings per org
    SELECT
        REGEXP_EXTRACT(EntityId, 'ORG#(.+)', 1) AS org_id,
        analog_input_1_role AS org_analog_input_1_role,
        analog_input_2_role AS org_analog_input_2_role,
        analog_input_3_role AS org_analog_input_3_role,
        analog_input_4_role AS org_analog_input_4_role,
        _approximate_creation_date_time AS org_timestamp
    FROM camera_role_settings_org_us
    QUALIFY ROW_NUMBER() OVER (PARTITION BY EntityId ORDER BY _approximate_creation_date_time DESC) = 1
),
camera_role_settings_us AS (
    SELECT
        EntityId,
        Namespace,
        FROM_JSON(
...
multicam_devices_base_us AS (
    ...
    WHERE
        ...
        AND dd.product_id = 213
),
multicam_devices_base_eu AS (
    SELECT
        dd.org_id,
        dd.device_id
...
device_camera_roles_us AS (
    SELECT
        md.device_id,
        md.org_id,
        -- Priority: device-level settings > org-level settings > defaults
        -- Defaults: Port 1=1 (LEFT_SIDE), Port 2=3 (RIGHT_SIDE), Port 3=5 (REAR_HIGH), Port 4=10 (OTHER_1)
        COALESCE(
            crs.analog_input_1_role,
            los.org_analog_input_1_role,
            1
        ) AS analog_input_1_role,  -- Default: LEFT_SIDE
        COALESCE(
            crs.analog_input_2_role,
            los.org_analog_input_2_role,
            3
        ) AS analog_input_2_role,  -- Default: RIGHT_SIDE
        COALESCE(
            crs.analog_input_3_role,
            los.org_analog_input_3_role,
            5
        ) AS analog_input_3_role,  -- Default: REAR_HIGH
        COALESCE(
            crs.analog_input_4_role,
            los.org_analog_input_4_role,
            10
        ) AS analog_input_4_role,  -- Default: OTHER_1
        -- Use device timestamp if device has settings, otherwise org timestamp, otherwise NULL
        COALESCE(
            crs._approximate_creation_date_time,
            los.org_timestamp
        ) AS _approximate_creation_date_time
    FROM multicam_devices_base_us md
    LEFT JOIN camera_role_settings_us crs
        ON crs.EntityId = CONCAT('DEVICE#', CAST(md.device_id AS STRING))
        AND crs.Namespace = 'MediaPlatformCameraRoles'
    LEFT JOIN latest_camera_role_settings_org_us los
        ON md.org_id = CAST(los.org_id AS BIGINT)
),
external_multicam_us AS (
    -- at least one camera is external-facing
    SELECT DISTINCT org_id, device_id
    FROM device_camera_roles_us
    WHERE
        (
            analog_input_1_role IN (1, 3, 5)
            OR analog_input_2_role IN (1, 3, 5)
            OR analog_input_3_role IN (1, 3, 5)
            OR analog_input_4_role IN (1, 3, 5)
        )
        AND (
            DATE(FROM_UNIXTIME(_approximate_creation_date_time / 1000)) <= '{PARTITION_START}'
            OR _approximate_creation_date_time IS NULL
        )
),
camera_role_settings_org_eu AS (
    SELECT
        EntityId,
        Namespace,
        FROM_JSON(
...
latest_camera_role_settings_org_eu AS (
    -- Get the latest org-level camera role settings per org
    SELECT
        REGEXP_EXTRACT(EntityId, 'ORG#(.+)', 1) AS org_id,
        analog_input_1_role AS org_analog_input_1_role,
        ...
        analog_input_4_role AS org_analog_input_4_role,
        _approximate_creation_date_time AS org_timestamp
    FROM camera_role_settings_org_eu
    QUALIFY ROW_NUMBER() OVER (PARTITION BY EntityId ORDER BY _approximate_creation_date_time DESC) = 1
),
camera_role_settings_eu AS (
    SELECT
        EntityId,
        Namespace,
        FROM_JSON(
...
device_camera_roles_eu AS (
    ...
    FROM multicam_devices_base_eu md
    LEFT JOIN camera_role_settings_eu crs
        ON crs.EntityId = CONCAT('DEVICE#', CAST(md.device_id AS STRING))
        AND crs.Namespace = 'MediaPlatformCameraRoles'
    LEFT JOIN latest_camera_role_settings_org_eu los
        ON md.org_id = CAST(los.org_id AS BIGINT)
),
external_multicam_eu AS (
    -- at least one camera is external-facing
    SELECT DISTINCT org_id, device_id
    FROM device_camera_roles_eu
    WHERE
        (
            analog_input_1_role IN (1, 3, 5)
            OR analog_input_2_role IN (1, 3, 5)
...
multicam_devices AS (
    SELECT DISTINCT emu.org_id
    FROM external_multicam_us emu

    UNION ALL

    SELECT DISTINCT emu.org_id
    FROM external_multicam_eu emu
),
total_active_multicams_us AS (
    SELECT DISTINCT
        dd.org_id,
        dd.device_id,
        lda.l1,
    ...
    JOIN external_multicam_us emu
        ON lda.device_id = emu.device_id
...
all_roadside_settings_org_us AS (
    SELECT
        EntityId,
        Namespace,
        FROM_JSON(Settings, 'struct<vulnerableRoadUserCollisionWarningSettings:struct<audioAlerts:struct<enabled:boolean>, enabled:boolean, enabledCameraStreams:struct<front:boolean, left:boolean, rear:boolean, right:boolean, roadFacingDashcam:boolean>>>').vulnerableRoadUserCollisionWarningSettings.enabled AS enabled,
        FROM_JSON(Settings, 'struct<vulnerableRoadUserCollisionWarningSettings:struct<audioAlerts:struct<enabled:boolean>, enabled:boolean, enabledCameraStreams:struct<front:boolean, left:boolean, rear:boolean, right:boolean, roadFacingDashcam:boolean>>>').vulnerableRoadUserCollisionWarningSettings.enabledCameraStreams.rear AS rear_enabled,
        FROM_JSON(Settings, 'struct<vulnerableRoadUserCollisionWarningSettings:struct<audioAlerts:struct<enabled:boolean>, enabled:boolean, enabledCameraStreams:struct<front:boolean, left:boolean, rear:boolean, right:boolean, roadFacingDashcam:boolean>>>').vulnerableRoadUserCollisionWarningSettings.enabledCameraStreams.left AS left_enabled,
        FROM_JSON(Settings, 'struct<vulnerableRoadUserCollisionWarningSettings:struct<audioAlerts:struct<enabled:boolean>, enabled:boolean, enabledCameraStreams:struct<front:boolean, left:boolean, rear:boolean, right:boolean, roadFacingDashcam:boolean>>>').vulnerableRoadUserCollisionWarningSettings.enabledCameraStreams.right AS right_enabled,
        _approximate_creation_date_time
    FROM dynamodb.settings
    WHERE
...
latest_org_settings_us AS (
    ...
    SELECT
        ...
        rear_enabled AS org_rear_enabled,
        left_enabled AS org_left_enabled,
        right_enabled AS org_right_enabled,
        _approximate_creation_date_time AS org_timestamp
...
latest_device_settings_us AS (
    ...
    -- Settings stores VG device ID, so getting multicam device ID from dim_devices
    SELECT
        associated_device.device_id AS device_id,
        enabled AS device_enabled,
        rear_enabled AS device_rear_enabled,
        left_enabled AS device_left_enabled,
        right_enabled AS device_right_enabled,
    ...
    LATERAL VIEW EXPLODE(dd.associated_devices_array) AS associated_device
    WHERE
        Namespace = 'SafetyEventDetection'
        AND dd.date = '{PARTITION_START}'
        AND associated_device.product_id = 213
    QUALIFY ROW_NUMBER() OVER (PARTITION BY EntityId ORDER BY _approximate_creation_date_time DESC) = 1
),
device_resolved_settings_us AS (
    ...
    SELECT
        ...
        COALESCE(ds.device_rear_enabled, los.org_rear_enabled) AS rear_enabled,
        COALESCE(ds.device_left_enabled, los.org_left_enabled) AS left_enabled,
        COALESCE(ds.device_right_enabled, los.org_right_enabled) AS right_enabled,
    ...
    FROM total_active_multicams_us d
...
device_enabled_history_us AS (
    ...
    WHERE
        enabled = true
        AND (right_enabled = true OR left_enabled = true OR rear_enabled = true)
),
...
current_device_settings_us AS (
    ...
    WHERE
        enabled = true
        AND (left_enabled = true OR right_enabled = true OR rear_enabled = true)
),
...
active_us AS (
    ...
    FROM total_active_multicams_us tam
...
device_resolved_settings_eu AS (
    ...
    SELECT
        ...
        -- Use device-level timestamp if device has settings, otherwise org-level timestamp
        COALESCE(ds.device_timestamp, los.org_timestamp) AS effective_timestamp
    FROM total_active_multicams_eu d
...
active_eu AS (
    SELECT
        ...
        COALESCE(COUNT(DISTINCT CASE WHEN DATE(edu.enabled_at) <= '{PARTITION_START}' AND tam.p28 > 0 THEN tam.device_id END), 0) AS usage_p28,
        COALESCE(COUNT(DISTINCT CASE WHEN DATE(edu.enabled_at) <= '{PARTITION_START}' AND tam.p7 > 0 THEN tam.device_id END), 0) AS usage_p7
    FROM total_active_multicams_eu tam
...
-- Definitions:
-- usage_weekly: Usage in the last 7 days (enabled for pedestrian collision warning)
-- usage_monthly: Usage in the last 28 days (enabled for pedestrian collision warning)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (enabled for pedestrian collision warning)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (enabled for pedestrian collision warning)
...
JOIN multicam_devices a
    ON o.org_id = a.org_id
JOIN multicam_licenses l
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Pedestrian Collision Warning (Multicam) feature""",
        row_meaning="""Each row represents an org's usage of Pedestrian Collision Warning (Multicam) over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_pedestrian_collision_warning_multicam_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_pedestrian_collision_warning_multicam_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_pedestrian_collision_warning_multicam_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_pedestrian_collision_warning_multicam_global_enablement", lookback_days=1, tolerance=0.02),
    ],
...
def agg_executive_scorecard_pedestrian_collision_warning_multicam_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_pedestrian_collision_warning_multicam_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("pedestrian_collision_warning_multicam")
