...
SCHEMA = [
    ...
    },
    {
        ...
        "nullable": False,
        "metadata": {
            "comment": "Queried from edw.salesforce_sterling.account. Classifies accounts by billing country"
        },
    },
    {
        "name": "region",
...
detection_types AS (
    ...
    SELECT 'Following Distance' AS detection_type, 'premier_enterprise' AS allowed_tiers, 'single-facing CM' AS device_type -- Following Distance

    UNION ALL

    SELECT 'Mobile Usage', 'premier_enterprise', 'dual-facing CM' -- Mobile Usage

    UNION ALL

    SELECT 'Drowsiness Detection', 'premier_enterprise', 'dual-facing CM' -- Drowsiness Detection

    UNION ALL

    SELECT 'Forward Collision Warning', 'premier_enterprise', 'single-facing CM' -- Forward Collision Warning

    UNION ALL

    SELECT 'No Seatbelt', 'premier_enterprise', 'dual-facing CM' -- No Seatbelt

    UNION ALL

    SELECT 'Inward Obstruction', 'premier_enterprise', 'dual-facing CM' -- Inward Obstruction

    UNION ALL

    SELECT 'Inattentive Driving', 'premier_enterprise', 'dual-facing CM' -- Inattentive Driving

    UNION ALL

    SELECT 'Lane Departure Warning', 'premier_enterprise', 'single-facing CM' -- Lane Departure Warning
),
...
daily_snapshot_source_us AS (
    -- Get device settings for 56 days - single scan with STACK to unpivot feature columns
    SELECT
        date,
        feature,
        feature_enabled,
    ...
    LATERAL VIEW STACK(8,
        'Mobile Usage', mobile_usage_audio_alerts_enabled,
        'Drowsiness Detection', drowsiness_detection_audio_alerts_enabled,
        'Forward Collision Warning', forward_collision_warning_audio_alerts_enabled,
        'Following Distance', following_distance_audio_alerts_enabled,
        'Inward Obstruction', inward_obstruction_audio_alerts_enabled,
        'No Seatbelt', seatbelt_audio_alerts_enabled,
        'Inattentive Driving', inattentive_driving_audio_alerts_enabled,
        'Lane Departure Warning', lane_departure_warning_audio_alerts_enabled
    ) t AS feature, feature_enabled
    WHERE
...
daily_snapshot_source_eu AS (
    SELECT
        ...
        vg_device_id,
        cm_device_id
    FROM data_tools_delta_share.product_analytics.dim_devices_safety_settings
    LATERAL VIEW STACK(8,
        'Mobile Usage', mobile_usage_audio_alerts_enabled,
        'Drowsiness Detection', drowsiness_detection_audio_alerts_enabled,
        'Forward Collision Warning', forward_collision_warning_audio_alerts_enabled,
...
daily_cms_base_data_for_joins AS (
    ...
    -- Mobile Usage/Drowsiness Detection: Dual-facing CMs
    -- Following Distance/Forward Collision Warning: Single-facing CMs
    SELECT /*+ BROADCAST(dt) */ DISTINCT
        ...
        l.p7,
        l.p28,
        CASE WHEN d_snap.feature_enabled = TRUE THEN 1 ELSE 0 END AS is_feature_enabled
    ...
    JOIN eligible_orgs eo
        ON dd.org_id = eo.org_id
    JOIN detection_types dt
        ON ( -- Eligibility based on device product name for the detection type
            (dt.device_type = 'dual-facing CM' AND dd.product_name IN ('CM32', 'CM34')) OR
            (dt.device_type = 'single-facing CM' AND dd.product_name IN ('CM31', 'CM32', 'CM33', 'CM34')) OR
    ...
    LEFT JOIN daily_snapshot_source_us d_snap
        ...
        AND dt.detection_type = d_snap.feature
        AND eo.feature = d_snap.feature
...
severe_speeding_settings AS (
    ...
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN l.l1 > 0 AND d.date = '{PARTITION_START}' AND ss.severe_speeding_in_cab_audio_alerts_enabled THEN d.device_id END) AS usage_l1,
        ...
        COUNT(DISTINCT CASE WHEN l.l7 > 0 AND d.date BETWEEN DATE_SUB('{PARTITION_START}', 6) AND '{PARTITION_START}' AND ss.severe_speeding_in_cab_audio_alerts_enabled THEN d.device_id END) AS usage_l7,
        ...
        COUNT(DISTINCT CASE WHEN l.l28 > 0 AND d.date BETWEEN DATE_SUB('{PARTITION_START}', 27) AND '{PARTITION_START}' AND ss.severe_speeding_in_cab_audio_alerts_enabled THEN d.device_id END) AS usage_l28,
        ...
        COUNT(DISTINCT CASE WHEN l.p28 > 0 AND d.date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND DATE_SUB('{PARTITION_START}', 28) AND ss.severe_speeding_in_cab_audio_alerts_enabled THEN d.device_id END) AS usage_p28,
        ...
        COUNT(DISTINCT CASE WHEN l.p7 > 0 AND d.date BETWEEN DATE_SUB('{PARTITION_START}', 34) AND DATE_SUB('{PARTITION_START}', 28) AND ss.severe_speeding_in_cab_audio_alerts_enabled THEN d.device_id END) AS usage_p7
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for safety incab alerts""",
        row_meaning="""Each row represents an org's usage of safety incab alerts over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_safety_incab_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_safety_incab_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_safety_incab_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_safety_incab_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_safety_incab_global_usage_change",
            sql_query="""
                WITH today_usage AS (
...
def agg_executive_scorecard_safety_incab_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_safety_incab_global")
