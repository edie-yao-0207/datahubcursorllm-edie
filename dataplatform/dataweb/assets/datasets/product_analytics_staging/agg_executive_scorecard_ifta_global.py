...
relevant_orgs AS (
    ...
    WHERE
        ...
        AND o.locale IN ('us', 'ca') --US and Canada only
...
relevant_licenses AS (
    -- The following licenses have access to the IFTA report
    -- Legacy
    -- VG-PREM
    -- VG-ENT
    -- VG-ENTERPRISE
    ...
    WHERE
        ...
        AND (
            ...
            OR fo.product_sku LIKE 'LIC-VG-PREM-%'
            OR fo.product_sku LIKE 'LIC-VG-ENT%'
            OR fo.product_sku LIKE 'LIC-VG-ESS-PLUS%'
...
cm_only_orgs AS (
    -- For CM-only orgs, you don't need fuel permissions to view the IFTA page
    SELECT DISTINCT organization_id AS org_id
    FROM clouddb.features_orgs
    WHERE feature_id IN (514, 14019) -- CM-only bundles

    UNION ALL

    SELECT DISTINCT organization_id AS org_id
    FROM delta.`s3://samsara-eu-rds-delta-lake/table-parquet/prod-clouddb/prod_db/features_orgs_v1`
    WHERE feature_id IN (514, 14019) -- CM-only bundles
),
{ENABLED_USERS_SQL}
events AS (
    -- Usage is defined as any visit to the IFTA page
    SELECT
...
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question (in this context, visiting the page)
-- weekly_active_users: Number of unique users using the feature in the last 7 days (in this context, visiting the page)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days (in this context, visiting the page)
-- monthly_active_users: Number of unique users using the feature in the last 28 days (in this context, visiting the page)
-- monthly_enabled_users: Number of unique users who had access to the feature in the last 28 days (in this context, visiting the page)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the IFTA feature""",
        row_meaning="""Each row represents an org's usage of IFTA over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_ifta_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_ifta_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_ifta_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ifta_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ifta_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ifta_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ifta_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_ifta_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_ifta_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:clouddb.features_orgs",
        "us-west-2|eu-west-1:finopsdb.customer_info",
    ],
)
def agg_executive_scorecard_ifta_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_ifta_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("ifta")
