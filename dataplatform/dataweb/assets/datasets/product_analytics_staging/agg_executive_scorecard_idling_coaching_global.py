...
relevant_orgs AS (
    -- Relevant orgs are not excluded from FF with proper license
    SELECT
        o.org_id,
        org_name,
        CASE
...
relevant_licenses AS (
    -- Core licenses for this feature:
    -- 1. Legacy Telematics
    -- 2. FY25: Telematics Enterprise, Telematics Premier
    -- 3. FY26: Telematics Enterprise, Telematics Premier
    ...
    WHERE
        ...
        AND (
            (
                fo.product_sku LIKE 'LIC-VG%' AND xref.is_legacy_license AND xref.is_core_license
            )
            OR fo.product_sku IN ('LIC-VG-ENT-PLTFM-ESS', 'LIC-VG-ENT-PLTFM-PREM', 'LIC-VG-PREM-PLTFM-ESS', 'LIC-VG-PREM-PLTFM-PREM', 'LIC-VG-PREMIER-PLTFM-ESS', 'LIC-VG-PREMIER-PLTFM-PREM') -- Telematics FY25
            OR fo.product_sku IN ('LIC-VG-ENTERPRISE', 'LIC-VG-PREMIER') -- Telematics FY26
        )
...
final_relevant_orgs AS (
    ...
    -- 1. EA/Phase 1/FF orgs with proper license
...
events AS (
    -- Usage is defined as all completed coaching sessions with idling as a behavior
    SELECT
        CAST(csb.org_id AS BIGINT) AS org_id,
        DATE(cs.completed_date) AS event_date,
    ...
    WHERE
        DATE(cs.completed_date) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
        AND coachable_item_type = 43 -- Idling
        AND csb.driver_id != 0
...
SELECT
    ...
    o.account_name,
    1 AS enabled,
    COALESCE(COUNT(DISTINCT CASE WHEN e.event_date BETWEEN DATE_SUB('{PARTITION_START}', 6) AND '{PARTITION_START}' THEN coaching_session_uuid END), 0) AS usage_weekly,
    COALESCE(COUNT(DISTINCT CASE WHEN e.event_date BETWEEN DATE_SUB('{PARTITION_START}', 27) AND '{PARTITION_START}' THEN coaching_session_uuid END), 0) AS usage_monthly,
    COALESCE(COUNT(DISTINCT CASE WHEN e.event_date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND DATE_SUB('{PARTITION_START}', 28) THEN coaching_session_uuid END), 0) AS usage_prior_month,
    COALESCE(COUNT(DISTINCT CASE WHEN e.event_date BETWEEN DATE_SUB('{PARTITION_START}', 34) AND DATE_SUB('{PARTITION_START}', 28) THEN coaching_session_uuid END), 0) AS usage_weekly_prior_month,
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Idling Coaching feature""",
        row_meaning="""Each row represents an org's usage of Idling Coaching over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_idling_coaching_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_idling_coaching_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_idling_coaching_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_idling_coaching_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_idling_coaching_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_idling_coaching_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_idling_coaching_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_idling_coaching_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_idling_coaching_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:coachingdb_shards.coaching_sessions_behavior",
...
def agg_executive_scorecard_idling_coaching_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_idling_coaching_global")
