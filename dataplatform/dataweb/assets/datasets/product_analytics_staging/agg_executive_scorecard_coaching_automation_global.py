...
relevant_orgs AS (
    -- GA, not for Phase 2 usage yet
    -- Including trial orgs from FF: https://app.launchdarkly.com/projects/samsara/flags/enable-automations/targeting?env=production&selected-env=production
    SELECT
        o.org_id,
        org_name,
        CASE
...
relevant_licenses AS (
    -- Safety Enterprise and Legacy Safety are the relevant licenses
    ...
    WHERE
        ...
        AND (
            fo.product_sku LIKE 'LIC-CM-D-ENT%'
            OR fo.product_sku LIKE 'LIC-CM-S-ENT%'
            OR (fo.product_sku LIKE 'LIC-CM%' AND xref.is_legacy_license)
            OR fo.product_sku IN ('LIC-CM-D-PREMIER', 'LIC-CM-S-PREMIER')
        )
...
workflow_runs_us AS (
    ...
    WHERE date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
),
workflow_runs_eu AS (
...
workflow_events_us AS (
    -- This shows all workflow runs off the automation configurations. If the configuration ran at least once, that's considered to be usage
    SELECT
...
events AS (
    SELECT * FROM workflow_events_us

    UNION ALL

    SELECT * FROM workflow_events_eu
)
-- Definitions:
-- usage_weekly: Usage in the last 7 days (automations that have run)
-- usage_monthly: Usage in the last 28 days (automations that have run)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (automations that have run)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (automations that have run)
-- daily_active_user: Number of unique users using the feature on the day in question (in this context, creating an automation)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question
-- weekly_active_users: Number of unique users using the feature in the last 7 days (in this context, creating an automation)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days
-- monthly_active_users: Number of unique users using the feature in the last 28 days (in this context, creating an automation)
...
SELECT
    ...
    o.account_name,
    1 AS enabled,
    COALESCE(COUNT(DISTINCT CASE WHEN e.event_date BETWEEN DATE_SUB('{PARTITION_START}', 6) AND '{PARTITION_START}' THEN CONCAT(e.org_id, '_', configuration_id) END), 0) AS usage_weekly,
    COALESCE(COUNT(DISTINCT CASE WHEN e.event_date BETWEEN DATE_SUB('{PARTITION_START}', 27) AND '{PARTITION_START}' THEN CONCAT(e.org_id, '_', configuration_id) END), 0) AS usage_monthly,
    COALESCE(COUNT(DISTINCT CASE WHEN e.event_date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND DATE_SUB('{PARTITION_START}', 28) THEN CONCAT(e.org_id, '_', configuration_id) END), 0) AS usage_prior_month,
    COALESCE(COUNT(DISTINCT CASE WHEN e.event_date BETWEEN DATE_SUB('{PARTITION_START}', 34) AND DATE_SUB('{PARTITION_START}', 28) THEN CONCAT(e.org_id, '_', configuration_id) END), 0) AS usage_weekly_prior_month,
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Coaching Automations feature""",
        row_meaning="""Each row represents an org's usage of Coaching Automation over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_coaching_automation_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_coaching_automation_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_coaching_automation_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_coaching_automation_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_coaching_automation_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_coaching_automation_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_coaching_automation_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_coaching_automation_global_enablement", lookback_days=1, tolerance=0.1),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_coaching_automation_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:dynamodb.automation_configurations",
        "us-west-2|eu-west-1:datamodel_core.dim_organizations",
        "us-west-2|eu-west-1:datastreams_history.dynamic_workflow_runs",
...
def agg_executive_scorecard_coaching_automation_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_coaching_automation_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("coaching_automation")
    enabled_users_sql = get_enabled_users_sql(json_config, PARTITION_START)
    query = QUERY.format(
        PARTITION_START=PARTITION_START,
        flag_name="enable-automations",
