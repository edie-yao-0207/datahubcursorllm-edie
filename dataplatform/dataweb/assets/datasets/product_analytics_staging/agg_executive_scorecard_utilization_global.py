...
relevant_orgs AS (
    -- Only FF orgs or opt-in orgs with proper license are relevant
    -- FF: https://app.launchdarkly.com/projects/samsara/flags/asset-utilization-v2-enabled/targeting?env=production&selected-env=production
    SELECT
        o.org_id,
        org_name,
        CASE
...
relevant_licenses AS (
    -- Telematics licenses are the relevant ones
...
final_relevant_orgs AS (
    ...
    -- 1. Orgs who have opted in via the Feature Flag and have a Telematics license
...
{ENABLED_USERS_SQL}
events AS (
    -- Usage is defined as all hits to the V2 Utilization page (from non-internal users)
    ...
    WHERE
        cr.routename IN ('fleet_asset_utilization_use_cases', 'fleet_asset_utilization_vehicle_engine_runtime')
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Utilization feature""",
        row_meaning="""Each row represents an org's usage of Utilization over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_utilization_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_utilization_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_utilization_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_utilization_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_utilization_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_utilization_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_utilization_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_utilization_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
           name="dq_sql_agg_executive_scorecard_utilization_global_usage_change",
           sql_query="""
               WITH today_usage AS (
...
def agg_executive_scorecard_utilization_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_utilization_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("utilization")
    enabled_users_sql = get_enabled_users_sql(json_config, PARTITION_START)
    query = QUERY.format(
        PARTITION_START=PARTITION_START,
        flag_name="asset-utilization-v2-enabled",
