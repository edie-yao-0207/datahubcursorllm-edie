...
{USAGE_SQL}
-- Definitions:
-- usage_weekly: Usage in the last 7 days (route planning actions executed)
-- usage_monthly: Usage in the last 28 days (route planning actions executed)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (route planning actions executed)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (route planning actions executed)
-- daily_active_user: Number of unique users using the feature on the day in question (users who performed actions)
-- daily_enabled_users: Number of unique users who had access to the feature on the day in question (users who had access to perform actions)
-- weekly_active_users: Number of unique users using the feature in the last 7 days (users who performed actions)
-- weekly_enabled_users: Number of unique users who had access to the feature in the last 7 days (users who had access to perform actions)
-- monthly_active_users: Number of unique users using the feature in the last 28 days (users who performed actions)
-- monthly_enabled_users: Number of unique users who had access to the feature in the last 28 days (users who had access to perform actions)
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Route Planning feature""",
        row_meaning="""Each row represents an org's usage of Route Planning over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_route_planning_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_route_planning_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_route_planning_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_route_planning_global_enabled_users",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_route_planning_global_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_route_planning_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_route_planning_global_high_active_users",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
        ...
        TrendDQCheck(name="dq_trend_agg_executive_scorecard_route_planning_global_enablement", lookback_days=1, tolerance=0.02),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_route_planning_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:auditsdb_shards.gql_audits",
...
def agg_executive_scorecard_route_planning_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_route_planning_global")
    partition_keys = partition_key_ranges_from_context(context)[0]
    PARTITION_START = partition_keys[0]
    json_config = load_product_usage_config("route_planning")
