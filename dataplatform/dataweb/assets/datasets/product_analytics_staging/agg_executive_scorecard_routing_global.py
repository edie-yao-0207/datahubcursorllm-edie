...
relevant_licenses AS (
    -- Any core VG license except for PubSec and FY2026 Premier has access
    ...
    WHERE
        ...
        AND (
            (fo.product_sku LIKE 'LIC-VG%' AND fo.product_sku != 'LIC-VG-PREMIER' AND fo.product_sku NOT LIKE 'LIC-VG-PUBSEC%' AND xref.is_core_license AND fo.product_sku != 'LIC-VG-ASAT')
            OR fo.product_sku IN (
                'LIC-AG-TRLR-ESS-PLTFM-PREM',
                'LIC-AG-UNPWR-PLTFM-PREM',
                'LIC-AG-TRLR-PREM-PLTFM-ESS',
                'LIC-AG-PWR-REEFER-PLTFM-PREM',
                'LIC-AG-TRLR-PREM-PLTFM-PREM',
                'LIC-AG-TRLR-PREM-KIT-PLTFM-PREM',
                'LIC-AG-PWR-PLUS-PLTFM-PREM',
                'LIC-AG-TRLR-ESS-PLTFM-ESS',
                'LIC-AG-PWR-REEFER-PLTFM-ESS',
                'LIC-AG-PWR-BASIC-PLTFM-ESS',
                'LIC-AG-TRLR-PREM-KIT-PLTFM-ESS',
                'LIC-AG-PWR-REEFER-KIT-PLTFM-ESS',
                'LIC-AG-PWR-PLUS-PLTFM-ESS',
                'LIC-AG-PWR-REEFER-KIT-PLTFM-PREM',
                'LIC-AG-PWR-BASIC-PLTFM-PREM',
                'LIC-AG2-ENT-PLTFM-PREM',
                'LIC-AG2-ENT-PLTFM-ESS',
                'LIC-AG-UNPWR-PLTFM-ESS'
            )
        )
...
route_daily_events_us AS (
    -- Usage is composed of route creation and job creation
    SELECT
        CAST(org_id AS BIGINT) AS org_id,
        DATE(timestamp_millis(scheduled_start_ms)) AS event_date
    FROM
        dispatchdb_shards.shardable_dispatch_routes
    WHERE
        org_id IS NOT NULL
        AND DATE(timestamp_millis(scheduled_start_ms)) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
),
route_daily_events_eu AS (
    SELECT
        CAST(org_id AS BIGINT) AS org_id,
        DATE(timestamp_millis(scheduled_start_ms)) AS event_date
    FROM
...
route_daily_events AS (
    SELECT * FROM route_daily_events_us

    UNION ALL

    SELECT * FROM route_daily_events_eu
),
job_daily_events_us AS (
    ...
    WHERE
        org_id IS NOT NULL
        AND TO_DATE(date) BETWEEN DATE_SUB('{PARTITION_START}', 55) AND '{PARTITION_START}'
),
...
job_daily_events AS (
    SELECT * FROM job_daily_events_us

    UNION ALL

    SELECT * FROM job_daily_events_eu
),
all_dispatch_events AS (
    SELECT org_id, event_date FROM route_daily_events

    UNION ALL

    SELECT org_id, event_date FROM job_daily_events
)
-- Definitions:
-- usage_weekly: Usage in the last 7 days (job/route creation)
-- usage_monthly: Usage in the last 28 days (job/route creation)
-- usage_prior_month: Usage in the 28 days prior to the last 28 days (job/route creation)
-- usage_weekly_prior_month: Usage in the 7 days prior to the last 28 days (job/route creation)
...
SELECT
    ...
    o.account_name,
    1 AS enabled,
    COALESCE(COUNT(CASE WHEN event_date BETWEEN DATE_SUB('{PARTITION_START}', 6) AND '{PARTITION_START}' THEN 1 END), 0) AS usage_weekly,
    COALESCE(COUNT(CASE WHEN event_date BETWEEN DATE_SUB('{PARTITION_START}', 27) AND '{PARTITION_START}' THEN 1 END), 0) AS usage_monthly,
    COALESCE(COUNT(CASE WHEN event_date BETWEEN DATE_SUB('{PARTITION_START}', 55) AND DATE_SUB('{PARTITION_START}', 28) THEN 1 END), 0) AS usage_prior_month,
    COALESCE(COUNT(CASE WHEN event_date BETWEEN DATE_SUB('{PARTITION_START}', 34) AND DATE_SUB('{PARTITION_START}', 28) THEN 1 END), 0) AS usage_weekly_prior_month,
...
LEFT OUTER JOIN all_dispatch_events d
...
@table(
    ...
    description=build_table_description(
        table_desc="""(Global) As part of Executive Scorecard, this shows usage stats for the Routing feature""",
        row_meaning="""Each row represents an org's usage of Routing over various time periods""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_agg_executive_scorecard_routing_global"),
        PrimaryKeyDQCheck(
            name="dq_pk_agg_executive_scorecard_routing_global",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_agg_executive_scorecard_routing_global",
            non_null_columns=PRIMARY_KEYS,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_routing_global_usage",
            sql_query="""
        ...
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_routing_global_high_usage",
            sql_query="""
                SELECT COUNT(*) AS observed_value
                FROM df
                WHERE usage_weekly > 500000
            """,
            expected_value=0,
            operator=Operator.eq,
        ...
        TrendDQCheck(
            name="dq_trend_agg_executive_scorecard_routing_global_enablement",
            lookback_days=1,
            tolerance=0.05,
            block_before_write=True,
        ),
        SQLDQCheck(
            name="dq_sql_agg_executive_scorecard_routing_global_usage_change",
            sql_query="""
                WITH today_usage AS (
    ...
    upstreams=[
        ...
        "us-west-2|eu-west-1:dispatchdb_shards.shardable_dispatch_routes",
        "us-west-2|eu-west-1:dispatchdb_shards.shardable_dispatch_jobs",
...
def agg_executive_scorecard_routing_global(context: AssetExecutionContext) -> str:
    context.log.info("Updating agg_executive_scorecard_routing_global")
