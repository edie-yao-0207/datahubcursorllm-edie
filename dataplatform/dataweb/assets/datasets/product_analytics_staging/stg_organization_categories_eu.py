...
SCHEMA = [
    ...
    },
    {
        ...
        "nullable": False,
        "metadata": {
            "comment": "Subregion name within the EU associated with maximal amount of trips in previous 28 days."
        },
    },
    {
    ...
    },
    {
        ...
        "nullable": False,
        "metadata": {
            "comment": "Category assignment of organization into 'region' category, all organizations assigned to 'eu_all'."
        },
    },
    {
        "name": "raw_fleet_size",
...
WITH
    ...
    max_weight AS (
    SELECT LOWER(associated_vehicle_make) as associated_vehicle_make,
       LOWER(associated_vehicle_model) as associated_vehicle_model,
       MEDIAN(max_weight_lbs) AS associated_vehicle_max_weight_lbs
    FROM read_files(
            's3://samsara-eu-databricks-workspace/dataengineering/device_eu_interpolation.csv',
            format => 'csv',
            header => true)
    WHERE max_weight_lbs > 0
    GROUP BY 1,2),
    eu_device_max_weight AS (
    SELECT LOWER(associated_vehicle_make) as associated_vehicle_make,
       LOWER(associated_vehicle_model) as associated_vehicle_model,
       MEDIAN(associated_vehicle_max_weight_lbs) AS associated_vehicle_max_weight_lbs
    FROM dim_devices
    WHERE associated_vehicle_max_weight_lbs > 0
    GROUP BY 1,2),
    devices AS (
    SELECT dim_devices.* EXCEPT(associated_vehicle_max_weight_lbs),
        CASE WHEN dim_devices.associated_vehicle_max_weight_lbs > 0 THEN dim_devices.associated_vehicle_max_weight_lbs
             WHEN eu_device_max_weight.associated_vehicle_max_weight_lbs > 0 THEN eu_device_max_weight.associated_vehicle_max_weight_lbs
             WHEN max_weight.associated_vehicle_max_weight_lbs > 0  THEN max_weight.associated_vehicle_max_weight_lbs
            END AS associated_vehicle_max_weight_lbs
    FROM dim_devices
    ...
    LEFT OUTER JOIN max_weight
    ON (COALESCE(dim_devices.associated_vehicle_make, "NONE") = max_weight.associated_vehicle_make
    AND COALESCE(dim_devices.associated_vehicle_model, "NONE") = max_weight.associated_vehicle_model)
    LEFT OUTER JOIN eu_device_max_weight
     ON (COALESCE(dim_devices.associated_vehicle_make, "NONE") = eu_device_max_weight.associated_vehicle_make
    AND COALESCE(dim_devices.associated_vehicle_model, "NONE") = eu_device_max_weight.associated_vehicle_model)),
    tenure AS (
    SELECT org_id,
    ...
    FROM {source}.stg_population_density_eu
    ...
    SELECT dim_organizations.date,
        ...
        CASE
        WHEN locale IN ('gb', 'ie') THEN 'uk_and_ireland'
        WHEN locale = 'de' THEN 'germany'
        ELSE 'eu_all'
    ...
    SELECT
        ...
        fct_trips.distance_miles,
        CASE
        WHEN fct_trips.start_country  IN ('GB', 'IE') THEN 'uk_and_ireland'
        WHEN fct_trips.start_country = 'DE' THEN 'germany' ELSE 'eu_all'
        END AS subregion
    ...
    SELECT trips.org_id,
        ...
        SUM(trips.duration_mins) / MAX(total_durations.total_duration_mins) AS proportion_duration_subregion
    FROM trips
    LEFT JOIN total_durations
        ON total_durations.org_id = trips.org_id
    WHERE trips.subregion IS NOT NULL
    GROUP BY 1,2),
    subregion AS (
    SELECT org_id,
        MAX_BY(subregion, proportion_duration_subregion) AS max_subregion,
        MAX(proportion_duration_subregion) AS max_proportion_duration_subregion
    FROM subregion_pre
    GROUP BY 1),
    ...
    SELECT '{PARTITION_START}' AS date,
        ...
        subregion.max_subregion AS raw_max_subregion,
        subregion.max_proportion_duration_subregion AS raw_max_proportion_duration_subregion,
        'eu_all' AS region,
        ...
        CAST(NULL AS DOUBLE) AS raw_hazmat_eligible_trip_proportion,
        CAST(NULL AS DOUBLE) AS raw_non_hazmat_eligible_trip_proportion,
        'all' AS transport_licenses_endorsements
    FROM orgs
    LEFT OUTER JOIN avg_mileage ON avg_mileage.org_id = orgs.org_id
    LEFT OUTER JOIN subregion ON subregion.org_id = orgs.org_id
...
@table(
    ...
    description=build_table_description(
        table_desc="""Assignment of organizations to categories used for Fleet Benchmarks report""",
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_stg_organization_categories_eu"),
        PrimaryKeyDQCheck(
            name="dq_pk_stg_organization_categories_eu",
            primary_keys=PRIMARY_KEYS,
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_stg_organization_categories_eu",
            non_null_columns=[
                "date",
                "fleet_composition",
        ...
        ValueRangeDQCheck(
            name="dq_value_range_stg_organization_categories_eu",
            column_range_map={"raw_fuel_category_diesel": (0, 1)},
            block_before_write=False,
        ),
    ],
    upstreams=[
        "eu-west-1:product_analytics_staging.stg_population_density_eu",
        "eu-west-1:datamodel_core.dim_organizations",
        "eu-west-1:datamodel_telematics.fct_trips",
        "eu-west-1:datamodel_core.dim_devices",
        "eu-west-1:datamodel_core.lifetime_device_activity",
        "eu-west-1:datamodel_core.dim_organizations_tenure",
    ],
)
def stg_organization_categories_eu(context: AssetExecutionContext) -> str:
    context.log.info("Updating stg_organization_categories_eu")
