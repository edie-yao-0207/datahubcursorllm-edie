...
@table(
    ...
    description=build_table_description(
        table_desc="""(Regional) A dataset containing safety inbox events and associated statuses.""",
    ...
    upstreams=[],
    primary_keys=["date", "event_id", "event_ms", "org_id", "device_id"],
    partitioning=["date"],
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_safety_inbox_events_status", block_before_write=False),
        NonNullDQCheck(name="dq_non_null_safety_inbox_events_status", non_null_columns=["date", "org_id", "is_coaching_assigned", "is_customer_dismissed", "is_viewed", "is_actioned", "is_auto_coached", "is_critical_event", "has_safety_event_review", "device_id", "event_ms"], block_before_write=True),
        PrimaryKeyDQCheck(name="dq_pk_safety_inbox_events_status", primary_keys=["date", "event_id", "event_ms", "org_id", "device_id"], block_before_write=True)
    ],
...
def safety_inbox_events_status(context: AssetExecutionContext) -> str:
    ...
    time_to_resolution AS (
        ...
        FROM coaching_status a
        INNER JOIN users du
            ON a.coaching_state.user_id = du.id
            AND a.date = du.date
        LEFT JOIN auto_needs_coaching_events ance
    ...
    safety_event_details AS (
        SELECT a.*,
            ...
            b.days_to_resolve
        FROM safety_events a
        LEFT JOIN coaching_status cs
        ...
        LEFT JOIN auto_needs_coaching_events ance
            ...
            AND a.vg_device_id = ance.vg_device_id
            AND a.event_ms = ance.event_ms
        LEFT JOIN time_to_resolution_final b
            ON a.org_id = b.org_id
    ...
    safety_inbox_events_status AS (
        SELECT
            ...
            coaching_status,
            auto_needs_coaching
        FROM safety_event_details AS sie
        LEFT JOIN viewed_safety_events AS ve
            ON sie.event_ms = ve.event_ms AND sie.vg_device_id = ve.device_id AND sie.org_id = ve.org_id
        LEFT JOIN coaching_orgs co
        ...
        LEFT JOIN most_recent_survey mrs
    ...
    SELECT
        ...
        auto_needs_coaching,
        has_ser_license
    FROM safety_inbox_events_status
    WHERE date BETWEEN '{FIRST_PARTITION_START}' AND '{FIRST_PARTITION_END}'
    """
    return query
