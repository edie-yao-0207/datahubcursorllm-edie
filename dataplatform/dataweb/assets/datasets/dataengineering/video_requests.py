...
@table(
    ...
    description=build_table_description(
        table_desc="""(Regional) Video request statuses across a given day""",
    ...
    partitioning=["date"],
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_video_requests", block_before_write=False),
        NonNullDQCheck(name="dq_non_null_video_requests", non_null_columns=["date", "org_id", "is_retrieval_successful", "id", "created_at_ms", "device_type"], block_before_write=True),
        PrimaryKeyDQCheck(name="dq_pk_video_requests", primary_keys=["id", "date"], block_before_write=True)
    ],
...
def video_requests(context: AssetExecutionContext) -> str:
    ...
    FROM videos_requested vrr
    LEFT JOIN video_requests_succeeded vrs
