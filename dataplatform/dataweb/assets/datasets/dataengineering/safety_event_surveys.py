...
@table(
    ...
    description=build_table_description(
        table_desc="""(Regional) A dataset of safety event surveys.""",
    ...
    partitioning=["date"],
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_safety_event_surveys", block_before_write=False),
        NonNullDQCheck(name="dq_non_null_safety_event_surveys", non_null_columns=["date", "org_id", "event_ms", "device_id", "is_safety_event_review"], block_before_write=False),
        PrimaryKeyDQCheck(name="dq_pk_safety_event_surveys", primary_keys=["date", "event_ms", "org_id", "device_id"], block_before_write=False)
    ],
    backfill_start_date="2023-01-01",
    backfill_batch_size=5,
    write_mode=WarehouseWriteMode.OVERWRITE,
)
def safety_event_surveys(context: AssetExecutionContext) -> str:
    ...
    INNER JOIN datamodel_core.dim_organizations o -- THIS ENSURES NON-INTERNAL ORG DATA
        ON a.org_id = o.org_id
        AND a.date = o.date
    LEFT JOIN ser_job_event_lookup ser -- review request metadata will always exist; can be duplicates
