...
PRIMARY_KEYS = ["date", "event_time", "account_id", "metastore_id", "workspace_id", "source_table_full_name", "source_path", "source_type", "target_table_full_name", "target_path", "target_type"]
...
QUERY = """
    ...
    SELECT
        ...
        metastore_id,
        workspace_id,
        SPLIT(tl.source_table_full_name, '.')[1] AS source_database_name,
        source_table_full_name,
        source_path,
        source_type,
        SPLIT(tl.target_table_full_name, '.')[1] AS target_database_name,
        target_table_full_name,
        target_path,
        target_type
    FROM table_lineage tl
    JOIN dataplatform.database_owners do
        ON SPLIT(tl.source_table_full_name, '.')[1] = do.db
...
@table(
    ...
    description=build_table_description(
        table_desc="""All table lineage events from Biztech-owned schemas""",
        row_meaning="""Each row represents a table lineage event from a Biztech-owned schema""",
        related_table_info={},
    ...
    dq_checks=[
        NonEmptyDQCheck(name="dq_non_empty_biztech_table_lineage"),
    ],
)
def biztech_table_lineage(context: AssetExecutionContext) -> str:
    context.log.info("Updating biztech_table_lineage")
