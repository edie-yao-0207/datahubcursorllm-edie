...
@table(
    ...
    description=build_table_description(
        table_desc="""A dataset containing weekly aggregated Platform MPR metrics for faster retrieval in Tableau""",
        row_meaning="""Each row contains weekly Platform MPR metrics for a given set of dimensions""",
        related_table_info={},
    ...
    schema=generate_metric_schema(schema=PLATFORM_SCHEMA, date_column_name="date"),
    upstreams=[
    ...
    partitioning=WeeklyPartitionsDefinition(start_date="2023-01-02", day_offset=1),
    dq_checks=[
        NonEmptyDQCheck(name="dq_agg_product_scorecard_platform_mpr_global_weekly"),
        NonNullDQCheck(name="dq_non_null_agg_product_scorecard_platform_mpr_global_weekly", non_null_columns=["date", "period_start", "period_end"], block_before_write=True),
        PrimaryKeyDQCheck(name="dq_pk_agg_product_scorecard_platform_mpr_global_weekly", primary_keys=PRIMARY_KEYS, block_before_write=True),
        TrendDQCheck(name="dq_trend_agg_product_scorecard_platform_mpr_global_weekly", lookback_days=1, tolerance=0.1, period="weeks"),
    ],
    backfill_batch_size=1,
    write_mode=WarehouseWriteMode.OVERWRITE,
    max_retries=3,
)
def agg_product_scorecard_platform_mpr_global_weekly(context: AssetExecutionContext) -> DataFrame:

    context.log.info("Updating agg_product_scorecard_platform_mpr_global_weekly")
    spark = SparkSession.builder.enableHiveSupport().getOrCreate()

    DIMENSION_CONFIGS["root"] = DimensionConfig(name="root", data_type=DimensionDataType.STRING, default_value="'Unknown'", output_name="date")
    ...
    anchor_day = base_date - relativedelta(weeks=0)
