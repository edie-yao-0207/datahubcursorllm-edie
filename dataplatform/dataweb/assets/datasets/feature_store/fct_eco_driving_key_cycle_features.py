...
def get_asset_efficiency_intervals(
    ...
    """Get asset efficiency intervals from the asset_efficiency_intervals table."""
...
def calculate_gps_distance_segments(
    ...
    Returns:
    - DataFrame with org_id, object_id, timestamp, segment_distance_km
    """
    ...
    # Calculate distance between consecutive points using simple window
    ...
    location_with_prev = (
        ...
        .withColumn("prev_ecu_speed", F.lag("ecu_speed_mps").over(window_spec))
    )
...
def calculate_distance_hierarchical(
    ...
    anomalous_intervals = odo_with_anomalies.select("org_id", "object_id", "interval_start", "interval_end")

    gps_df = calculate_gps_distance(
        spark, anomalous_intervals, start_date, end_date, object_ids, region
    )

    # Combine: use GPS for anomalous periods, odometer otherwise
    final_distance_df = (
        gps_df.alias("gps")
        .join(odo_with_anomalies.alias("odo"), ["org_id", "object_id", "interval_start"], "left")
        .withColumn(
            "distance_km",
            F.when(
...
def filter_by_engine_engage_events(
    ...
    # Only keep fuel consumption during engine engage events
    result = (
        fuel_df.alias("f")
        .hint("range_join", RANGE_JOIN_BIN_SIZE_SECONDS)
        .join(
    ...
    result = (
        ...
        .withColumn(
            "overlap_pct",
            F.when(
                F.col("total_duration") > 0,
                F.col("overlap_duration") / F.col("total_duration"),
            ).otherwise(0),
        )
    )
    ...
    fuel_columns = [
        ...
        if col not in ["org_id", "object_id", "prev_timestamp", "timestamp"]
    ]
    ...
    for col_name in fuel_columns:
        select_cols.append(
            (F.col(f"f.{col_name}") * F.col("overlap_pct")).alias(col_name)
        )
    ...
    agg_exprs = [F.sum(col).alias(col) for col in fuel_columns]
...
def generate_eco_driving_report(
    ...
    asset_efficiency_intervals_df = get_asset_efficiency_intervals(
        spark, start_date, end_date, org_ids, object_ids
    )
    ...
    intervals_df = generate_intervals_from_key_cycles(asset_efficiency_intervals_df)
    ...
    fuel_df = fuel_df.drop("interval_end")
...
@table(
    ...
    schema=SCHEMA,
    primary_keys=["date", "cycle_id", "interval_start"],
    partitioning=["date"],
    upstreams=[],
    dq_checks=[
        NonEmptyDQCheck(
            name="dq_non_empty_fct_eco_driving_key_cycle_features",
            block_before_write=True,
        ),
        NonNullDQCheck(
            name="dq_non_null_fct_eco_driving_key_cycle_features",
            non_null_columns=["date", "org_id", "device_id", "interval_start"],
            block_before_write=True,
        ),
        PrimaryKeyDQCheck(
            name="dq_pk_fct_eco_driving_key_cycle_features",
            primary_keys=["date", "cycle_id", "interval_start"],
        ),
    ],
...
def fct_eco_driving_key_cycle_features(context: AssetExecutionContext) -> DataFrame:
    ...
    return generate_eco_driving_report(
        spark=spark,
