...
@metric(
    description="(Regional) Critical push alerts",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CONCAT(workflow_id, occurred_at_ms, object_ids))",
            "metadata": {
                "comment": "The count of critical push alerts"
            },
        },
    ],
    dimensions=[
        ...
        },
        {
            ...
            "metadata": {
                "comment": "Status of the incident"
            },
        },
        {
            "name": "severity",
            "metadata": {
                "comment": "Severity of the incident"
            },
        },
    ],
    ...
    upstreams=[
        ...
        AssetKey(["workflowsdb_shards", "actions"]),
        AssetKey(["workflowsdb_shards", "notification_role_recipients"]),
        AssetKey([Database.PRODUCT_ANALYTICS, "map_org_sam_number_latest"]),
        *REGIONAL_STG_ORGANIZATION_CATEGORY_DEPENDENCIES,
    ],
)
def critical_push_notifications(context: AssetExecutionContext) -> str:
