...
@metric(
    ...
    expressions=[
        {
            "expression_type": "avg",
            "expression": "COALESCE(any_value(mpge), 0)",
            "metadata": {
                "comment": "Org-level weighted miles per gallon, based on the average MPG per vehicle weighted by distance traveled."
            },
        }
    ],
    dimensions=[
        {
            "name": "date",
            "metadata": {"comment": "The end date of the window for which MPG is calculated."},
        },
        {
            "name": "org_id",
            "metadata": {
                "comment": "The Samsara cloud dashboard ID corresponding to the MPG value."
            },
        },
        {
            "name": "sam_number",
        ...
        },
        {
            ...
            "metadata": {"comment": "The length of time for which MPG is calculated"},
        },
        {
            "name": "avg_mileage",
            "metadata": {
    ...
    upstreams=[
        AssetKey([Database.DATAENGINEERING, "vehicle_mpg_lookback_by_org"]),
...
def vehicle_mpg_by_org(context: AssetExecutionContext) -> str:
