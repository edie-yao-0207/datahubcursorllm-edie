...
@metric(
    description="Drivers with at least one instance of Driver App activity",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN last_mobile_login_date IS NOT NULL THEN CONCAT(org_id, '_', driver_id) END)",
            "metadata": {
                "comment": "The total number of drivers with at least one instance of Driver App activity"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN last_mobile_login_date IS NOT NULL THEN CONCAT(org_id, '_', driver_id) END) * 1.0 / COUNT(DISTINCT CONCAT(org_id, '_', driver_id))",
            "metadata": {
                "comment": "The percent of drivers with at least one instance of Driver App activity"
            },
        },
    ],
    dimensions=[
...
def driver_app_users(context: AssetExecutionContext) -> str:
