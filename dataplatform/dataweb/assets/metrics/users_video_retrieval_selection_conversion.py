...
@metric(
    description="(Global) The percentage of users that have converted to the Selection page in the safety video retrieval funnel.",
    expressions=[
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN is_selection_conversion = 1 THEN distinct_id END) * 1.0 / COUNT(DISTINCT distinct_id)",
            "metadata": {
                "comment": "The percentage of users that have converted to the Selection page in the safety video retrieval funnel."
            },
        }
    ],
    dimensions=[
...
def users_video_retrieval_selection_conversion(context: AssetExecutionContext) -> str:
