...
@metric(
    description="(Regional) The percent of drivers with at least one attribute",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN attributes_size > 0 THEN CONCAT(org_id, '_', driver_id) END)",
            "metadata": {
                "comment": "The count of drivers with at least one attribute"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN attributes_size > 0 THEN CONCAT(org_id, '_', driver_id) END) * 1.0 / COUNT(DISTINCT CONCAT(org_id, '_', driver_id))",
            "metadata": {
                "comment": "The percent of drivers with at least one attribute"
            },
        },
    ],
    dimensions=[
...
def driver_attributes(context: AssetExecutionContext) -> str:
