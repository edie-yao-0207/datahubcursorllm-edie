...
@metric(
    description="(Regional) The total number of orgs having all users as admins",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN total_users >= 2 AND fraction_admin >= 1 THEN org_id END)",
            "metadata": {
                "comment": "The total number of orgs having all users as admins"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN total_users >= 2 AND fraction_admin >= 1 THEN org_id END) * 1.0 / COUNT(DISTINCT CASE WHEN total_users >= 2 THEN org_id END)",
            "metadata": {
                "comment": "The percent of orgs where all users are admins"
            },
        },
    ],
    dimensions=[
...
def full_admin_orgs(context: AssetExecutionContext) -> str:
