...
@metric(
    description="(Global) Summarizes devices and their last date of activity (heartbeat and trip). Right now, this is only focusing on powered devices.",
    expressions=[
        {
            "expression_type": "avg",
            "expression": "AVG(days_since_last_activity)",
            "metadata": {
                "comment": """Average days since last activity for devices"""
            },
        },
        {
            "expression_type": "median",
            "expression": "PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY days_since_last_activity)",
            "metadata": {
                "comment": "The median number of days since the last activity for devices."
            },
        }
    ],
    dimensions=[
...
def devices_duration_since_last_activity(context: AssetExecutionContext) -> str:
