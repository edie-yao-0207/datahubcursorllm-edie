    ...
    expressions=[
        {
            ...
            "expression": "COUNT(DISTINCT device_id) FILTER(WHERE is_device_active = 1)",
            "metadata": {
                "comment": """The total number of customer devices that are active (either a heartbeat or a heartbeat/trip)."""
            },
        },
        {
            "expression_type": "percent",
            "expression": "SUM(is_device_active) * 1.0 / COUNT(is_device_active)",
            "metadata": {
                "comment": """The percentage of devices that are active (either a heartbeat or a heartbeat/trip)."""
            },
        },
    ],
    dimensions=[
...
def devices_active(context: AssetExecutionContext) -> str:
