...
@metric(
    description="(Global) Tracks the number of completed coaching sessions",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(CASE WHEN is_session_completed = 1 THEN CONCAT(id, '_', session_started_time) END)",
            "metadata": {
                "comment": "The number of completed coaching sessions"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(CASE WHEN is_session_completed = 1 THEN CONCAT(id, '_', session_started_time) END) * 1.0 / COUNT(CONCAT(id, '_', session_started_time))",
            "metadata": {
                "comment": "The percent of completed coaching sessions"
            },
        }
    ],
    dimensions=[
        ...
        },
        {
            ...
            "metadata": {
                "comment": "A boolean flag indicating whether the coaching session was completed."
            },
        },
        {
            "name": "region",
            "metadata": {
                "comment": "The Samsara cloud region that the coaching session occurred in."
            },
        },
        {
            "name": "avg_mileage",
    ...
    upstreams=[
        AssetKey([Database.DATAENGINEERING, "safety_event_coaching_sessions_global"]),
...
def coaching_sessions_completed(context: AssetExecutionContext) -> str:
