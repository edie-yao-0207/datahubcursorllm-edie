...
@metric(
    description="(Regional) Inattentive driving statistics for vehicles (VGs with CM32/CM34)",
    expressions=[
        {
            "expression_type": "sum",
            "expression": "COALESCE(SUM(total_distraction_count), 0)",
            "metadata": {
                "comment": "The total number of inattentive driving events"
            },
        },
        {
            "expression_type": "avg",
            "expression": "SUM(COALESCE(total_distraction_count, 0)) / (SUM(COALESCE(distance_miles, 0)) / 1000)",
            "metadata": {
                "comment": "The average number of inattentive driving events per 1k miles"
            },
        }
    ],
    dimensions=[
...
def vehicle_inattentive_driving(context: AssetExecutionContext) -> str:
