...
@metric(
    description="Paying customers: non-internal orgs with an active license, calculated at SAM level",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN is_paid_customer THEN sam_number END)",
            "metadata": {
                "comment": "The total number of SAMs that are considered to be paid customers"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN is_paid_customer THEN sam_number END) * 1.0 / COUNT(DISTINCT sam_number)",
            "metadata": {
                "comment": "The total number of SAMs that are considered to be paid customers"
            },
        },
...
def paying_customers(context: AssetExecutionContext) -> str:
