...
@metric(
    description="(Regional) Completed routes from fct_dispatch_routes",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN is_completed_route THEN CONCAT(org_id, '_', route_id) END)",
            "metadata": {
                "comment": "The total number of completed routes"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN is_completed_route THEN CONCAT(org_id, '_', route_id) END) * 1.0 / COUNT(DISTINCT CONCAT(org_id, '_', route_id))",
            "metadata": {
                "comment": "The percent of routes that are completed"
            },
        },
    ],
    dimensions=[
    ...
    upstreams=[
        AssetKey([Database.DATAMODEL_TELEMATICS, "fct_dispatch_routes"]),
...
def completed_routes(context: AssetExecutionContext) -> str:
