...
@metric(
    description="(Regional) The total number of CSV uploads with at least one error row",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(CASE WHEN total_rows_errored > 0 THEN uuid END)",
            "metadata": {
                "comment": "The total number of CSV uploads with at least one error row"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(CASE WHEN total_rows_errored > 0 THEN uuid END) * 1.0 / COUNT(uuid)",
            "metadata": {
                "comment": "The percent of CSV uploads with at least one error row"
            },
        },
    ],
    dimensions=[
        ...
        },
        {
            "name": "is_internal",
            "metadata": {
                "comment": "Whether the user who uploaded the CSV is an internal Samsara user or not"
            },
        },
        {
            "name": "status",
            "metadata": {
                "comment": "Status of CSV upload"
            },
        },
    ],
    ...
    upstreams=[
        AssetKey(["csvuploadsdb_shards", "csv_uploads"]),
...
def csv_upload_failures(context: AssetExecutionContext) -> str:
