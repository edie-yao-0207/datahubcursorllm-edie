...
@metric(
    description="(Regional) Percentage of time drivers were in violation of HOS rules for an org.",
    expressions=[
        {
            "expression_type": "ratio",
            "expression": "SUM(violation_duration_ms) / SUM(possible_driver_ms)",
            "metadata": {
                "comment": "Org-level proportion of driver time in violation of HOS to the total amount of possible driver time for the org.",
            }
         },
    ],
    dimensions=[
        {
            "name": "date",
            "metadata": {"comment": "Date when HOS violations took place"},
        },
        {
            "name": "org_id",
            "metadata": {"comment": "Samsara Cloud Dashboard ID"},
        },
        {
            "name": "sam_number",
            "metadata": {
    ...
    upstreams=[
        ...
        AssetKey(
            ["eldhosdb_shards", "driver_hos_ignored_violations"],
        ),
        AssetKey(
            [Database.DATAMODEL_PLATFORM, "dim_drivers"],
        ),
        AssetKey(
            [Database.DATAMODEL_CORE, "dim_organizations"],
        ),
        AssetKey([Database.PRODUCT_ANALYTICS, "map_org_sam_number_latest"]),
        *REGIONAL_STG_ORGANIZATION_CATEGORY_DEPENDENCIES,
    ],
)
def org_hos_violation(context: AssetExecutionContext) -> str:
