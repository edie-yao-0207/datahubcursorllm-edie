...
@metric(
    description="(Global) The total number of customer reviewed safety inbox events.",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(CASE WHEN is_viewed = 1 THEN CONCAT(device_id, '_', event_ms) END)",
            "metadata": {"comment": "The total number of customer reviewed safety inbox events."},
        }
    ],
    dimensions=[
        {
...
def safety_inbox_events_reviewed(context: AssetExecutionContext) -> str:
