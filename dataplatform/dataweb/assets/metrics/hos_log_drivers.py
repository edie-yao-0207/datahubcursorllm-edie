...
@metric(
    description="(Regional) Count of drivers creating HoS logs",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CONCAT(org_id, '_', driver_id))",
            "metadata": {
                "comment": "The total number of drivers who have created an HoS log"
            },
        },
    ],
    dimensions=[
        ...
        {
            "name": "driver_id",
            "metadata": {"comment": "The ID of the driver that the data belongs to"},
        },
        {
            "name": "device_id",
            "metadata": {"comment": "The ID of the customer device that the data belongs to"},
        },
        {
            "name": "sam_number",
            "metadata": {
    ...
    upstreams=[
        AssetKey([Database.DATAMODEL_TELEMATICS_SILVER, "stg_hos_logs"]),
...
def hos_log_drivers(context: AssetExecutionContext) -> str:
