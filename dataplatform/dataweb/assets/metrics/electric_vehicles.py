...
@metric(
    description="(Regional) Electric vehicles and their fleet composition",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN fuel_group = 4 THEN device_id END)",
            "metadata": {
                "comment": "The total number of electric vehicles."
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN fuel_group = 4 THEN device_id END) / COUNT(DISTINCT device_id)",
            "metadata": {
                "comment": "The percentage of vehicles in the fleet that are EVs"
            },
        },
    ],
    dimensions=[
    ...
    upstreams=[
        ...
        AssetKey([Database.PRODUCT_ANALYTICS_STAGING, "dim_device_vehicle_properties"]),
        AssetKey([Database.PRODUCT_ANALYTICS, "map_org_sam_number_latest"]),
        *REGIONAL_STG_ORGANIZATION_CATEGORY_DEPENDENCIES,
    ],
)
def electric_vehicles(context: AssetExecutionContext) -> str:
