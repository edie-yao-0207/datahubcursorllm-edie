...
@metric(
    description="(Regional) Crash statistics for vehicles (VGs with cameras)",
    expressions=[
        {
            "expression_type": "sum",
            "expression": "COALESCE(SUM(crash_count), 0)",
            "metadata": {
                "comment": "The total number of crashes"
            },
        },
        {
            "expression_type": "avg",
            "expression": "SUM(COALESCE(crash_count, 0)) / (SUM(COALESCE(distance_miles, 0)) / 1000)",
            "metadata": {
                "comment": "The average number of crashes per 1000 miles"
            },
        }
    ],
    dimensions=[
...
def vehicle_crashes(context: AssetExecutionContext) -> str:
