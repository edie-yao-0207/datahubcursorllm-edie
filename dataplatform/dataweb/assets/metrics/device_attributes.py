...
@metric(
    description="(Regional) The percent of devices with at least one attribute",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN attributes_size > 0 THEN device_id END)",
            "metadata": {
                "comment": "The count of devices with at least one attribute"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN attributes_size > 0 THEN device_id END) * 1.0 / COUNT(DISTINCT device_id)",
            "metadata": {
                "comment": "The percent of devices with at least one attribute"
            },
        },
    ],
    dimensions=[
...
def device_attributes(context: AssetExecutionContext) -> str:
