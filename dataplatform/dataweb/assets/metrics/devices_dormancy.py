...
@metric(
    description="(Global) Breaks down the overall dormancy (last active date) for devices",
    expressions=[
        {
            ...
            "metadata": {
                "comment": """
                    The count of devices having a specific dormancy bucket. Buckets are based on the last active/online date for devices.
                    For powered devices (including powered AGs), this is derived from the lifetime_device_activity table (all powered devices need to have a heartbeat from a trip to be considered active).
                    Dormancy is determined as the difference between the current date and last active date.
                    Right now, this metric is only accounting for powered devices.
                    """
            },
        },
    ],
    dimensions=[
        ...
        {
            "name": "dormancy_bucket",
            "metadata": {"comment": "Bucket representing days since last activity"},
        },
        {
    ...
    glossary_terms=[GlossaryTerm.PLATFORM, GlossaryTerm.BUSINESS_VALUE],
    upstreams=[
        AssetKey([Database.DATAENGINEERING, "device_dormancy_global"]),
...
def devices_dormancy(context: AssetExecutionContext) -> str:
