...
@metric(
    description="(Regional) Average time (minutes) to complete training",
    expressions=[
        {
            "expression_type": "avg",
            "expression": "AVG(COALESCE(number_value, 0) / 60)",
            "metadata": {
                "comment": "The average time to complete a training course"
            },
        },
    ],
    dimensions=[
    ...
    upstreams=[
        ...
        AssetKey(["formsdb_shards", "form_index_number_fields"]),
...
def trainings_time_to_complete(context: AssetExecutionContext) -> str:
