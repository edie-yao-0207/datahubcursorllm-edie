...
@metric(
    description="(Regional) Statistics for total time spent severe speeding (VG devices only).",
    expressions=[
        {
            "expression_type": "sum",
            "expression": "SUM(severe_speeding_ms)",
            "metadata": {
                "comment": "The total amount of time the vehicle is considered to be severe speeding (in milliseconds)"
            },
        },
        {
            "expression_type": "percent",
            "expression": "100.00 * SUM(COALESCE(severe_speeding_ms, 0)) / SUM(COALESCE(driving_time_ms, 0))",
            "metadata": {
                "comment": "The percentage of time that the vehicle is considered to be severe speeding."
            },
        }
    ],
    dimensions=[
    ...
    upstreams=[
        ...
        AssetKey(["scoringdb_shards", "trip_scores"]),
...
def vehicle_severe_speeding(context: AssetExecutionContext) -> str:
