...
from dataweb.userpkgs.constants import AWSRegion, Database, DATAENGINEERING, ColumnDescription, REGIONAL_STG_ORGANIZATION_CATEGORY_DEPENDENCIES
from dataweb.userpkgs.utils import get_metric_sql, get_all_regions


@metric(
    description="(Regional) Tracks the number of drowsiness events detected in VG devices",
    expressions=[
        {
            "expression_type": "sum",
            "expression": "SUM(COALESCE(drowsy_count, 0))",
            "metadata": {
                "comment": "The total drowsy event count for the device."
            },
        },
        {
            "expression_type": "avg",
            "expression": "SUM(COALESCE(drowsy_count, 0)) / (SUM(COALESCE(distance_miles, 0)) / 1000)",
            "metadata": {
                "comment": "The number of drowsiness events per 1000 miles"
            },
        }
    ],
    dimensions=[
...
def vehicle_drowsiness(context: AssetExecutionContext) -> str:
