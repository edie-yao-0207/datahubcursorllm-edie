...
@metric(
    description="(Regional) Count of drivers with infringements",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CONCAT(org_id, '_', driver_id))",
            "metadata": {
                "comment": "The total number of drivers with an infringement"
            },
        },
    ],
    dimensions=[
        ...
        },
        {
            "name": "violation_id",
            "metadata": {
                "comment": "Type of violation that the driver committed"
            },
        },
        {
            "name": "avg_mileage",
    ...
    upstreams=[
        AssetKey(["eucompliancedb_shards", "infringements"]),
...
def infringement_drivers(context: AssetExecutionContext) -> str:
