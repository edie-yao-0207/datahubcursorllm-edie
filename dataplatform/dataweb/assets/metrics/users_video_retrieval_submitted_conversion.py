...
@metric(
    description="The percentage of users that have Submitted the safety video retrieval request.",
    expressions=[
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN is_submitted_conversion = 1 THEN distinct_id END) * 1.0 / COUNT(DISTINCT distinct_id)",
            "metadata": {
                "comment": "The percentage of users that have Submitted the safety video retrieval request."
            },
        }
    ],
    dimensions=[
...
def users_video_retrieval_submitted_conversion(context: AssetExecutionContext) -> str:
