...
SELECT
  ...
  s.sossignaluuid,
  i.incidentstatus AS incident_status,
  i.priority AS incident_priority,
  i.incidentuuid,
  i.assignedtopolymorphicuserid AS assigned_user_id,
...
JOIN orgs o
  ON s.orgid = o.org_id
  AND s.date = o.date
JOIN dynamodb.worker_safety_incidents i
  ON s.incidentuuid = i.incidentuuid
  AND s.orgid = i.orgid
LEFT OUTER JOIN internal_users e
  ON SPLIT_PART(i.assignedtopolymorphicuserid, '-', 2) = e.user_id
  AND SPLIT_PART(i.assignedtopolymorphicuserid, '-', 1) = 'user'
