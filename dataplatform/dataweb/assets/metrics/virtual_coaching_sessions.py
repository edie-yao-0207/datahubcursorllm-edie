...
@metric(
    description="(Regional) Virtual coaching sessions",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(CASE WHEN completed_date IS NOT NULL THEN uuid END)",
            "metadata": {
                "comment": "The number of completed virtual coaching sessions"
            },
        },
    ],
    dimensions=[
    ...
    upstreams=[
        AssetKey(["coachingdb_shards", "coaching_sessions"]),
...
def virtual_coaching_sessions(context: AssetExecutionContext) -> str:
