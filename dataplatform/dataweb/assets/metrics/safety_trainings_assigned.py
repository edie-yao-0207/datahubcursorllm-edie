...
@metric(
    description="(Regional) Tracks assigned safety trainings",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(CASE WHEN assigned_at IS NOT NULL THEN uuid END)",
            "metadata": {
                "comment": "The number of assigned safety trainings"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(CASE WHEN assigned_at IS NOT NULL THEN uuid END) * 1.0 / COUNT(uuid)",
            "metadata": {
                "comment": "The percent of assigned safety trainings"
            },
        },
    ],
    dimensions=[
        {
            "name": "date",
            "metadata": {
                "comment": "The calendar date when the safety training was assigned in YYYY-mm-dd format"
            },
        },
        {
            "name": "org_id",
        ...
        },
        {
            ...
            "metadata": {
                "comment": "Driver that training is assigned to"
            },
        },
        {
        ...
        },
        {
            "name": "is_internal",
            "metadata": {
                "comment": "Whether the assigned user is an internal user or not"
            },
        },
    ],
    regions=get_all_regions(),
    owners=[DATAENGINEERING],
    glossary_terms=[GlossaryTerm.SAFETY, GlossaryTerm.TRAINING],
    upstreams=[
        AssetKey(["formsdb_shards", "form_submissions"]),
        AssetKey(["trainingdb_shards", "course_behavior_mappings"]),
        AssetKey(["trainingdb_shards", "courses"]),
...
def safety_trainings_assigned(context: AssetExecutionContext) -> str:
