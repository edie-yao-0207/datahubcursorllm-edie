...
@metric(
    ...
    expressions=[
        {
            ...
            "expression": "COUNT(DISTINCT CASE WHEN tags_size > 0 AND attributes_size > 0 THEN device_id END)",
            "metadata": {
                "comment": "The count of devices with at least one attribute and at least one tag"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN tags_size > 0 AND attributes_size > 0 THEN device_id END) * 1.0 / COUNT(DISTINCT device_id)",
            "metadata": {
                "comment": "The percent of devices with at least one attribute and at least one tag"
            },
        },
    ],
    dimensions=[
    ...
    upstreams=[
        ...
        AssetKey([Database.DATAMODEL_CORE, "lifetime_device_activity"]),
        AssetKey(["clouddb", "tag_devices"]),
        AssetKey(["clouddb", "tags"]),
...
def device_tags_attributes(context: AssetExecutionContext) -> str:
