...
from dataweb.userpkgs.constants import DATAENGINEERING, REGIONAL_STG_ORGANIZATION_CATEGORY_DEPENDENCIES, Database
from dataweb.userpkgs.utils import get_metric_sql, get_all_regions

@metric(
    description="Users who have been active in the Fleet App at least once",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN last_fleet_app_usage_date IS NOT NULL THEN user_id END)",
            "metadata": {
                "comment": "The total number of Fleet App users in the org"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN last_fleet_app_usage_date IS NOT NULL THEN user_id END) * 1.0 / COUNT(DISTINCT user_id)",
            "metadata": {
                "comment": "The percent of users who have been active in the Fleet App at least once"
            },
        },
    ],
    dimensions=[
        ...
        },
        {
            ...
            "metadata": {
                "comment": "Whether the email is internal or not"
            },
        },
        {
            "name": "avg_mileage",
    ...
    upstreams=[
        AssetKey([Database.DATAMODEL_PLATFORM, "dim_users_organizations"]),
        AssetKey([Database.DATAMODEL_PLATFORM_BRONZE, "raw_clouddb_users_organizations"]),
...
def admin_app_users(context: AssetExecutionContext) -> str:
