...
@metric(
    description="(Global) Response time between alert firing and when the corresponding Mixpanel link was opened",
    expressions=[
        {
            "expression_type": "avg",
            "expression": "AVG(CASE WHEN time_to_open_ms != -1 THEN time_to_open_ms END) / (1000 * 60)",
            "metadata": {
                "comment": """Average incident acknowledgment time (in minutes)"""
            },
        },
        {
            "expression_type": "median",
            "expression": "PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY CASE WHEN time_to_open_ms != -1 THEN time_to_open_ms END) / (1000 * 60)",
            "metadata": {
                "comment": """Median resolution time for incidents (in minutes)."""
            },
        },
    ],
    dimensions=[
        ...
        },
        {
            ...
            "metadata": {
                "comment": "The Industry classification of the account."
            },
        },
        {
        ...
        },
        {
            "name": "trigger_type",
            "metadata": {
                "comment": "Which type of alert was triggered"
            },
        },
        {
            "name": "alert_action_type",
            "metadata": {
                "comment": "Which action type is configured to be taken when the alert is triggered. For e.g., DashboardNotification, Webhook"
            },
        },
        {
            "name": "avg_mileage",
    ...
    upstreams=[
        AssetKey([Database.DATAMODEL_PLATFORM, "fct_alert_details_global"]),
...
def alerts_response_time(context: AssetExecutionContext) -> str:
