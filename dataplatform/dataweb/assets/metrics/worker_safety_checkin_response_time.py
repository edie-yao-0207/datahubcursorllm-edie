...
@metric(
    description="(Regional) Average response time for Worker Safety alarms",
    expressions=[
        {
            "expression_type": "avg",
            "expression": "AVG(DATE_DIFF(MINUTE, timer_start_at, recipient_acknowledged_at))",
            "metadata": {
                "comment": "Average response time in minutes for Worker Safety alarms"
            },
        },
    ],
    dimensions=[
...
def worker_safety_checkin_response_time(context: AssetExecutionContext) -> str:
