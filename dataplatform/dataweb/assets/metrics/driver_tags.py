...
@metric(
    description="(Regional) The percent of drivers with at least one tag",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN tags_size > 0 THEN CONCAT(org_id, '_', driver_id) END)",
            "metadata": {
                "comment": "The count of drivers with at least one tag"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN tags_size > 0 THEN CONCAT(org_id, '_', driver_id) END) * 1.0 / COUNT(DISTINCT CONCAT(org_id, '_', driver_id))",
            "metadata": {
                "comment": "The percent of drivers with at least one tag"
            },
        },
    ],
    dimensions=[
...
def driver_tags(context: AssetExecutionContext) -> str:
