...
@metric(
    description="(Global) Users that have engaged in at least one coaching session.",
    expressions=[
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN session_completed_time IS NOT NULL THEN id END) * 1.0 / COUNT(DISTINCT id)",
            "metadata": {
                "comment": "The percentage of users that have completed at least one coaching session."
            },
        },
    ],
    dimensions=[
...
def users_coaching_sessions_completed(context: AssetExecutionContext) -> str:
