...
@metric(
    description="(Regional) The percent of devices with at least one tag",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN tags_size > 0 THEN device_id END)",
            "metadata": {
                "comment": "The count of devices with at least one tag"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN tags_size > 0 THEN device_id END) * 1.0 / COUNT(DISTINCT device_id)",
            "metadata": {
                "comment": "The percent of devices with at least one tag"
            },
        },
    ],
    dimensions=[
...
def device_tags(context: AssetExecutionContext) -> str:
