...
@metric(
    description="Users who have been active in the Cloud Dashboard at least once",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN last_web_login_date IS NOT NULL THEN user_id END)",
            "metadata": {
                "comment": "The total number of users who have logged into the Cloud Dashboard at least once"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN last_web_login_date IS NOT NULL THEN user_id END) * 1.0 / COUNT(DISTINCT user_id)",
            "metadata": {
                "comment": "The percent of users who have been active in the Cloud Dashboard at least once"
            },
        },
    ],
    dimensions=[
...
def cloud_dashboard_users(context: AssetExecutionContext) -> str:
