...
@metric(
    description="(Global) The total number of customers with Safety-related API activity.",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT sam_number)",
            "metadata": {
                "comment": "The total number of customers with Safety-related API activity."
            },
        }
    ],
    dimensions=[
        ...
        {
            "name": "api_domain",
            "metadata": {"comment": "The safety domain area of the accessed api."},
        },
        {
            "name": "avg_mileage",
            "metadata": {
    ...
    glossary_terms=[GlossaryTerm.SAFETY, GlossaryTerm.MPR],
    upstreams=[
        AssetKey([Database.DATAENGINEERING, "safety_api_usage_global"]),
...
def customers_with_safety_api_activity(context: AssetExecutionContext) -> str:
