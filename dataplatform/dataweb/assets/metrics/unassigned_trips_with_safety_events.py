...
@metric(
    description="(Global) Safety events from trips where a driver was not assigned.",
    expressions=[
        {
            "expression_type": "percent",
            "expression": "SUM(IF(has_safety_event = 1, 1, 0)) * 1.0 / COUNT(has_safety_event)",
            "metadata": {
                "comment": "The percentage of trips with unassigned drivers having at least one safety event."
            },
        }
    ],
    dimensions=[
...
def unassigned_trips_with_safety_events(context: AssetExecutionContext) -> str:
