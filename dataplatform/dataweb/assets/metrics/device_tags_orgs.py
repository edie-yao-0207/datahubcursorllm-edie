...
@metric(
    description="(Regional) The percent of orgs having a device with at least one tag",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN tags_size > 0 THEN org_id END)",
            "metadata": {
                "comment": "The count of orgs having a device with at least one tag"
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN tags_size > 0 THEN org_id END) * 1.0 / COUNT(DISTINCT org_id)",
            "metadata": {
                "comment": "The percent of orgs having a device with at least one tag"
            },
        },
    ],
    dimensions=[
...
def device_tags_orgs(context: AssetExecutionContext) -> str:
