...
@metric(
    description="(Regional) Tracks the number of mobile usage incidents while driving (VG devices only).",
    expressions=[
        {
            "expression_type": "sum",
            "expression": "SUM(COALESCE(mobile_usage_count, 0))",
            "metadata": {
                "comment": "The total mobile usage incidents for the device"
            },
        },
        {
            "expression_type": "avg",
            "expression": "SUM(COALESCE(mobile_usage_count, 0)) / (SUM(COALESCE(distance_miles, 0)) / 1000)",
            "metadata": {
                "comment": "The number of mobile usage incidents per 1000 miles"
            },
        }
    ],
    dimensions=[
...
def vehicle_mobile_usage(context: AssetExecutionContext) -> str:
