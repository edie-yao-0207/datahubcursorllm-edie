...
@metric(
    description="(Global) The total number of customer dismissed safety inbox events.",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(CASE WHEN is_customer_dismissed = 1 THEN CONCAT(device_id, '_', event_ms) END)",
            "metadata": {"comment": "The total number of customer dismissed safety inbox events."},
        }
    ],
    dimensions=[
        {
...
def safety_inbox_events_dismissed(context: AssetExecutionContext) -> str:
