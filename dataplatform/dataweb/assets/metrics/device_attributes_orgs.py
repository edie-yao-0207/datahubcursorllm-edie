...
@metric(
    description="(Regional) The percent of orgs having a device with at least one attribute",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT CASE WHEN attributes_size > 0 THEN org_id END)",
            "metadata": {
                "comment": "The count of orgs having a device with at least one attribute"
            },
        },
        {
...
def device_attributes_orgs(context: AssetExecutionContext) -> str:
