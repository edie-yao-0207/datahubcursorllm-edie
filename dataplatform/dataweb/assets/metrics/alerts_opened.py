...
@metric(
    description="(Global) All raised alerts and whether they were opened in the dashboard or not.",
    expressions=[
        {
            "expression_type": "count",
            "expression": "COUNT(DISTINCT alert_id)",
            "metadata": {
                "comment": """The total number of incidents that fired"""
            },
        },
        {
            "expression_type": "percent",
            "expression": "COUNT(DISTINCT CASE WHEN mp_processing_time_ms > 0 THEN alert_id END) * 1.0 / COUNT(DISTINCT alert_id)",
            "metadata": {
                "comment": """The percent of alerts that were opened through the Mixpanel URL."""
            },
        },
    ],
    dimensions=[
...
def alerts_opened(context: AssetExecutionContext) -> str:
