...
@metric(
    description="(Regional) The ratio of total utilized time (time on trip) to total available time for Vehicle type devices by organization and lookback window.",
    expressions=[
        {
            "expression_type": "percent",
            "expression": "CASE WHEN SUM(total_available_hours) = 0 AND SUM(total_utilized_hours) > 0 THEN 100 ELSE SUM(total_utilized_hours * 1.0) / SUM(total_available_hours) END",
            "metadata": {
                "comment": "The ratio of time spent utilized on trip to available hours by date. Returns 100% if available hours is 0 but there are positive utilized hours."
            },
        }
    ],
    dimensions=[
    ...
    upstreams=[
        ...
        AssetKey([Database.PRODUCT_ANALYTICS_STAGING, "fct_device_utilization"]),
        AssetKey([Database.PRODUCT_ANALYTICS, "map_org_sam_number_latest"]),
        *REGIONAL_STG_ORGANIZATION_CATEGORY_DEPENDENCIES,
    ],
)
def vehicle_utilization_by_org(context: AssetExecutionContext) -> str:
