...
CREATE OR REPLACE TEMP VIEW speed_alert_two_distance AS (
  WITH trips AS (
    SELECT ft.date,
          ft.org_id,
          do.org_name,
  ...
  alert_two AS (
    SELECT date,
          ...
          revgeo_state,
          revgeo_country,
          COUNT(occurred_at_ms) AS num_alerts_two
    FROM data_analytics.speed_alert_two
  ...
  SELECT COALESCE(t.date,o.date) AS date,
        ...
        COALESCE(o.num_alerts_two,0) AS num_alerts_two
  FROM trips t
  LEFT JOIN alert_two o
    ON t.date = o.date
...
CREATE TABLE IF NOT EXISTS data_analytics.speed_alert_two_distance
USING delta
PARTITIONED BY (date)
AS
SELECT * FROM speed_alert_two_distance

-- COMMAND ----------

CREATE OR REPLACE TEMPORARY VIEW speed_alert_two_distance_updates AS (
  SELECT *
  FROM speed_alert_two_distance
  WHERE date BETWEEN COALESCE(NULLIF('$start_date',''),DATE_SUB(CURRENT_DATE(),3)) AND COALESCE(NULLIF('$end_date',''),DATE_SUB(CURRENT_DATE(),1))
);

MERGE INTO data_analytics.speed_alert_two_distance AS target
USING speed_alert_two_distance_updates AS updates
