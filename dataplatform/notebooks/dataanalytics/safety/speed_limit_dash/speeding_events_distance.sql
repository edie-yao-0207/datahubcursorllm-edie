...
CREATE OR REPLACE TEMP VIEW speeding_events_distance AS (
  WITH trips AS (
    SELECT ft.date,
          ft.org_id,
          do.org_name,
  ...
  events AS (
    SELECT date,
          ...
          revgeo_state,
          revgeo_country,
          COUNT(start_ms) AS num_events
    FROM data_analytics.speeding_events
  ...
  SELECT COALESCE(t.date,o.date) AS date,
        ...
        COALESCE(o.num_events,0) AS num_events
  FROM trips t
  LEFT JOIN events o
    ON t.date = o.date
...
CREATE TABLE IF NOT EXISTS data_analytics.speeding_events_distance
USING delta
PARTITIONED BY (date)
AS
SELECT * FROM speeding_events_distance

-- COMMAND ----------

CREATE OR REPLACE TEMPORARY VIEW speeding_events_distance_updates AS (
  SELECT *
  FROM speeding_events_distance
  WHERE date BETWEEN COALESCE(NULLIF('$start_date',''),DATE_SUB(CURRENT_DATE(),3)) AND COALESCE(NULLIF('$end_date',''),DATE_SUB(CURRENT_DATE(),1))
);

MERGE INTO data_analytics.speeding_events_distance AS target
USING speeding_events_distance_updates AS updates
