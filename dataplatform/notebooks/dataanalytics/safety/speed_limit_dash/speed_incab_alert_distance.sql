...
CREATE OR REPLACE TEMP VIEW speed_incab_alerts_distance AS (
  WITH trips AS (
    SELECT ft.date,
          ft.org_id,
          do.org_name,
  ...
  incab_alert AS (
    SELECT date,
          ...
          revgeo_state,
          revgeo_country,
          COUNT(alerted_at_ms) AS num_incab_alerts
    FROM data_analytics.speed_incab_alerts
  ...
  SELECT COALESCE(t.date,o.date) AS date,
        ...
        COALESCE(o.num_incab_alerts,0) AS num_incab_alerts
  FROM trips t
  LEFT JOIN incab_alert o
    ON t.date = o.date
...
CREATE TABLE IF NOT EXISTS data_analytics.speed_incab_alerts_distance
USING delta
PARTITIONED BY (date)
AS
SELECT * FROM speed_incab_alerts_distance

-- COMMAND ----------

CREATE OR REPLACE TEMPORARY VIEW speed_incab_alerts_distance_updates AS (
  SELECT *
  FROM speed_incab_alerts_distance
  WHERE date BETWEEN COALESCE(NULLIF('$start_date',''),DATE_SUB(CURRENT_DATE(),3)) AND COALESCE(NULLIF('$end_date',''),DATE_SUB(CURRENT_DATE(),1))
);

MERGE INTO data_analytics.speed_incab_alerts_distance AS target
USING speed_incab_alerts_distance_updates AS updates
