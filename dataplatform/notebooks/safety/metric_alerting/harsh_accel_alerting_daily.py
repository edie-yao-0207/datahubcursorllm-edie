...
# MAGIC We'll fetch normalized harsh accel from Feb to Mid March, and we aggregate on a weekly basis. The timestamp column must be called ds, and the value column must be called y. These should be the only two columns in the dataframe.
...
# MAGIC   where
# MAGIC     accel.detail_proto.accel_type = 1 and
# MAGIC     accel.date >= date_sub(current_date(), 50) and
# MAGIC     accel.date <= date_sub(current_date(), 14)
# MAGIC   group by
# MAGIC     date
# MAGIC );
...
# MAGIC Look back at the past month to evaluate breaks in data
...
# MAGIC   where
# MAGIC     accel.detail_proto.accel_type = 1 and
# MAGIC     accel.date >= date_sub(current_date(), 14) and
