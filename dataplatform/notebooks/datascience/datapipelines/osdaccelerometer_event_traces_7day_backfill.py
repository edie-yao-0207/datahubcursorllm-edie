...
osdaccel = (
    spark.table("kinesisstats.osdaccelerometer")
    .select("date", "org_id", "object_id", "time")
    .where(F.col("date").between(START_DATE, END_DATE))
)
et_written = (
    spark.table("dataprep.osdaccelerometer_event_traces")
    .select("date", "org_id", "object_id", "time")
    .where(F.col("date").between(START_DATE, END_DATE))
)
missing = (
    osdaccel.join(
        et_written,
        on=(
            (et_written.date == osdaccel.date)
            & (et_written.org_id == osdaccel.org_id)
            & (et_written.object_id == osdaccel.object_id)
            & (et_written.time == osdaccel.time)
        ),
        how="left",
    )
    .where(et_written.time.isNull())
    .select(osdaccel.date, osdaccel.org_id, osdaccel.object_id, osdaccel.time)
)

df_stg = df_stg.join(missing, on=["date", "org_id", "object_id", "time"])


df_stg = df_stg.withColumn("batch_id", F.hash("org_id") % NUM_BATCHES)
df_stg.write.partitionBy("batch_id").mode("overwrite").option(
    "overwriteSchema", "true"
).saveAsTable("datascience.stg_osdaccelerometer_event_traces_7d_backfill")

for batch_id in range(NUM_BATCHES):
    print(f"Running Batch: {batch_id+1} of {NUM_BATCHES}")
    start = datetime.now()
    print(f"Starting at {start}...")
    df = spark.table("datascience.stg_osdaccelerometer_event_traces_7d_backfill")
