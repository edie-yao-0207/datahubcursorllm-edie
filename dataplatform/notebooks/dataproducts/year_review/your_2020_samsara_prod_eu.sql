...
from delta.`s3://samsara-eu-report-staging-tables/report_aggregator/time_on_site_report/`
where date >= to_date('2020-01-01')
and date <= to_date('2020-11-30')
and org_id in (select org_id from dataproducts.eligible_orgs)
...
left join playground.ava_coaching_state cs
  on cs.enum = events.coaching_state
...
from delta.`s3://samsara-eu-report-staging-tables/report_aggregator/fuel_energy_efficiency_report/`
where date >= to_date('2020-01-01')
  and date <= to_date('2020-11-30')
...
-- MAGIC %sql
-- MAGIC select count(1) from your_2020_samsara_data;
...
-- MAGIC %python
-- MAGIC s3_bucket = f"s3://samsara-eu-year-review-metrics/year_review"
-- MAGIC
