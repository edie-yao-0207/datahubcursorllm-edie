   ...
   select
    ...
    "proto_value", named_struct(
        "dashcam_connection_state", named_struct(
            "camera_id", a.value.proto_value.dashcam_connection_state.camera_id
        )
    ),
...
dashcam_connected_to_vg_and_vg_connected_to_cloud_and_vg_full_power_intervals_hourly_df = create_intervals_hourly(
    dashcam_connected_to_vg_and_vg_connected_to_cloud_and_vg_full_power_intervals_by_camera_id_df,
    start_date,
    end_date,
)

dashcam_connected_to_vg_and_vg_connected_to_cloud_and_vg_full_power_and_dashcam_connected_to_cloud_intervals_hourly_df = create_intervals_hourly(
...
denominator_df = dashcam_connected_to_vg_and_vg_connected_to_cloud_and_vg_full_power_intervals_hourly_df.groupBy(
    "date", "org_id", "device_id"
).agg(
    F.sum(F.col("end_ms") - F.col("start_ms")).alias("total_ms")
)

numerator_df = dashcam_connected_to_vg_and_vg_connected_to_cloud_and_vg_full_power_and_dashcam_connected_to_cloud_intervals_hourly_df.groupBy(
    "date", "org_id", "device_id"
).agg(
    F.sum(F.col("end_ms") - F.col("start_ms")).alias("total_ms")
)

cm_device_hourly_cloud_connection_df = (
    denominator_df.join(
        numerator_df,
    ...
    .filter((denominator_df.org_id != 1))
    .select(
        denominator_df["*"],
...
create_or_update_table(
    "dataprep_firmware.cm_device_hourly_cloud_connection",
    cm_device_hourly_cloud_connection_df,
    "date",
    ["date", "org_id", "device_id"],
)
