...
create or replace temp view cell_info as (
    ...
    case 
      when product_type like "%VG5%" and COALESCE(cell.value.proto_value.interface_state.cellular.sim_slot, 0) = 1 then 'Vodafone' -- VG5x: Thor and Freya
      when product_type like "%VG5%" and COALESCE(cell.value.proto_value.interface_state.cellular.sim_slot, 0) = 2 then 'ATT' -- VG5x: Thor and Freya
  ...
  where 
    ...
    ---cell.date >= date(now() - interval 6 months) --added in to speed up scheduled run
...
create or replace temp view device_network_operator_time as (
  ...
  where dev.date <= date_sub(current_date(),1)
     --dev.date >= date_sub(current_date(),65)  --added in to speed up scheduled run
  group by
    dev.date,
    dev.org_id,
    dev.org_type,
