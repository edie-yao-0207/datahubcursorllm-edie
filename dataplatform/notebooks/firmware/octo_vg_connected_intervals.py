...
table_name = "dataprep_firmware.octo_vg_connected_intervals"
...
osdmulticamconnected_df = spark.sql(
    ...
    SELECT *
    FROM
        kinesisstats_history.osdmulticamconnected
    WHERE
        date >= DATE_SUB('{}', {})
        AND date <= '{}'
    --endsql
...
octo_connected_intervals_df = create_intervals_v2(
    osdmulticamconnected_df, lambda x: x == 1, query_start_ms, query_end_ms
)
...
octo_connected_intervals_hb_df = terminate_intervals_at_last_hb(
    octo_connected_intervals_df, heartbeats_df
)
...
octo_connected_intervals_daily_df = create_intervals_daily(
    octo_connected_intervals_hb_df, start_date, end_date
).withColumn(
...
octo_connected_intervals_daily_agg_df = octo_connected_intervals_daily_df.groupBy(
    "date", "org_id", "device_id"
).agg(F.sum(F.col("end_ms") - F.col("start_ms")).alias("octo_connected_ms"))

# Assert that no device exceeds 24 hours of connected time per day
max_duration_hours = octo_connected_intervals_daily_agg_df.select(
    F.max(F.col("octo_connected_ms") / (1000 * 60 * 60))
).collect()[0][0]
assert (
...
create_or_update_table(
    table_name,
    octo_connected_intervals_daily_df.filter(F.col("date") >= start_date),
    "date",
