...
create or replace temp view device_daily_anomalies as (
  select
    date,
    org_id,
    device_id,
...
create or replace temp view joined as (
  ...
  left join device_daily_anomalies as c
    on a.date = c.date
...
create table if not exists dataprep_firmware.device_daily_anomalies_by_service using delta
partitioned by (date)
select * from joined

-- COMMAND ----------

merge into dataprep_firmware.device_daily_anomalies_by_service as target
