...
table_name = "dataprep_firmware.cm_dashcam_state_intervals"

# COMMAND ----------

osddashcamstate_df = spark.sql(
    """
    select a.*
    from kinesisstats_history.osddashcamstate as a
...
# dashcam state with state value 1, 6, 7 are recording states
osddashcamstate_df = osddashcamstate_df.withColumn(
    "recording_reason",
    F.when(
        ~F.col("value.int_value").isin(1, 6, 7)
        | F.col("value.proto_value.dashcam_state.recording_reason").isNull(),
        F.array().cast("array<integer>"),
    ).otherwise(F.col("value.proto_value.dashcam_state.recording_reason")),
)
...
create_or_update_table(
    table_name,
    dashcam_state_intervals_df.filter(F.col("date") >= start_date),
    "date",
