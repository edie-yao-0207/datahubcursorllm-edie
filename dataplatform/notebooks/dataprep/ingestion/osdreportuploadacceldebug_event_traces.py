...
def deduplicate(iterable, field):
    event_type, trace_type = field.split(".")
...
def merge_trace_data(iterable, field, schema):
    ...
    for trace_slice in iterable:
        ...
        for element in trace_slice_data:
            #  fill timestamps
            if (
...
def transform_event_trace(s3_proto_value_list, field, s3_proto_value_list_schema):
    assert len(field.split(".")) == 2
    event_trace_data = findall(s3_proto_value_list, field)
    event_trace_data = list(map(lambda x: deduplicate(x, field), event_trace_data))
...
df = spark.table("kinesisstats.osdreportuploadacceldebug_with_s3_big_stat").where(
...
df = df.select(
    ...
    .over(
        W.partitionBy(
            "date", "stat_type", "org_id", "object_type", "object_id", "value.int_value"
        ).orderBy(F.col("time").asc())
    )
    .alias("trace_slice_idx"),
)
df = df.withColumn(
    "value",
    F.struct(
...
df = df.withColumn(
    "value", F.create_map(F.col("trace_slice_idx").cast("string"), F.col("value"))
)
df = df.withColumn(
    "s3_proto_value_slice",
    F.create_map(
...
df = df.groupBy(
    "date", "stat_type", "org_id", "object_type", "object_id", "int_value"
).agg(
    F.collect_list("s3_proto_value_slice").alias("s3_proto_value_list"),
    F.collect_list("value").alias("value_list"),
...
df = df.select(
    "*",
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.recent_accel", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
    ...
    )(F.col("s3_proto_value_list")).alias("recent_accel_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.recent_gps", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct("accel_event_data.recent_gps", S3_PROTO_VALUE_SCHEMA)
        ),
    )(F.col("s3_proto_value_list")).alias("recent_gps_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.accel_pedal", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct("accel_event_data.accel_pedal", S3_PROTO_VALUE_SCHEMA)
        ),
    )(F.col("s3_proto_value_list")).alias("accel_pedal_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.brake", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct("accel_event_data.brake", S3_PROTO_VALUE_SCHEMA)
        ),
    )(F.col("s3_proto_value_list")).alias("brake_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.recent_gyro", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct("accel_event_data.recent_gyro", S3_PROTO_VALUE_SCHEMA)
        ),
    )(F.col("s3_proto_value_list")).alias("recent_gyro_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.ecu_speed", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct("accel_event_data.ecu_speed", S3_PROTO_VALUE_SCHEMA)
        ),
    )(F.col("s3_proto_value_list")).alias("ecu_speed_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.cruise_control", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct(
    ...
    )(F.col("s3_proto_value_list")).alias("cruise_control_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.forward_vehicle_speed", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct(
    ...
    )(F.col("s3_proto_value_list")).alias("forward_vehicle_speed_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.forward_vehicle_distance", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct(
    ...
    )(F.col("s3_proto_value_list")).alias("forward_vehicle_distance_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.oriented_accel_raw", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct(
    ...
    )(F.col("s3_proto_value_list")).alias("oriented_accel_raw_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.oriented_gyro_raw", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct(
    ...
    )(F.col("s3_proto_value_list")).alias("oriented_gyro_raw_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.oriented_accel_lp_filtered", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct(
    ...
    )(F.col("s3_proto_value_list")).alias("oriented_accel_lp_filtered_trace"),
    F.udf(
        lambda x: transform_event_trace(
            x, "accel_event_data.oriented_gyro_lp_filtered", S3_PROTO_VALUE_SCHEMA
        ),
        returnType=T.ArrayType(
            gen_timeseries_struct(
    ...
    )(F.col("s3_proto_value_list")).alias("oriented_gyro_lp_filtered_trace"),
)

df.write.format("delta").mode("overwrite").option(
    "replaceWhere", f"date >= '{START_DATE}' AND date <= '{END_DATE}'"
).option("mergeSchema", "true").saveAsTable(
    "dataprep.osdreportuploadacceldebug_event_traces", partitionBy=["date", "org_id"]
)
