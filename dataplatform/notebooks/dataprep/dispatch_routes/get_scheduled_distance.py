...
# How far we look back for data to process in this job run
job_duration_hours = 6
job_duration_ms = job_duration_hours * 60 * 60 * 1000
...
end_ms = int(time_now.timestamp() * 1e3)
start_ms = end_ms - job_duration_ms

end_date = time_now.strftime("%Y-%m-%d")
start_date = (time_now - timedelta(hours=job_duration_hours)).strftime("%Y-%m-%d")

# the scheduled distance table
scheduled_distances = (
    spark.table("dataprep_routing.scheduled_distances")
    .where(
        f"route_scheduled_or_updated_date >= '{start_date}' and route_scheduled_or_updated_date <= '{end_date}'"
    )
    .select(
        "route_id",
        "job_id",
...
# last updated time
last_updated_time = (
    scheduled_distances.where(
        scheduled_distances["route_last_updated_time"] <= time_now
    )
    .agg({"route_last_updated_time": "max"})
    .collect()[0]["max(route_last_updated_time)"]
)

current_start_ms = start_ms
current_end_ms = min(start_ms + batch_duration_ms, end_ms)
...
while current_start_ms < end_ms:
    # Restrict to routes which
    # 1. have scheduled_start_ms within the current time window
    # 2. are associated with a job that has updated_at within the current time window
    # 3. has updated_at within the current time window
    filtered_completed_route_ids = (
        completed_routes.where(
            f"scheduled_start_ms >= {current_start_ms} and scheduled_start_ms < {current_end_ms}"
        )
        .select("route_id")
        .distinct()
    )

    # Routes with scheduled_start_ms within current time window
    filtered_completed_routes = filtered_completed_route_ids.join(
    ...
    recently_updated_route_ids = (
        dispatch_routes_table.where(
            f"""
            route_updated_at is not null
            and (CAST(to_unix_timestamp(route_updated_at) AS BIGINT) * 1000 >= {current_start_ms} and CAST(to_unix_timestamp(route_updated_at) AS BIGINT) * 1000 < {current_end_ms})
            """
        )
        .join(filtered_completed_route_ids, "route_id", "left_anti")
        .selectExpr(
            "route_id",
            "TO_DATE(route_updated_at) as route_scheduled_or_updated_date",
        )
    )

    recently_updated_job_route_ids = (
        dispatch_jobs_table.where(
            f"""
            job_updated_at is not null
            and (CAST(to_unix_timestamp(job_updated_at) AS BIGINT) * 1000 >= {current_start_ms} and CAST(to_unix_timestamp(job_updated_at) AS BIGINT) * 1000 < {current_end_ms})
            """
        )
        .join(filtered_completed_route_ids, "route_id", "left_anti")
        .join(recently_updated_route_ids, "route_id", "left_anti")
        .selectExpr(
            "route_id",
            "TO_DATE(job_updated_at) as route_scheduled_or_updated_date",
        )
        .groupBy("route_id")
        .agg({"route_scheduled_or_updated_date": "max"})
        .withColumnRenamed(
            "MAX(route_scheduled_or_updated_date)", "route_scheduled_or_updated_date"
        )
    )

    all_recently_updated_routes_ids = recently_updated_route_ids.union(
        recently_updated_job_route_ids
    )

    # Routes with updated_at or at least one job with updated_at within current time window
    recently_updated_routes = (
        all_recently_updated_routes_ids.alias("all_recently_updated_routes_ids")
        .join(completed_routes, "route_id")
        .selectExpr(
            "route_id",
            "scheduled_start_ms",
    ...
    # All routes to consider this
    selected_routes = filtered_completed_routes.union(recently_updated_routes).where(
        "scheduled_meters is null"
    )
    ...
    completed_routes_and_jobs_addr = job_address_ids.selectExpr(
        ...
        "if (route_created_at > route_updated_at, route_created_at, coalesce(route_updated_at, cast(0 as timestamp))) as route_recent_time",
        "coalesce(job_clouddb_lat, destination_lat) as dest_lat",
        "coalesce(job_clouddb_lng, destination_lng) as dest_lng",
        "if (job_created_at > job_updated_at, job_created_at, coalesce(job_updated_at, cast(0 as timestamp))) as job_recent_time",
        "scheduled_arrival_time",
    )

    # creating the list of route ids that need to be recalculated
    #   1. new routes
    #   2. new jobs
    #   3. deleted jobs
    #   4. out of date route
    #   5. out of date job
    route_ids_to_recalculate = (
        completed_routes_and_jobs_addr.join(
            scheduled_distances, ["route_id", "job_id"], "full"
        )
        .where(
            (
                completed_routes_and_jobs_addr["route_id"].isNotNull()
                & scheduled_distances["route_id"].isNull()
            )  # new route
            | (
                completed_routes_and_jobs_addr["job_id"].isNotNull()
                & scheduled_distances["job_id"].isNull()
            )  # new job
            | (
                completed_routes_and_jobs_addr["route_id"].isNotNull()
                & completed_routes_and_jobs_addr["job_id"].isNull()
                & scheduled_distances["job_id"].isNotNull()
            )  # deleted job
            | (
                completed_routes_and_jobs_addr["route_recent_time"] >= last_updated_time
            )  # updated route
            | (
                completed_routes_and_jobs_addr["job_recent_time"] >= last_updated_time
            )  # updated job
        )
        .select("route_id")
        .distinct()
    ...
    routes_to_recalculate = completed_routes_and_jobs_addr.join(
        route_ids_to_recalculate, "route_id"
    ).selectExpr(
        "route_id",
        "job_id",
        "scheduled_arrival_time",
    ...
    recalculated_rows = routes_to_recalculate.groupby("route_id").apply(
        recalculate_route
    )

    updates = recalculated_rows.where(recalculated_rows["error"] == "").selectExpr(
        "route_id",
        "job_id",
        "route_scheduled_start_ms",
    ...
    current_end_ms = min(current_start_ms + batch_duration_ms, end_ms)
